...
public class TechController extends BaseController{
	...
	@RequestMapping
	public ModelAndView index(Map<String, Object> out,PageDto<ProductsDto> page,ProductsDO product,
			NewsTechDTO newsTech){
		//页头询盘
		...
		out.put("feiplasg", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		product.setProductsTypeCode("10331001");
		out.put("feiplasq", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		//废金属热门供求
		product.setTitle("废金属");
		product.setProductsTypeCode("10331000");
		out.put("feimetg", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		product.setProductsTypeCode("10331001");
		out.put("feimetq", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		//废纸热门供求
		page.setPageSize(2);
		product.setTitle("废纸橡胶");
		product.setProductsTypeCode("10331000");
		out.put("paperg", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		page.setPageSize(3);
		product.setProductsTypeCode("10331001");
		out.put("paperq", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		//其它热门
		page.setPageSize(2);
		product.setTitle("其它废料");
		product.setProductsTypeCode("10331000");
		out.put("otherg", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		page.setPageSize(3);
		product.setProductsTypeCode("10331001");
		out.put("otherq", productsService.pageProductsBySearchEngine(product, null, null, page).getRecords());
		//最新展会
		out.put("newestExhibit", exhibitService.queryNewestExhibit(null, null, 4));
		SeoUtil.getInstance().buildSeo("tech", out);
		return null;
	}
	
	@RequestMapping
	public ModelAndView list(Map<String, Object> out,String code,String keywords,
			NewsTechDTO newsTech,PageDto<NewsTech> page) throws UnsupportedEncodingException{
			page.setPageSize(24);
	...
	@RequestMapping
	public ModelAndView detail(Map<String, Object> out,Integer id,NewsTechDTO newsTechDto){
		do{
			if (id==null) {
				break;
			}
			...
			if (onTech != null && newsTech.getCategoryCode().equals(onTech.getCategoryCode())) {
				out.put("onTech", onTech);
			}
			// 查询下一篇文章
			NewsTech downTech=newsTechService.queryDownNewsTechById(id);
			if (downTech != null && newsTech.getCategoryCode().equals(downTech.getCategoryCode())) {
			...
			SeoUtil.getInstance().buildSeo("techdetail", new String[] { newsTech.getTitle() },
