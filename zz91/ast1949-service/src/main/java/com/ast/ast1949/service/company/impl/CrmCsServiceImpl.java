...
//		sb=buildStringQuery(sb, "(area_name,area_province)", CategoryFacade.getInstance().getValue(search.getAreaCode()));

//		try {
//			buildDateRangQuery(cl, "gmt_end", search.getSvrEndFrom(), search.getSvrEndTo());
//			buildDateRangQuery(cl, "gmt_next_visit_phone", search.getNextVisitPhoneFrom(), search.getNextVisitPhoneTo());
//			buildDateRangQuery(cl, "gmt_visit", search.getVisitFrom(), search.getVisitTo());
...
//			}
			...
			//新分配未联系
//			if(search.getUnvisitFlag()!=null && search.getUnvisitFlag().longValue()==0){
...
//			cl.SetSortMode(SphinxClient.SPH_SORT_EXTENDED, page.getSort()+" "+page.getDir());
//			
...
//					CrmCsDto dto=crmCsDao.queryCoreCompanyById(Integer.valueOf(""+info.docId), search.getSvrCode());
//					if(dto!=null){
//						result.add(dto);
//					}
//				}
//			}
			...
			for(CrmCsDto dto:result){
				...
				//普通会员 黑色
				dto.setMembershipLabel(categoryFacade.getValue(CrmSvrService.PT_CODE));
				//再生通会员 绿色
...
//	private void buildDateRangQuery(SphinxClient cl, String column, Date from, Date to) throws SphinxException{
//		if(from!=null && to!=null){
//			cl.SetFilterRange(column, DateUtil.getMillis(from), DateUtil.getMillis(to), false);
//		}
//	}
