...
public class MapServiceImpl implements MapService {
	...
	public List<ProductsDto> indexHotProductsByArea(String industryCode,
		...
		// size=60;
		// }
		// if("feijinshu".equals(industryCode)){
		// industryCode= "1000";
		// }else if("feisuliao".equals(industryCode)){
		// industryCode = "1001";
		// }
		// 综合废料 区分 金属和塑料 的其他
		...
		List<ProductsDO> plist = productsService.queryNoRepeatProducts(
				industryCode, areaCode, keywords, typeCode, page);
		for (ProductsDO obj : plist) {
			ProductsDto dto = new ProductsDto();
			dto.setProducts(obj);
			dto.setCompany(companyService.queryCompanyById(obj.getCompanyId()));
			list.add(dto);
		}
		return list;
	}
	...
	public List<ProductsDto> indexHotCompanysByArea(String industryCode,
		...
		page.setPageSize(size);
		List<CompanyDto> clist = companyService
				.queryCompanyZstMemberByAreacode(industryCode, areaCode,
						keywords, page);
	...
	public PageDto<ProductsDto> pageHotCompanysByArea(String industryCode,
			String areaCode, String keywords, Integer size,
			PageDto<ProductsDto> page) {
		page.setRecords(indexHotCompanysByArea(industryCode, areaCode,
				keywords, size, page));
		page.setTotalRecords(companyService
				.countQueryCompanyZstMemberByAreacode(industryCode, areaCode,
						keywords));
		return page;
	}
	...
	public PageDto<ProductsDto> pageHotProductsByArea(String industryCode,
			String areaCode, String keywords, String typeCode, Integer size,
			PageDto<ProductsDto> page) {
		page.setRecords(indexHotProductsByArea(industryCode, areaCode,
				keywords, typeCode, size, page));
		page.setTotalRecords(productsService.countQueryNoRepeatProducts(
				industryCode, areaCode, keywords, typeCode));
		return page;
	}
	...
	public PageDto<ProductsDto> queryBySearchEnegine(String province,
		...
		for (ProductsDto p : page.getRecords()) {
			p.setProducts(productsService.queryProductsById(p.getProducts()
					.getId()));
			if (p.getProducts() != null) {
				p.setCompany(companyService.queryCompanyById(p.getProducts()
						.getCompanyId()));
			}
			pList.add(p);
		}
