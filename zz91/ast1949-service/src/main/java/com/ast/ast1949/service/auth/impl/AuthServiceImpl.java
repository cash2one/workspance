...
public class AuthServiceImpl implements AuthService {
	...
	public boolean resetPasswordFromForgotPasswordKey(String am, String k,
			String pwd1) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		do {
			...
			//验证用户是否正确
			if(!am.equals(AlgorithmUtils.MD5(o.getEmail()+o.getUsername(), AlgorithmUtils.LENGTH))){
				break;
			}
			//更新密码
			userDao.updatePasswordByUsername(o.getUsername(),
					AlgorithmUtils.MD5(pwd1, AlgorithmUtils.LENGTH));
			companyAccountDao.updatePassword(o.getUsername(), pwd1);
//			companyContactsDAO.updatePasswordByAccount(o.getUsername(), pwd1);
			//销毁Key
...
//			.append(AlgorithmUtils.MD5(o.getEmail()+o.getUsername(), AlgorithmUtils.LENGTH))
//			.append("&k=").append(key)
...
//				, title, sb.toString());
		...
		map.put("am", AlgorithmUtils.MD5(o.getEmail()+o.getUsername(), AlgorithmUtils.LENGTH));
		map.put("k", key);
		map.put("url", url);
		map.put("email", email);
		MailUtil.getInstance().sendMail(title, email, "zz91-repwd", map, MailUtil.PRIORITY_HEIGHT);
	}
...
//			String newPassword, String verifyPassword) throws AuthorizeException, NoSuchAlgorithmException, UnsupportedEncodingException{
...
//		}
//		if(!AlgorithmUtils.MD5(originalPassword, AlgorithmUtils.LENGTH).equals(user.getPassword())){
