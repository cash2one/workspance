...
Ext.onReady(function(){
	var _url = Context.ROOT + "/zz91/admin/charts/edit.htm";
	
	var _editPanel = ast.ast1949.admin.charts.category.editPanel({
	...
	var _left = new Ext.Panel({
		title:  '类别信息<span style="color:red">【点击右键更多操作】</span>',
		region: 'west',
		collapsible:true,
		margins:  "0 0 2 2",
		cmargins: "0 2 2 2",
		...
		minSize:120,
		autoScroll:true,
		items:[{html:"<div id='category-tree'></div>"}]
	});
	...
	var _tree = ast.ast1949.admin.charts.category.treePanel({el:"category-tree",rootData:""});
	...
	_tree.on("click",function(node,event){
		...
		f.findById("combo-relevanceId").setValue("请选择...");
    ...
    function doDelete(_btn){
		...
		conn.request({
			url: Context.ROOT + Context.PATH + "/admin/charts/delete.htm?id="+_id,
			method : "get",
			scope : this,
  ...
  //更新
    ...
    function resumeForm(f){
		...
		f.findById("combo-relevanceId").setValue("");
		Ext.get("relevanceId").dom.value="";
