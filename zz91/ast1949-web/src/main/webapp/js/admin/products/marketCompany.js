...
ast.ast1949.admin.products.market.SEX=["男","女"];

ast.ast1949.admin.products.market.CSMAP={};

var ACCOUNT = new function(){
	this.ACCOUNT_GRID = "accountgrid";
}
ast.ast1949.admin.products.market.ACCOUNTFIELD=[
        {name:"id",mapping:"id"},
...
ast.ast1949.admin.products.market.COMPANYFIELD=[
...
ast.ast1949.admin.products.market.CompanyForm=Ext.extend(Ext.form.FormPanel,{
...
//				})
			...
			buttons:[{
				...
				text:"加入黑名单",
				handler:function(btn){
					ast.ast1949.admin.products.market.block(form.companyId)
				}
			},{
				text:"取消黑名单",
				handler:function(){
		...
		ast.ast1949.admin.products.market.CompanyForm.superclass.constructor.call(this,c);
		
	},
	fields:ast.ast1949.admin.products.market.COMPANYFIELD,
	companyId:0,
...
ast.ast1949.admin.products.market.CompanyAccountGrid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		config=config||{};
		Ext.apply(this,config);
		
		var _store = new Ext.data.JsonStore({
			fields : ast.ast1949.admin.products.market.ACCOUNTFIELD,
		...
		var _cm=new Ext.grid.ColumnModel( [_sm,{
			...
			dataIndex:"sex",
			renderer:function(value,metadata,record,rowindex,colindex,store){
				return ast.ast1949.admin.products.market.SEX[value];
			}
		},{
			header : "职位",
			dataIndex:"position"
		...
		var _tbar=config.tbar||[{
			...
			handler:function(btn){
				...
				ast.ast1949.admin.products.market.updateCompanyAccount(rows.get("companyId"));
			}
		},{
			text:"重置密码",
			iconCls:"item-key",
			handler:function(btn){
				...
				if(row!=null && typeof row != "undefined"){
					ast.ast1949.admin.products.market.resetPassword(row.get("account"),row.get("password"),row.get("mobile"),row.get("email"));
				}
			}
		}]
		...
		ast.ast1949.admin.products.market.CompanyAccountGrid.superclass.constructor.call(this,c);
	},
...
//账户信息修改
ast.ast1949.admin.products.market.updateCompanyAccount=function(companyId){
	
	var form=new ast.ast1949.admin.products.market.UpdateCompanyAccountForm({
	});
...
ast.ast1949.admin.products.market.UpdateCompanyAccountForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		ast.ast1949.admin.products.market.UpdateCompanyAccountForm.superclass.constructor.call(this,c);
	},
...
//密码重置
ast.ast1949.admin.products.market.resetPassword=function(account,password,mobile,email){
	
	var form=new ast.ast1949.admin.products.market.ResetPasswordForm({
	});
...
ast.ast1949.admin.products.market.ResetPasswordForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		ast.ast1949.admin.products.market.ResetPasswordForm.superclass.constructor.call(this,c);
	},
...
ast.ast1949.admin.products.market.CompanyAccountContactGrid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		config=config||{};
		Ext.apply(this,config);
...
//			totalProperty:'totalRecords',
		...
		ast.ast1949.admin.products.market.CompanyAccountContactGrid.superclass.constructor.call(this,c);
	}
});

ast.ast1949.admin.products.market.Grid = Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		var reader = [ {name:"id",mapping:"company.id"},
		   ...
		   {name:"marketName",mapping:"marketName"},
		   {name:"membershipLabel",mapping:"membershipLabel"},
		   {name:"membershipCode",mapping:"company.membershipCode"},
		   {name:"gmtLastLogin",mapping:"account.gmtLastLogin"}];

		var storeUrl = Context.ROOT + Context.PATH + "/admin/products/queryCompanyBymarketId.htm";
		...
		store:new Ext.data.JsonStore({
			fields : ['name', 'value'],
			data   : [
					{name:'普通会员',value:10051000},
					{name:'再生通会员',value:10051001},
					{name:'来电宝',value:10051003},
					{name:'银牌品牌通会员',value:100510021000},
					{name:'金牌品牌通会员',value:100510021001},
					{name:'钻石品牌通会员',value:100510021002}
			]
		}),
		listeners:{
			"blur":function(field){
				...
				if(Ext.get(field.getId()).dom.value!=""){
					B["membershipCode"] = field.getValue();
				}else{
		...
		ast.ast1949.admin.products.market.Grid.superclass.constructor.call(this,c);
	},
	loadDefault:null,
	contextmenu:null
 ...
 * 分配客户
 * */
ast.ast1949.admin.products.market.reassign=function(grid,oldassignArr,cs, csname){
...
// 拉黑表单
ast.ast1949.admin.products.market.block=function(companyId){
	
	var form=new ast.ast1949.admin.products.market.blockForm({
	});
	form.loadCompanyAccount(companyId);
	var win = new Ext.Window({
		id:"blockwin",
...
ast.ast1949.admin.products.market.blockForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		ast.ast1949.admin.products.market.blockForm.superclass.constructor.call(this,c);
	},
	loadCompanyAccount:function(companyId){
		var form=this;
		if(form.store!=null){
