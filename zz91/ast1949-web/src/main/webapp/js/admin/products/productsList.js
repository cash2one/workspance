...
//			}
		...
		var reader = [{name:"id",mapping:"products.id"},
			  ...
			  {name:"score",mapping:"products.score"}];
...
//			"checkPerson", "companyName","checkStatus"]
		...
		var storeUrl = Context.ROOT + Context.PATH + "/admin/products/listProducts.htm";
	...
	,"->",{
		...
		listeners:{
			"blur":function(field){
				...
				B = B||{};
				if(Ext.get(field.getId()).dom.value!=""){
					B["expireFlag"] = field.getValue();
				}else{
					B["expireFlag"]=undefined;
				}
				_store.baseParams = B;
				_store.reload({params:{"startIndex":0, "pageSize":Context.PAGE_SIZE}});
			}
