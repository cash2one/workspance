...
Ext.onReady(function(){
	...
	var _url = Context.ROOT + "/zz91/admin/productsalbums/edit.htm";

	var _editPanel = ast.ast1949.admin.productsAlbums.editPanel({
	...
	var _left = new Ext.Panel({
		title:  '相册类别<span style="color:red">【点击右键更多操作】</span>',
		region: 'west',
		collapsible:true,
		margins:  "0 0 2 2",
		cmargins: "0 2 2 2",
		...
		minSize:120,
		layout: 'fit',
		items:[{html:"<div id='productsAlbums-tree'></div>"}]  //item,{title:"添加广告组",load:{url:"http://www.g.cn"}}
	});
	...
	var _tree = ast.ast1949.admin.productsAlbums.treePanel({el:"productsAlbums-tree",rootData:""});
...
//    	alert("context click")
    ...
    function doDelete(_btn){
		...
		conn.request({
			url: Context.ROOT + Context.PATH + "/admin/productsalbums/delete.htm?id="+_id,
			method : "get",
			scope : this,
    ...
    function resumeForm(f){
		...
		f.findById("productsAlbumsId").setValue("0");
