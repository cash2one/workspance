Ext.namespace("ast.ast1949.admin.yuanliao.yuanliaoListExportInquiry");

/**
 * 用于提供搜索输入框的表单
 * */
ast.ast1949.admin.yuanliao.yuanliaoListExportInquiry.searchForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		...
		ast.ast1949.admin.yuanliao.yuanliaoListExportInquiry.searchForm.superclass.constructor.call(this,c);
...
/**
 * 用于显示各种条件查询后的原料供求信息列表
 * */
ast.ast1949.admin.yuanliao.yuanliaoListExportInquiry.resultGrid = Ext.extend(Ext.grid.GridPanel,{
	readOnly:false,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		var cm = new Ext.grid.ColumnModel([sm,{
				...
				width:80,
				renderer : function(value, metadata, record, rowIndex,colIndex, store) {
					return "<a href='"+Context.ROOT+Context.PATH+"/yuanliao/createInquiry.htm?id="+record.get("id")+"' target='blank'>转询盘</a>";
				}
			},{
				header : "次数",
				sortable : true,
				...
				renderer : function(value, metadata, record, rowIndex,colIndex, store) {
					return "<a href='"+Context.ROOT+Context.PATH+"/admin/inquiry/listOfYuanLiaoExport.htm?yuanLiaoId="+record.get("id")+"' target='blank'>"+value+"</a>";
				}
			},{
				header : "发送时间",
				sortable : true,
		...
		// 字段信息
		var reader = [{name:"id",mapping:"yuanliao.id"},
			  {name:"post_time",mapping:"yuanliao.postTime"},//发布时间
			  ...
			  {name:"isPause",mapping:"yuanliao.isPause"},//是否发布
			  {name:"isVip",mapping:"yuanliao.isVip"},//是否高会
			  ...
			  {name:"countInquiry",mapping:"countInquiry"},//询盘次数
			  {name:"isExpire",mapping:"isExpire"},//过期状态
			  {name:"gmtInquiryStr",mapping:"gmtInquiryStr"}//群发时间
			  ];

		var storeUrl = Context.ROOT + Context.PATH + "/yuanliao/listYuanLiaoExportInquiry.htm";
...
//			plugins : expander,
		...
		ast.ast1949.admin.yuanliao.yuanliaoListExportInquiry.resultGrid.superclass.constructor.call(this,c);
	},
