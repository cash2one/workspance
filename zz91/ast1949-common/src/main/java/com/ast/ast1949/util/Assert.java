 ...
 * For example: <pre class="code"> Assert.notNull(clazz,
 * "The class must not be null"); Assert.isTrue(i > 0,
 * "The value must be greater than zero");</pre> Mainly for internal use within
 * the framework; consider Jakarta's Commons Lang >= 2.0 for a more
 * comprehensive suite of assertion utilities.
 *
 * @author Keith Donald
...
public abstract class Assert {
    ...
    public static void doesNotContain(String textToSearch, String substring, String message) {
        if (textToSearch != null && textToSearch.length() > 0 && substring != null
                && substring.length() > 0 && textToSearch.indexOf(substring) != -1) {
            throw new IllegalArgumentException(message);
        }
    }
     ...
     * <code>null</code> and must have at least one element. <pre
