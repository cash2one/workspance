...
public class PriceController extends BaseController {
	...
	@RequestMapping
	public ModelAndView priceSearch(String title, String p,
	...
	public ModelAndView priceDetails(Integer id, String code,
		...
		if ("metal".equals(code)) {
			...
			if (priceCategory != null && priceCategory.getName() != null) {
				keywords = keywords + ",金属," + priceCategory.getName() + ","
						+ pindao + "价格,今日" + pindao + "价格";
			}
		} else if ("plastic".equals(code)) {
			pindao = "废塑料";
			if (priceCategory != null && priceCategory.getName() != null) {
				keywords = keywords + ",塑料,废料,价格,行情,POM," + pindao + "价格,今日"
						+ pindao + "价格";
			}
		} else {
			pindao = "综合废料";
			if (priceCategory != null && priceCategory.getName() != null) {
				keywords = keywords + "," + pindao + "价格,行情,行情动态," + pindao
						+ "价格,今日" + pindao + "价格";

			}
		}
		SeoUtil.getInstance().buildSeo("price",
				new String[] { price.getTitle(), pindao },
				new String[] { keywords },
				new String[] { price.getTitle(), pindao }, out);
		if (inDays <= 3) {
		...
		if ("plastic".equals(out.get("code"))) {
			List<ProductsDto> list = productsService
					.queryProductsForSpotByCondition(null, null, null, 5);
			List<ProductsDto> nlist = new ArrayList<ProductsDto>();
			for (ProductsDto obj : list) {
				// 获取三级类目的类别名
				String mainCode = obj.getProducts()
