...
public class CrmCsDaoImpl extends BaseDaoSupport implements CrmCsDao {
	...
	public Integer insertCs(String csAccount, Integer companyId) {
		...
		root.put("csAccount", csAccount);
		root.put("companyId", companyId);
		return (Integer) getSqlMapClientTemplate().insert(addSqlKeyPreFix(SQL_PREFIX, "insertCs"), root);
	}
	...
	public Integer updateLogInfo(Integer companyId, String csAccount,
		...
		return getSqlMapClientTemplate().update(addSqlKeyPreFix(SQL_PREFIX, "updateLogInfo"), root);
	}

	@Override
	public CrmCs queryCsOfCompany(Integer companyId) {
		
		return (CrmCs) getSqlMapClientTemplate().queryForObject(addSqlKeyPreFix(SQL_PREFIX, "queryCsOfCompany"), companyId);
	}
	...
	public List<CrmCsDto> queryCoreCompany(CrmSearchDto search,
		...
		root.put("page", page);
		return getSqlMapClientTemplate().queryForList(addSqlKeyPreFix(SQL_PREFIX, "queryCoreCompany"), root);
	}
	...
	public Integer queryCoreCompanyCount(CrmSearchDto search) {
		Map<String, Object> root=new HashMap<String, Object>();
		root.put("search", search);
		return (Integer) getSqlMapClientTemplate().queryForObject(addSqlKeyPreFix(SQL_PREFIX, "queryCoreCompanyCount"), root);
	}

	@Override
	public String queryCsAccountByCompanyId(Integer companyId) {
		
		return (String) getSqlMapClientTemplate().queryForObject(addSqlKeyPreFix(SQL_PREFIX, "queryCsAccountByCompanyId"), companyId);
	}
