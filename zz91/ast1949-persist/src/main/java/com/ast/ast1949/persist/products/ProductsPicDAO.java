...
public interface ProductsPicDAO {
	 ...
	 * @return true 返回查询所有结果集
	 *         false 返回 o
	 *
	 */
    public List<ProductsPicDO> queryProductsPicByCondition(ProductsPicDTO productsPicDTO);
    ...
    public int getProductsPicRecordCountByCondition(ProductsPicDTO productsPicDTO);
...
//    public List<ProductsPicDO> queryProductPicByproductId(int productId);
     ...
     * @param productsDTO为供求信息查询条件<br/>
     * 		productsDTO.getProductsDO().getCompanyId()不能为空，否则抛出异常
     * @return ProductsPicDTO信息集合，可为空
     */
//    public List<ProductsPicDTO> queryAllProductsPicByTitle(ProductsDTO productsDTO);
...
//    public Integer queryAllProductsPicByTitleCount(ProductsDTO productsDTO);
     ...
     * @param productsDO为供求信息，不能为空，否则抛出异常
     * @return ProductsPicDTO信息集合，可为空
     */
//    public List<ProductsPicDTO> queryforthProductsPic(ProductsDO productsDO);
     ...
     * @return 返回记录条数，没查询到返回为0
     */
//    public Integer queryAllProductsPicByHadSubSeriesCount(ProductsDTO productsDTO);
     ...
     * @return ProductsPicDTO信息集合，可为空
     */
//    public List<ProductsPicDTO> queryAllProductsPicByHadSubSeries(ProductsDTO productsDTO);
     ...
     * @return 返回记录条数，没查询到返回为0
     */
//    public Integer queryAllProductsPicByNoHadSubSeriesCount(ProductsDTO productsDTO);
     ...
     * @return ProductsPicDTO信息集合，可为空
     */
//    public List<ProductsPicDTO> queryAllProductsPicByNoHadSubSeries(ProductsDTO productsDTO);
    
    /**
     * 根据供求ID查询供求图片信息
     * @param productId为供求id，不能为空
     * @return ProductsPicDO信息集合，可为空
     */
    public List<ProductsPicDO> queryProductPicInfoByProductsId(Integer productId);
    
    /**
     * 根据供求id查询供求图片信息 (必须是通过审核的)
     * 使用于前台页面展示
     * @param productId
     * @return
     */
    ...
    /**
     * 为图片添加供求信息关联
     * @param productId：供求信息ID，不能为null
     * @param id：图片ID，不能为null
     * @return
     */
