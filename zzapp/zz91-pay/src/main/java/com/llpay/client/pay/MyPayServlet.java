...
import com.llpay.client.utils.PayUtil;
...
public class MyPayServlet extends HttpServlet{
    ...
    private void prepositPay(HttpServletRequest req, OrderInfo order)
    {
        ...
        paymentInfo.setUserreq_ip(PayUtil.getIpAddr(req).replaceAll("\\.", "_"));
        paymentInfo.setUrl_order("");
        paymentInfo.setValid_order("10080");// 单位分钟，可以为空，默认7天
        paymentInfo.setRisk_item(createRiskItem());
        paymentInfo.setTimestamp(PayUtil.getCurrentDateTimeStr());
        if (!PayUtil.isnull(req.getParameter("no_agree")))
        {
        ...
        // 加签名
        String sign = PayUtil.addSign(JSON.parseObject(JSON
    ...
    private void plainPay(HttpServletRequest req, OrderInfo order)
    {
        ...
        paymentInfo.setMoney_order(order.getMoney_order());
        paymentInfo.setNotify_url(PartnerConfig.NOTIFY_URL);
        paymentInfo.setUrl_return(PartnerConfig.URL_RETURN+"&no_order="+order.getNo_order()+"&dt_order="+order.getDt_order()+"&serviceId="+req.getParameter("name_goods"));
        paymentInfo.setUserreq_ip(PayUtil.getIpAddr(req).replaceAll("\\.", "_"));
        paymentInfo.setUrl_order("");
        paymentInfo.setValid_order("10080");// 单位分钟，可以为空，默认7天
        paymentInfo.setTimestamp(PayUtil.getCurrentDateTimeStr());
	...
	private OrderInfo createOrder(HttpServletRequest req) {
		...
		orderInfo.setNo_order(PayUtil.getCurrentDateTimeStr());
		orderInfo.setDt_order(PayUtil.getCurrentDateTimeStr());
		orderInfo.setMoney_order(req.getParameter("money_order"));
		String nameGoods = req.getParameter("name_goods");
		// 如果没有传进金钱，用配置中的默认配置
		if (StringUtils.isEmpty(req.getParameter("money_order"))) {
			orderInfo.setMoney_order(getMonkey(nameGoods));
		}
		orderInfo.setName_goods(getServiceName(nameGoods));
		orderInfo.setInfo_order("用户购买" + nameGoods);
		return orderInfo;
	}
	
	private String getMonkey(String nameGoods){
		String monkey = "0.01";
		String value = ParamUtils.getInstance().getValue("zz91_pay", nameGoods);
	...
	private String getServiceName(String nameGoods){
		String name = "ZZ91再生网";
		String value = ParamUtils.getInstance().getValue("zz91_pay", nameGoods);
		String[] valueArray = value.split("|");
