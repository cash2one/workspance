...
public class SMSHttpClient {
	...
	public int sendSMSAndLog(String[] mobiles, String content, String title,
			String typeCode, SMSFlagEnum flag,Integer [] companyId) throws RemoteException {
		int value = -1;
		if(content != null && content.length() > 2){
			value = binding.sendSMS(softwareSerialNo, key, "", mobiles, content,
					ADDSERIAL, CHARSET, SMS_PRIORITY, 0);
			int statId = new SMSStatImpl().recordStat(title, content,
					mobiles.length, typeCode, value);
			new SMSLogImpl().recordLogs(mobiles, statId, companyId, flag);
		}
		return value;
	}
