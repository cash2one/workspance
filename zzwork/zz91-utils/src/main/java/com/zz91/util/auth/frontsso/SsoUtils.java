...
public class SsoUtils extends SessionUtils{
	...
	public SsoUser validateUser(HttpServletResponse response, String account, String password, Integer expired, String ip) 
			throws NoSuchAlgorithmException, AuthorizeException, IOException{
		String encodePwd="";
		try {
	...
	public SsoUser validateUserByEncodePwd(HttpServletResponse response, String account, String password, Integer expired, String ip) 
			throws AuthorizeException, IOException, NoSuchAlgorithmException {
		SsoUser ssoUser = null;
		String encodeAccount = URLEncoder.encode(account, "utf-8");
		String result = "{}";
		try {
			result = HttpUtils.getInstance().httpGet(SsoConst.API_HOST+"/validationUser.htm?a="+encodeAccount+"&p="+password+"&ip="+ip, HttpUtils.CHARSET_UTF8);
		} catch (Exception e) {
			return null;
		}
	...
	public SsoUser validateTicket(HttpServletRequest request){
		...
		String result = HttpUtils.getInstance().httpGet(SsoConst.API_HOST+"/validationTicket.htm?t="+ticket, HttpUtils.CHARSET_UTF8);
