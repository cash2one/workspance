...
public class MailUtil {
	...
	public void init(String properties) {
		...
		try {
			...
			LOG.error("An error occurred when load mail properties:"
					+ properties, e);
		}
	}
	 ...
	 * @return
	 */
	public boolean sendMail(String sendName, String senderPassword,
			String title, String receiver, String sender, Date gmtPost,
			String accountCode, String templateId,
			Map<String, Object> emailParameter, Integer priority) {
		if (priority == null) {
		...
		try {
			...
			NameValuePair[] data = {
					...
					new BasicNameValuePair("sendName", sendName),
					new BasicNameValuePair("senderPassword", senderPassword),
					new BasicNameValuePair("emailTitle", title),
					new BasicNameValuePair("templateId", templateId),
					new BasicNameValuePair("gmtPostStr", DateUtil.toString(gmtPost,
							"yyyy-MM-dd HH:mm:ss")),
					new BasicNameValuePair("accountCode", accountCode),
					new BasicNameValuePair("dataMap", js.toString()),
					new BasicNameValuePair("priority", priority.toString()) };
			// NameValuePair[] data = { new NameValuePair("email",
			// jto.toString()) };
			HttpUtils.getInstance().httpPost(API_HOST + "/email/send.htm",
	 ...
	 * @return
	 */
	public boolean sendMail(String title, String receiveMail, String sender,
			Date gmtPost, String accountCode, String templateCode,
			Map<String, Object> map, Integer priority) {
		return sendMail(null, null, title, receiveMail, sender, gmtPost,
				accountCode, templateCode, map, priority);
	}
	 ...
	 * @return
	 */
	public boolean sendMail(String username, String password, String title,
			String receiveMail, String templateCode, Map<String, Object> map,
			Integer priority) {
		return sendMail(username, password, title, receiveMail, null, null,
				null, templateCode, map, priority);
	}
	...
	public boolean sendMail(String title, String receiveMail,
			String accountCode, String templateCode, Map<String, Object> map,
			Integer priority) {
		return sendMail(null, null, title, receiveMail, null, null,
				accountCode, templateCode, map, priority);
	}
...
//		MailUtil.API_HOST = "http://apps1.zz91.com/zz91-mail";
		MailUtil.API_HOST = "http://web.zz91.com:8077/zzmail";
		// Date date = null;
		// try {
		// date = DateUtil.getDate("2011-12-13 11:30:00",
		// "yyyy-mm-dd HH:mm:ss");
		...
		// MailUtil.getInstance().sendMail("邮件测试from 163", "mays@zz91.net",
		// null, null, "test", "test", map, null);
		// MailUtil.getInstance().sendMail("邮件测试from 163-maillist",
		// "analysis@asto.mail", null, null, "test", "test_maillist", null,
		// null);
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("content", "123123321");
//		MailUtil.getInstance().sendMail("test", "kxzjzjzzkx@gmail.com",
//				"zz91-repwd", map, MailUtil.PRIORITY_HEIGHT);
		MailUtil.getInstance().sendMail("油价快报自动抓取报错","294427236@qq.com", null,null, "zz91", "blank",map, MailUtil.PRIORITY_DEFAULT);

		// map.put("username", "testaccount1");
		// map.put("repwdurl", "http://www.zz91.com/?account=testaccount1");
		// MailUtil.getInstance().sendMail("邮件测试from zz91", "kongsj@zz91.net",
		// null, date, "zz91", "test", map, null);
	}
}
