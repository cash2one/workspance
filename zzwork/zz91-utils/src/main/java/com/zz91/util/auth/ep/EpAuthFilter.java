 ...
 */
public class EpAuthFilter implements Filter{
...
//			}
			...
			EpAuthUser epAuthUser = EpAuthUtils.getInstance().getEpAuthUser(request, null);
			String tickkey = HttpUtils.getInstance().getCookie(request,EpAuthConst.TICKET_KEY, EpAuthConst.EP_DOMAIN);
			if(epAuthUser == null || !tickkey.equals(epAuthUser.getTicket())){
				epAuthUser = EpAuthUtils.getInstance().validateTicket(request);
				if(epAuthUser!=null){
					EpAuthUtils.getInstance().setEpAuthUser(request, epAuthUser, null);
				}
			}
			request.setAttribute("epAuthUser", epAuthUser);
			...
			if(!filterByAuth(epAuthUser.getRightList(), request)){
				break;
			}
