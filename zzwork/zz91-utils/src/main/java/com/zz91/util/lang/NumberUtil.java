...
public class NumberUtil {
	 ...
	 *            对于小数，有几个＃或0，就保留几位的小数； 例如： "###.00" -->表示输出的数值保留两位小数，不足两位的
	 *            补0，多于两位的四舍五入 "###.0#" -->表示输出的数值可以保留一位或两位小数；
	 *            整数显示为有一位小数，一位或两位小数 的按原样显示，多于两位的四舍五入； "###" --->表示为整数，小数部分四舍五入
	 *            ".###" -->12.234显示为.234 "#,###.0#" -->表示整数每隔3位加一个"，";
	 * @param l
	 *            Locale
	 * @return String
	 */
	...
	/**
	 * desc：使用默认方式显示货币： 例如:￥12,345.46 默认保留2位小数，四舍五入
	 * 
	 * @param d
	 *            double
	 * @return String
	...
	/**
	 * service的参数size 判空 方法
	 * @param i 传入size的值
	 * @param max size的上限
	 * @param min size的下限
	 * @return
	 * 例子：	getServiceSize(50,10,3) return 10
	 * 			getServiceSize(null,10,3) return 3
	 * 			getServiceSize(5,10,3) return 5
	 */
	public static Integer getServiceSize(Integer i,Integer max,Integer min){
		if(i==null){
			i=min;
		}
		if(i>max){
	 ...
	 * 判断是否为重复数字
	 * @param i
	 * @return true：是重复数字 false：不是重复数字
	 */
	public static Boolean validateSameNumber(Integer i){
		if(i==null){
			return false;
		}
	...
	public static void main(String[] args) {
		String s = formatCurrency(9876543210.89343, "￥###,###.00");
		String s2 = formatCurrency(1234567890.89643, "#####.00");
