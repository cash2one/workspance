 ...
 * 2012-6-19
 */
public class Kl91SsoFilter implements Filter{
	...
	public void doFilter(ServletRequest rq, ServletResponse rp,
		...
		Kl91SsoUser ssoUser = Kl91SsoUtils.getInstance().getSessionUser(request, null);
		String tickkey=HttpUtils.getInstance().getCookie(request, Kl91SsoConst.TICKET_KEY, Kl91SsoConst.SSO_DOMAIN);
		if(ssoUser == null || !tickkey.equals(ssoUser.getTicket())){
			ssoUser = Kl91SsoUtils.getInstance().validateTicket(request);
			if(ssoUser!=null){
				Kl91SsoUtils.getInstance().setSessionUser(request, ssoUser, null);
			}
		}
		request.setAttribute("sessionUser", ssoUser);
		do {
...
//			if(!filterByAuth(ssoUser.getRightArr(), request)){
//				url=deniedURL;
//				break;
//			}
