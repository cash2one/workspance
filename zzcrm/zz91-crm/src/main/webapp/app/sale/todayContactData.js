...
com.zz91.ep.crm.DataGrid = Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		...
		var _store = new Ext.data.JsonStore({
				...
				url:Context.ROOT + "/sale/data/queryDataByToday.htm",
				autoLoad:true
		});
		...
		var _cm=new Ext.grid.ColumnModel( [_sm,{
				...
				renderer:function(value, metaData, record, rowIndex, colIndex, store){
					var account=record.get("sale_account");
					var url = "<a href='"+Context.ROOT + "/sale/mycompany/todayCrmLog.htm?account="+account+"&tdate="+Ext.util.Format.date(new Date(), 'Y-m-d')+"' target='_blank'>"+value+"</a>";
					return url ;
				}
			},{
				header : "有效联系率(%)",
