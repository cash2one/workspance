...
com.zz91.ep.crm.LIKEFIELD=[
	...
	{name:"sale_name",mapping:"saleName"},
	{name:"repeat_id",mapping:"repeatId"}
];
...
com.zz91.ep.crm.FIELD=[
	...
	{name:"matchDegree",mapping:"matchDegree"}
];

com.zz91.ep.crm.Form=Ext.extend(Ext.form.FormPanel,{
...
//				frame:true,
				...
				buttons:[{
					...
					handler:function(btn){
						if (Ext.get("ds").dom.value==1){
							...
							com.zz91.ep.crm.updateAutoBlack(Ext.get("logCid").dom.value);
						}
					}
				},{
					id:"set-weight",
					...
					iconCls:"fav16",
					handler:function(btn){
						com.zz91.ep.crm.updateCompanyType(Ext.get("saleCompId").dom.value,1);
					}
				},{
					id:"on-save",
					iconCls:"saveas16",
					...
					scope:this,
					handler:function(){
						var url=Context.ROOT+"/sale/mycompany/updateCrmCompany.htm";
		...
		com.zz91.ep.crm.Form.superclass.constructor.call(this,c);
	},
	loadCompany:function(companyId){
		...
		form.store = new Ext.data.JsonStore({
			...
			url : Context.ROOT+"/sale/mycompany/queryCompDetails.htm?id="+companyId, 
			autoLoad : true,
			listeners : {
...
com.zz91.ep.crm.updateCompanyType = function(saleCompId,companyType){
	Ext.Ajax.request({
...
com.zz91.ep.crm.updateAutoBlack = function(cid){
	Ext.Ajax.request({
...
com.zz91.ep.crm.phoneNoteForm = Ext.extend(Ext.form.FormPanel,{
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		var c={
			...
			items:[{
				...
				items:[{
					...
					columns:6,
					items:[
					       {boxLabel:"新签中环通",name:"contactType",anchor:"100",inputValue:1,checked:true},
					       {boxLabel:"SEO销售",name:"contactType",anchor:"100",inputValue:2},
					       {boxLabel:"增值/续签",name:"contactType",anchor:"100",inputValue:3},
					...
					if (this.getForm().isValid()) {
						this.getForm().submit({
							...
							success : function(_form,_action){
								...
								Ext.getCmp(MYCOMP.NOTE_GRID).getStore().reload();
							},
							failure : function(_form,_action){
							}
						});
		...
		com.zz91.ep.crm.phoneNoteForm.superclass.constructor.call(this,c);
	},
	saveUrl:Context.ROOT+ "/sale/mycompany/createCrmLog.htm",
	initCidAndStar:function(cid){
		...
		var _render=[{name:"star",mapping:"star"},
		...
		form.store = new Ext.data.JsonStore({
			fields :_render,
			url : Context.ROOT+"/sale/mycompany/queryStarById.htm?id="+cid, 
			autoLoad : true,
			listeners : {
				"datachanged" : function(s) {
					var record = s.getAt(0);
					if (record == null) {
						...
						form.getForm().loadRecord(record);
						Ext.getCmp("starOld").setValue(record.get("star"));
...
com.zz91.ep.crm.phoneNoteGrid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		config=config||{};
		Ext.apply(this,config);
		
		var _store = new Ext.data.JsonStore({
			fields: com.zz91.ep.crm.NOTEFIELD,
			url:Context.ROOT + "/sale/mycompany/queryCrmLog.htm",
			autoLoad:false
		});
		...
		com.zz91.ep.crm.phoneNoteGrid.superclass.constructor.call(this,c);
	},
...
com.zz91.ep.crm.addContactWin = function(cid){
	
	var form=new com.zz91.ep.crm.contactForm({
		id:MYCOMP.CONTACT_FORM,
		saveUrl:Context.ROOT+ "/sale/mycompany/createContact.htm",
		region:"center",
	});
...
com.zz91.ep.crm.contactForm = Ext.extend(Ext.form.FormPanel,{
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		com.zz91.ep.crm.contactForm.superclass.constructor.call(this,c);
	},
	saveUrl:"",
...
com.zz91.ep.crm.Grid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		...
		var _store = new Ext.data.JsonStore({
			root : "records",
			fields:com.zz91.ep.crm.CONTACTFIELD,
			url:Context.ROOT + "/sale/mycompany/queryContact.htm",
			autoLoad:false
		});
		...
		var c={
			...
			tbar:[{
				...
				iconCls : 'add16',
				handler : function(btn){
					com.zz91.ep.crm.addContactWin(Ext.get("logCid").dom.value);
				}
			}]
		};
...
com.zz91.ep.crm.likeCompanyGrid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		config=config||{};
		Ext.apply(this,config);
		...
		var _store = new Ext.data.JsonStore({
			root : "records",
			fields:com.zz91.ep.crm.LIKEFIELD,
			url:Context.ROOT + "/sale/mycompany/queryLikeCompany.htm",
			autoLoad:false
		});
		...
		var c={
			...
			tbar:[{
				...
				iconCls : 'add16',
				handler : function(btn){
					grid.applyMerger(_store);
				}
			}]
		};
		
		com.zz91.ep.crm.likeCompanyGrid.superclass.constructor.call(this,c);
	},
	applyMerger:function(store){
		var rows=this.getSelectionModel().getSelections();
		if (Ext.get("visbile").dom.value==5){
			return ;
		};
		if(rows.length==0){
			com.zz91.utils.Msg("提示","请至少选择一条记录!");
			return ;
		};
		Ext.MessageBox.confirm(Context.MSG_TITLE,"确定提交合并申请?",function(btn){
			if(btn!="yes"){
				return ;
			}
			//注释代码用于批量操作
			...
			for(var j=0;j<rows.length;j++){
				var _id=rows[j].get("id");
				if(rows[j].get("repeat_id")>0){
					com.zz91.utils.Msg("提示","所选记录包含已合并的公司,操作无效!");
					return ;
					break;
				}else{
					_ids.push(_id);
			...
			Ext.Ajax.request({
				url:Context.ROOT+"/sale/mycompany/insertCrmRepeat.htm",
				params:{
					"id":Ext.get("showCid").dom.value,
					"cids":_ids.join(",")
					},
				success:function(response,opt){
					var obj = Ext.decode(response.responseText);
