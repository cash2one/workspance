...
//	}
     ...
     */
    @RequestMapping
    public ModelAndView createCredit(Map<String, Object> out, HttpServletRequest request, String picture, String fileName) {
    	EpAuthUser cachedUser = getCachedUser(request);
     ...
     */
    @RequestMapping
    public ModelAndView updateCompany(Map<String, Object> out,HttpServletRequest request,Integer info){
	...
	public ModelAndView doUpdateAccount(Map<String, Object> out,
		...
		do {
			Integer count = compAccountService.updateCompAccount(account);
			if (count != null && count > 0) {
				return new ModelAndView("redirect:updateComp.htm?info=3");
			}
		} while (false);
		myEsiteService.init(out, getCachedUser(request).getCid());
		return new ModelAndView("redirect:updateComp.htm?info=4");
	}
	 ...
	 */
    @RequestMapping
    public ModelAndView doUpdatePwd(Map<String, Object> out,HttpServletRequest request,String password, 
    	...
    	do {
			...
			Integer count = compAccountService.updateAccountPwd(
					getUid(request), password, newPassword);
			if (count != null && count > 0) {
				return new ModelAndView("redirect:updatePwd.htm?info=1");
			}
		} while (false);
    	myEsiteService.init(out, getCachedUser(request).getCid());
		return new ModelAndView("redirect:updatePwd.htm?info=2");
    }
     ...
     */
    @RequestMapping
    public ModelAndView updatePwd(Map<String, Object> out,HttpServletRequest request,Integer info){
    	String msg=null;
    	if(info!=null && info.intValue()==1){
    		msg="友情提示:密码修改成功!";
    	}else if(info!=null && info.intValue()==2){
    		msg="友情提示:发生错误,密码未修改!";
    	}
    	out.put("msg", msg);
    	myEsiteService.init(out, getCachedUser(request).getCid());
    ...
    @RequestMapping
    public ModelAndView updateMainProducts(Map<String, Object> out,HttpServletRequest request,Integer info){
    	EpAuthUser cachedUser = getCachedUser(request);
    	CompProfile compProfile = compProfileService.getCompProfileById(cachedUser.getCid());
    	String msg=null;
    	if(info!=null && info.intValue()==1){
    ...
    @RequestMapping
    public ModelAndView doUpdateMainProducts(Map<String, Object> out,HttpServletRequest request,String mainProductSupply,String mainProductBuy){
    	EpAuthUser cachedUser = getCachedUser(request);
    	Integer i=compProfileService.updateMainProduct(cachedUser.getCid(),mainProductSupply,mainProductBuy);
    	if (i!=null && i.intValue()>0){
    		return new ModelAndView("redirect:updateMainProducts.htm?info=1");
    	}
    	myEsiteService.init(out, getCachedUser(request).getCid());
    ...
    @RequestMapping
    public ModelAndView updateLg(Map<String, Object> out,HttpServletRequest request,Integer info){
    	EpAuthUser cachedUser = getCachedUser(request);
    	String msg=null;
		if (info!=null && info==1){
			msg="友情提示:公司Logo上传成功!";
		}else if(info!=null && info==2){
			msg="友情提示:发生错误,Logo上传失败!";
		}else if (info!=null && info==3){
			msg="友情提示:公司Logo删除成功!";
		}else if (info!=null && info==4){
			msg="友情提示:发生错误,公司Logo删除失败!";
		}
    	out.put("msg", msg);
    	myEsiteService.init(out, getCachedUser(request).getCid());
    	List<Photo> logoImage = photoService.queryPhotoByTargetType(PhotoService.TARGET_LOGO, cachedUser.getCid());
     ...
     */
    @RequestMapping
    public ModelAndView updateWebStyle(Map<String, Object> out,HttpServletRequest request,Integer info){
    	EpAuthUser cachedUser = getCachedUser(request);
    	String cssStyle = compProfileService.queryCssStyleByCid(cachedUser.getCid());
    	out.put("cssStyle", cssStyle);
    	String msg=null;
    	if (info !=null && info==0){
    		msg = "友情提示:发生错误:模板更新失败!";
    	}
    	if (info!=null && info==1){
    		msg = "友情提示:模板更新成功!";
    	}
     ...
     */
    @RequestMapping
    public ModelAndView doUpdateWebStyle(Map<String, Object> out, HttpServletRequest request,String type) {
    	EpAuthUser cachedUser = getCachedUser(request);
    	...
    	myEsiteService.init(out, getCachedUser(request).getCid());
    	return new ModelAndView("redirect:updateWebStyle.htm?info="+info);
    }
     ...
     */
    @RequestMapping
    public ModelAndView uploadCompCredit(Map<String, Object> out,HttpServletRequest request,Integer info){
    	String msg=null;
    	if (info!=null && info==0){
     ...
     */
    @RequestMapping
    public ModelAndView allCredit(Map<String, Object> out, HttpServletRequest request,Integer info) {
    	String msg=null;
    	if (info!=null && info==0){
    		msg="友情提示:发生错误,证书名称修改失败!";
    	}
    	if (info!=null && info==1){
    		msg="友情提示:证书名称修改成功!";
    	}
    ...
    @RequestMapping
    public ModelAndView updateCompCredit(Map<String, Object> out,HttpServletRequest request,String fileName,Integer id){
    	EpAuthUser cachedUser = getCachedUser(request);
    	 ExtResult result = creditFileService.updateCreditFileName(id,cachedUser.getCid(),fileName);
    	 myEsiteService.init(out, getCachedUser(request).getCid());
         return printJson(result, out);
    ...
    @RequestMapping
    public ModelAndView doCompCreditDelete(Map<String, Object> out,HttpServletRequest request,Integer id,String path){
    	EpAuthUser cachedUser = getCachedUser(request);
    	ExtResult result = creditFileService.deleteCreditById(id, cachedUser.getCid(), path);
    	myEsiteService.init(out, getCachedUser(request).getCid());
        return printJson(result, out);
