Ext.namespace("com.zz91.trade.bwDetails");

com.zz91.trade.bwDetails.FORMFIELD=[
	{name:"id",mapping:"id"},
...
com.zz91.trade.bwDetails.BWForm = Ext.extend(Ext.form.FormPanel,{
	constructor:function(config){
		...
		com.zz91.trade.bwDetails.BWForm.superclass.constructor.call(this,c);
	},
	saveUrl:Context.ROOT+"/trade/tradesupply/updateKeyWord.htm",
	saveForm:function(){
	...
	loadRecord:function(id,type){
		var form = this;
		var store= new Ext.data.JsonStore({
			fields:com.zz91.trade.bwDetails.FORMFIELD,
			url:Context.ROOT+"/trade/tradesupply/queryDetails.htm",
			baseParams:{"id":id},
			autoLoad:true,
			listeners:{
				"datachanged":function(s){
					...
					if(record!=null){
						...
						form.findById("startStr").setValue(Ext.util.Format.date(new Date(record.get("start").time), 'Y-m-d H:i:s'));
						form.findById("endStr").setValue(Ext.util.Format.date(new Date(record.get("end").time), 'Y-m-d H:i:s'));
					}
				}
			}
		});
