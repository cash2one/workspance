...
com.zz91.ep.category.TreePanel = Ext.extend(Ext.tree.TreePanel,{
	...
	constructor:function(config){
		...
		var treeLoad = new Ext.tree.TreeLoader({
			url:Context.ROOT + "/news/newscategory/child.htm",
			listeners:{
			...
			handler:function(btn){
				if(btn.parentMenu.contextNode.attributes.data>0){
					...
					Ext.MessageBox.confirm("系统提示","您确定要删除该分类及其子分类吗?", function(btn){
						...
						Ext.Ajax.request({
							url:Context.ROOT + "/news/newscategory/deleteCategory.htm",
...
com.zz91.ep.category.EditForm=Ext.extend(Ext.form.FormPanel,{
	saveUrl:Context.ROOT+"/news/newscategory/createCategory.htm",
	constructor:function(config){
		config=config||{};
		Ext.apply(this,config);
	...
	loadRecord:function(code){
		...
		var store=new Ext.data.JsonStore({
			fields : _fields,
			url : Context.ROOT + "/news/newscategory/queryCategoryByCode.htm",
...
com.zz91.ep.category.EditWin = function(code){
	...
	form.saveUrl = Context.ROOT + "/news/newscategory/updateCategory.htm";
...
com.zz91.ep.category.AddWin = function(code,text){
	...
	form.saveUrl = Context.ROOT + "/news/newscategory/createCategory.htm";
