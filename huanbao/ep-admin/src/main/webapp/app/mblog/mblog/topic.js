...
com.zz91.ep.mblog.Field=[
	...
	{name:"title",mapping:"mBlog.title"},
...
com.zz91.ep.mblog.Grid=Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		...
		var _store = new Ext.data.JsonStore({
			...
			url:Context.ROOT + "/mblog/mblog/queryAllTopic.htm",
			autoLoad:true
		});
		...
		var _cm=new Ext.grid.ColumnModel([_sm,{
				...
				renderer:function(value,metadata,record,rowIndex,colndex,store){
					...
					var returnWvalue=value;
					if(isDelete=='1'){
						returnWvalue=value+"(已删除)";
					}
					return returnWvalue;
				}
			},{
	...
	mytoolbar:[{
		...
		menu:[{
			...
			handler:function(btn){
				...
				com.zz91.ep.mblog.queryTopic(MBLOG.GRID,{"isDelete":1})
			}
		},{
			text:"全部",
			handler:function(btn){
				Ext.getCmp("filter-config").setText("全部");
				com.zz91.ep.mblog.queryTopic(MBLOG.GRID);
			}
		}]
	},"->","话题:",{
		xtype:"textfield",
...
com.zz91.ep.mblog.queryTopic = function(gridid,param){
	var _store = Ext.getCmp(MBLOG.GRID).getStore();
...
com.zz91.ep.mblog.CommentListGrid=Ext.extend(Ext.grid.GridPanel,{
	...
	constructor:function(config){
		...
		var _store=new Ext.data.JsonStore({
			...
			autoLoad:false,
			remoteSort:false,
			fields:com.zz91.ep.mblog.TopicField,
		});
		...
		var _cm=new Ext.grid.ColumnModel([_sm,{
				...
				header:"参与讨论人", width:200, dataIndex:"name",sortable:true
			},{
				header:"发布时间", width:200, dataIndex:"gmtCreated",sortable:true,
				renderer : function(value, metadata, record, rowIndex,colIndex, store) {
					if(value!=null){
						return Ext.util.Format.date(new Date(value.time), 'Y-m-d H:i:s');
					}
	...
	listUrl:Context.ROOT+"/mblog/mblog/queryTopicByTitle.htm",
	load:function(g){
		var cgrid=this;
		var row = g.getSelectionModel().getSelected();
		var title=row.get("title");
		cgrid.getStore().reload({params:{"title":title}});
	},
});
