...
com.zz91.ep.mblog.Field=[
	...
	{name:"sex",mapping:"mBlogInfo.sex"},
...
com.zz91.ep.mblog.grid = Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		...
		var _store = new Ext.data.JsonStore({
				...
				url:Context.ROOT +  "/mblog/info/queryAllMblogInfo.htm",
				autoLoad:true
		});
	...
	unCompCode:function(code,store){
		...
		for(var i=0;i<rows.length;i++){
			...
			Ext.Ajax.request({
				url:Context.ROOT+"/mblog/info/updateCompCodeAndInfoCode.htm",
	...
	okwincompcode:function(code,store){
		var rows=this.getSelectionModel().getSelections();
		for(var i=0;i<rows.length;i++){
			Ext.Ajax.request({
				url:Context.ROOT+"/mblog/info/updateInfoDeleteStatus.htm",
				params:{"infoId":rows[i].get("id"),"codeBlock":code,"id":rows[i].get("cid")},
				success:function(response,opt){ 
					var obj = Ext.decode(response.responseText);
					if(obj.success){
						com.zz91.utils.Msg("","解除冻结成功");
						  store.reload();
					}else{
						com.zz91.utils.Msg("","解除冻结失败");
					}
