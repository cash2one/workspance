Ext.ns("com.zz91.ep.subnetCategory");
...
Ext.define("com.zz91.ep.subnetCategory.MainGrid", {
	...
	initComponent:function(){
		...
		var _store=Ext.create("Ext.data.Store",{
			...
			proxy:{
				type:"ajax",
				url:Context.ROOT+"/trade/subnetcategory/queryCategoryByParentId.htm?parentId=0",
				simpleSortMode:true,
				reader: {
		            type: 'json'
		        },
		...
		var c={
			...
			dockedItems:[{
				...
				items:[{
					...
					com.zz91.ep.subnetCategory.addWin(null);
					}
				},{
					xtype:"button",
					iconCls:"edit16",
					...
					scope:this,
					handler:function(btn,e){
					com.zz91.ep.subnetCategory.editWin(grid.getSelectionModel().getLastSelected());
					}
				},{
					xtype:"button",
					iconCls:"delete16",
					...
					scope:this,
					handler:function(btn,e){
						this.deleteModel(grid.getSelectionModel().getSelection());
					}
				}]
			}]
		};
	...
	deleteModel:function(selections){
		...
		Ext.Msg.confirm(Context.MSG_TITLE,"您确定要删除选中记录？",function(o){
			...
			Ext.Array.each(selections, function(obj, idx, countriesItSelf){
				Ext.Ajax.request({
					url: Context.ROOT+"/trade/subnetcategory/deleteCategory.htm",
					params: {
						id: obj.get("id"),
					},
...
Ext.define("com.zz91.ep.subnetCategory.Form",{
			...
			buttons:[{
				...
				handler:function(){
					...
					if (form.getForm().findField("id").getValue()>0){
						url=Context.ROOT+"/trade/subnetcategory/updateSubnetCategory.htm";
					}else{
						url=Context.ROOT+"/trade/subnetcategory/addSubnetCategory.htm";
					}
...
com.zz91.ep.subnetCategory.addWin=function(record){
	var form=Ext.create("com.zz91.ep.subnetCategory.Form",{
		region:"center"
	});
...
com.zz91.ep.subnetCategory.editWin=function(record){
	var form=Ext.create("com.zz91.ep.subnetCategory.Form",{
		region:"center"
	});
...
Ext.define("com.zz91.ep.subnetCategory.ChildGrid", {
	extend:"Ext.grid.Panel",
	initComponent:function(){
		...
		var _store=Ext.create("Ext.data.Store",{
			...
			proxy:{
				type:"ajax",
				url:Context.ROOT+"/trade/subnetcategory/queryCategoryByParentId.htm",
				simpleSortMode:true,
				reader: {
		            type: 'json'
		        },
			...
			dockedItems:[{
				...
				items:[{
					...
					handler:function(btn,e){
						...
						if (select.length>0){
							com.zz91.ep.subnetCategory.addWin(select[0]);
						}
					}
				},{
					xtype:"button",
					...
					handler:function(btn,e){
						...
						if (select.length>0){
						com.zz91.ep.subnetCategory.editWin(this.getSelectionModel().getLastSelected());
						}
					}
				},{
					xtype:"button",
	...
	deleteModel:function(selections){
		...
		Ext.Msg.confirm(Context.MSG_TITLE,"您确定要删除选中记录？",function(o){
			...
			Ext.Array.each(selections, function(obj, idx, countriesItSelf){
				Ext.Ajax.request({
					url: Context.ROOT+"/trade/subnetcategory/deleteChildCategory.htm",
					params: {
						id: obj.get("id"),
...
Ext.define("com.zz91.ep.subnetCategory.ChildGrid2", {
	extend:"Ext.grid.Panel",
	initComponent:function(){
		...
		var c={
			...
			dockedItems:[{
				...
				items:[{
					...
					scope:this,
					handler:function(btn,e){
						var select=Ext.getCmp("childGrid2").getSelectionModel().getSelection();
						if (select.length>0){
						com.zz91.ep.subnetCategory.editWin(this.getSelectionModel().getLastSelected());
						}
					}
