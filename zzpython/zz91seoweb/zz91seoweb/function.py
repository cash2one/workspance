...
def getcompinfo(pdtid,cursort,keywords):
	...
	if (list1==None):
		...
		if productlist:
			...
			,'pdt_detaillclear':subString(filter_tags(productlist[5]),150),'pdt_time_en':productlist[6],'com_subname':productlist[7],'vipflag':arrviptype
...
def getcompanyproductslist(kname="",frompageCount="",limitNum="",company_id="",pdt_type="",seriesid='',fcflag='',haveprice="",maxcount=20000,groupbyflag=""):
	if not seriesid:
		if fcflag:
			...
			sql="select id from products where company_id=%s and is_del=0  and is_pause=0 and check_status=1 limit %s,%s"
...
def getcompanydetail(company_id):
	...
	#listdir=None
	if not listdir:
		sqlc="select c.name,c.business,c.regtime,c.address,c.introduction,c.membership_code,e.label,c.area_code,c.address_zip,c.zst_year,c.sale_details,c.buy_details,c.website,c.industry_code,c.tags,c.domain_zz91 from company as c  LEFT OUTER JOIN category as e ON c.area_code=e.code where c.id=%s"
		cursor.execute(sqlc,company_id)
		clist=cursor.fetchone()
		if clist:
			compname=clist[0]
			...
			logo_pic='http://img0.zz91.com/front/images/esite/logo.gif'
			sqlc="select contact,tel_country_code,tel_area_code,tel,mobile,fax_country_code,fax_area_code,fax,email"
			sqlc=sqlc+",sex,position,qq,is_use_back_email,back_email,account "
			...
			if alist:
				...
				if alist:
					...
					tel=tel[0:10]
					if not photo_cover:
						midpic="http://img0.zz91.com/zz91/ppc/images/banner.jpg"
					else:
		...
		if plist:
			...
			if productspic:
				...
				for p in productspic:
					...
					picurl={'im':str(im),'images':pdt_images,'images_big':pdt_images_big}
					im=im+1
...
def getcompanynewslist(frompageCount,limitNum,company_id):
	...
	if alist:
		for list1 in alist:
			title=list1[0]
			id=list1[3]
			content=filter_tags(list1[1])[0:100]
