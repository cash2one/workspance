...
def default(request, company_id):
	...
	if list:
		#不是会员
		if list.get("viptype").get("vipcheck")==None:
			html = render(request, '404.html', locals())
			return HttpResponseNotFound(html)
		#做百度优化，但是不是会员
		if list.get("baidu") and list.get("viptype").get("vipcheck")==None:
			html = render(request, '404.html', locals())
			return HttpResponseNotFound(html)
	# 非来电宝用户跳转到公司首页
	if not list:
	...
	prolist = getcompanyproductslist(frompageCount=0, limitNum=12, company_id=company_id)
...
def tuijian(request, pinyin='', page='', pdt_type='',keyword=''):
	...
	if company_id:
		...
		if viptype=='10051003':
			isldb=1
			ldbblance=getldblaveall(company_id,cursor)
			qianbaoblance=ldbblance
		else:
			qianbaoblance=getqianbaoblance2(company_id,cursor)
