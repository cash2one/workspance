...
#             print a_code
            #获得当前登录用户的顶级菜单
            sql='select id,label from menu where parent_id=0 order by ord desc'
            ...
            for top in topresult:
                ...
                if str(tid) in a_code:
                    #print 'ok'
                    tlabel=top['label']
                    sql1='select id,parent_id,label from menu where parent_id=%s order by ord desc'
                    ...
                    for fir in firresult:
                        ...
                        if str(fid) in a_code:
                            ...
                            flabel=fir['label']
                            sql2='select id,label,link from menu where parent_id=%s order by ord desc'
                            ...
                            for sec in secresult:
                                ...
                                if str(sid) in a_code:
                                    ...
                                    slist={'slabel':slabel,'link':link,'sec_id':sec_id}
                                    seclistall.append(slist)
                            flist={'fid':fid,'flabel':flabel,'seclistall':seclistall}
    ...
    def get_user_categorylist(self):
        ...
        for first in firstlist:
            ...
            secondlist=db.fetchalldb(sql2,[code1])
            if secondlist:
                listall2=[]
                for second in secondlist:
                    ...
                    thirdlist=db.fetchalldb(sql2,[code2])
                    if thirdlist:
                        listall3=[]
...
#             level=[{'':},{},{}]
            return n
    #用户列表
    def get_user(self,frompageCount,limitNum,username=''):
        sqlarg=''
        argument=[]
        if username:
            sqlarg+=' and u.username like %s'
            argument.append('%'+username+'%')
        sqlc="select count(0) as count from user as u where id>0 "+sqlarg
        result=db.fetchonedb(sqlc,argument)
        listcount=result['count']
        sql="select u.id,u.username,u.password,u.user_category_code,u.auth_category_id,u.closeflag,c.label as user_category,a.label as auth_category,u.realname from user as u left join user_category as c on u.user_category_code=c.code  left join auth_category as a on u.auth_category_id=a.id where c.id>0"+sqlarg
    ...
    def getusercategory_code4(self):
        sql="select code,label from user_category where code like '____'"
        listall=db.fetchalldb(sql)
        return listall
    #一键删除所有用户
