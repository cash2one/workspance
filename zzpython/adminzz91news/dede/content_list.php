...
if(empty($totalresult) && empty($keyword) && empty($orderby) && empty($flag))
{
    ...
    if(!empty($channelid) && empty($cid) && empty($fid))
    {
	...
	if(!empty($fid))
    {
        $tinyQuerys[] = " typeid2 in(".GetSonIds($fid).") ";
    }
...
if($fid != 0)
{
    $whereSql .= ' AND arc.typeid2 IN ('.GetSonIds($fid).')';
}
if($arcrank != '')
{
...
$query = "SELECT arc.id,arc.typeid,arc.typeid2,arc.senddate,arc.flag,arc.ismake,
