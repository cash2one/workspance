...
<asp:Content ID="Content1" ContentPlaceHolderID="cphAdmin" runat="Server">
    ...
    <div class="settings">
        ...
        <asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ControlToValidate="txtName" ErrorMessage="必填" /><br />
        ...
        <asp:RequiredFieldValidator ID="RequiredFieldValidator3" runat="server" ControlToValidate="txtPostsPerPage" ErrorMessage="必填" />
        ...
        <%--//by Spoony--%>
        <label for="<%=cbEnableShowPagination.ClientID %>">启用页码分页</label>        
        <asp:CheckBox runat="server" ID="cbEnableShowPagination" /><br />
        
        <label for="<%=ddlTheme.ClientID %>"><%=Resources.labels.theme %></label>
        <asp:DropDownList runat="server" ID="ddlTheme" />
        <a href="javascript:void(PreviewTheme());">效果预览</a> | 
        <a href="http://www.blogyi.net/" target="_blank">更多主题...</a><br />
        ...
        <asp:DropDownList runat="Server" ID="ddlCulture" Style="text-transform: capitalize">
            <asp:ListItem Text="自动选择" Value="Auto" />
            <asp:ListItem Text="英文" Value="en" />
        </asp:DropDownList><br />
        
        <label for="<%=txtTimeZone.ClientID %>"><%=Resources.labels.timezone %></label>
        <asp:TextBox runat="Server" ID="txtTimeZone" Width="30" /> 服务器时间: <%=DateTime.Now.ToShortTimeString() %>
    ...
    <div class="settings">
        ...
        <asp:CompareValidator ID="CompareValidator2" runat="Server" ControlToValidate="txtSmtpServerPort" Operator="datatypecheck" Type="integer" ErrorMessage="无效数" /><br />
        ...
        <asp:Button runat="server" CausesValidation="False" ID="btnTestSmtp" Text="测试邮件配置" />
    ...
    <div class="settings">
        ...
        <label for="<%=txtTrackingScript.ClientID %>">
          访问回溯脚本<br /><br />这段 JavaScript 代码可被 Google 解析。<br /><br />
          将添加到每个页面中，而不管使用的是何种主题风格。<br /><br />(建议加上 &lt;script&gt; 标签)</label>
    ...
    <div class="settings">
      <h1>清除缓存</h1>
      <p>
        清除缓存有助于强制更新博客页面的数据信息，当您对系统数据进行操作之后却没有看到效果，则可以尝试此操作。
      </p>
      <asp:Button ID="btnClearCache" runat="server" Text="执行清除" OnClick="btnClearCache_Click" />
        <asp:Literal ID="litClearCacheList" runat="server"></asp:Literal>
    </div>
