	...
	public string AdminLinks
	{
		get
		{
			if (System.Threading.Thread.CurrentPrincipal.Identity.IsAuthenticated)
			{
				...
				sb.AppendFormat("<a href=\"javascript:void(0);\" onclick=\"if (confirm('您确定要删除该页面吗？')) location.href='?deletepage={0}'\">{1}</a>", this.Page.Id.ToString(), Resources.labels.delete);//by Spoony
				sb.AppendLine("</div>");
				return sb.ToString();
			}
