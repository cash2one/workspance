...
public partial class _default : BlogEngine.Core.Web.Controls.BlogBasePage
{
	...
	private void DisplayApmlFiltering()
	{
		...
		{
			Page.Title = "满足 APML 条件的记录";//by Spoony
			try
			{
				...
				{
                    divError.InnerHtml = "<h1 style=\"text-align:center\">APML 文档未找到</h1><br /><br />";//by Spoony
					Page.Title = "APML not found";
				}
			}
			catch (NotSupportedException)
			{
                divError.InnerHtml = "<h1 style=\"text-align:center\">该站点并不包含任何关于 APML 的信息</h1><br /><br />";//by Spoony
				Page.Title = "APML not found";
			}
			catch (System.Net.WebException)
			{
                divError.InnerHtml = "<h1 style=\"text-align:center\">抱歉，尚未能连接到您的站点</h1><br /><br />";//by Spoony
				Page.Title = "APML address invalid";
			}
			catch (XmlException)
			{
                divError.InnerHtml = "<h1 style=\"text-align:center\">APML 文档并不是有效的 XML 格式</h1><br /><br />";//by Spoony
				Page.Title = "APML document error";
			}
		}
		else if (PostList1.Posts == null || PostList1.Posts.Count == 0)
		{
            divError.InnerHtml = "<h1 style=\"text-align:center\">该 URL 无法解析</h1><br /><br />";//by Spoony
            Page.Title = "APML 未找到";//by Spoony
		}
	}
