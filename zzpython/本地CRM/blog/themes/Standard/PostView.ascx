...
<div class="post xfolkentry">
  ...
  <div class="text"><asp:PlaceHolder ID="BodyContent" runat="server" /><div style="clear:both;"></div></div>
  <div class="bottom">
    <%=Rating %>
  ...
  <div class="footer">    
    ...
    <%=AdminLinks %>
    <a rel="bookmark" href="<%=Post.PermaLink %>">сю╬ца╢╫с</a> |
    <a rel="nofollow" href="<%=Post.RelativeLink %>#comment"><%=Resources.labels.comments %> (<%=Post.ApprovedComments.Count %>)</a> |
    <a rel="nofollow" href="<%=CommentFeed %>">╤╘тдндуб<img src="<%=VirtualPathUtility.ToAbsolute("~/pics/")%>rssButton.gif" alt="RSS comment feed" style="margin-left:3px" /></a>    
  </div>
</div>