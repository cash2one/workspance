...
using rcu_en_news.IDAL;
using Maticsoft.DBUtility;
namespace rcu_en_news.SQLServerDAL
{
	/// <summary>
	/// 用参数方式实现数据层示例。
	/// </summary>
    public class SysManage1 : ISysManage//这里必须实现接口，否则工厂类创建接口报错
	{
        ...
        public int AddTreeNode(rcu_en_news.Model.SysNode model)
		{
        ...
        public void UpdateNode(rcu_en_news.Model.SysNode model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// <returns></returns>
        public rcu_en_news.Model.SysNode GetNode(int NodeID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from S_Tree ");	
			strSql.Append(" where NodeID=@NodeID");
            ...
            rcu_en_news.Model.SysNode node = new rcu_en_news.Model.SysNode();
