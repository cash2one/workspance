...
Class clsPageRs2
  ...
  Public Function pageRs
	...
	GBL_EXEString="if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[#temp]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)"&chr(13)
	GBL_EXEString=GBL_EXEString&"drop table [dbo].[#temp]"&chr(13)
	GBL_EXEString=GBL_EXEString&"CREATE TABLE [dbo].[#temp] ("&chr(13)
	GBL_EXEString=GBL_EXEString&"[TempID] [bigint] IDENTITY (1, 1) NOT NULL ,"&chr(13)
	GBL_EXEString=GBL_EXEString&"["&keyFld&"] [bigint] NULL, "&chr(13)
	GBL_EXEString=GBL_EXEString&"[com_email] [varchar] (50) COLLATE Chinese_PRC_CI_AS NULL "&chr(13)
	GBL_EXEString=GBL_EXEString&") ON [PRIMARY]"&chr(13)
	conn.ExeCute GBL_EXEString
    set cmd=server.createobject("adodb.command") 
	cmd.activeconnection=conn 
	cmd.commandtext = "zz91_Allpage_test" 
	...
	'sql="select "&sltFld&" from "&fFromTbl&" "&fSqlWhere&fSqlOrder
	'response.Write(sql)
	'set pageRs=server.CreateObject("adodb.recordset")
	'pageRs.open sql,conn,1,1
    'rn=50
  ...
  Sub pageNav(urlPrefix,speTipStr)
    ...
    <table width="100%" border=0 cellpadding=0 cellspacing=0 style="margin:5px 0px 3px 0px;">
     <tr align="center">    
     <%If fCurPage>1 Then %>
      <td><a href="<%=pageUrl%>&page=1">首页</a></td>
      <td><a href="<%=pageUrl%>&page=<%=fCurPage-1%>">上一页</a></td>
     <%
     Else
     ...
     Response.Write fCurPage&"/"& fTotalPage&" 页"
     response.Write "&nbsp;&nbsp;"& fPageSize&"/页"
     %>
     </td> 
     <%If fCurPage-fTotalPage<0 Then %>
     <td> <a href="<%=pageUrl%>&page=<%=fCurPage+1%>">下一页</a></td>
     <td> <a href="<%=pageUrl%>&page=<%=fTotalPage%>">尾页</a></td>
     <%
     Else 
     ...
     <td align="center">
     &nbsp;到&nbsp;
     <input type=text size=4 onKeyUp="value=value.replace(/[^\d]/,'')" name=windowpage maxlength=5>&nbsp;页&nbsp;
