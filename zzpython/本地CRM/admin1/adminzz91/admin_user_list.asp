...
page=1 
 ...
 sqldel="delete from users where id in ("& selectcb &")"
	conn.Execute(sqldel)
	response.Redirect("admin_user_list.asp?"&sear&"&page="&page&"&userid="&request("userid"))
 end if
%>
<html>
<head>
      ...
      <tr>
        <td bgcolor="E7EBDE"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <table width="95%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
			...
			sqlu="select * from cate_adminuser order by id desc"
			...
			%>
              <td align="center" class="<%=buclass%>"><a href="?userid=<%=rsu("code")%>"><%=rsu("meno")%></a></td>
			<%
			rsu.movenext
			loop
			end if
            ...
            <td valign="top" bgcolor="#F1F3ED"><table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
              <form action="admin_user_list.asp" method="post" name=form1>
                ...
                <tr bgcolor="<%=bgcol%>" >
				  ...
				  <input name="Submit4" type="button" class="button01-out" value="开通定时" onClick="window.location='admin_adminuser_closepart.asp?id=<%=rs("id")%>&flag=1&userid=<%=userid%>'">
				  <%else%>
				  <input name="Submit4" type="button" class="button01-out" value="关闭定时" style=" background:#FF0000;color:#ffffff;" onClick="window.location='admin_adminuser_closepart.asp?id=<%=rs("id")%>&flag=0&userid=<%=userid%>'">
				  <%end if%>
				  <%if rs("closeflag")=1 then%>
				  <input name="Submit4" type="button" class="button01-out" value="开通帐号" onClick="window.location='admin_adminuser_close.asp?id=<%=rs("id")%>&flag=0&userid=<%=userid%>'">
				  <%else%>
				  <input name="Submit4" type="button" class="button01-out" value="关闭帐号" style=" background:#FF0000;color:#ffffff;" onClick="window.location='admin_adminuser_close.asp?id=<%=rs("id")%>&flag=1&userid=<%=userid%>'">
				  <%end if%>
				  
                    <input name="Submit3" type="button" class="button01-out" value="权限设置" onClick="window.location='admin_user_class_qx.asp?uname=<%=rs("id")%>&userid=<%=userid%>'">
                    <input name="Submit" type="button" class="button01-out" value="修改" onClick="window.location='admin_adminuser_mod.asp?id=<%=rs("id")%>&userid=<%=userid%>'">				    </td>
                </tr>
                <%
...
-->
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td align="right"><input type="hidden" name="dostay">
                            ...
                            <input name="Submit2" type="button" class="button01-out" onClick="delitem(this.form)" value="删除">                        </td>
                      </tr>
                    </table>                  </td>
                </tr>
              </form>
              ...
              <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td><!-- #include file="../include/page.asp" -->                  </td>
                </tr>
              </table></td>
          </tr>
        </table>
