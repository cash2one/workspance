...
'response.Write(request.servervariables("zz91.vicp.net"))
posttype=Request.ServerVariables("REQUEST_METHOD")
server_v1=Cstr(Request.ServerVariables("HTTP_REFERER"))
server_v2=Cstr(Request.ServerVariables("SERVER_NAME"))
'response.Write(server_v1&"<br>"&posttype)
...
if session("personid")="" or isnull(session("personid")) then
	...
	response.Write("<script>parent.window.location='loginofrc.asp';</script>")
	%>
...
lmmu=lmmarray(0)
sql="select myqx from user_qx where uname='"&session("personid")&"'"
...
sqlw="select id from cate_qx where link = '"&lmu&"' order by id asc"
set rsw=server.CreateObject("adodb.recordset")
rsw.open sqlw,conn,1,1
if not rsw.eof or not rsw.bof then
	codebb=rsw("id")
	Response.Cookies("lmcode")=rsw("id")
	session("lmcode")=rsw("id")
    bbb=0
	if qx<>"" then
	mqx=split(qx,",",-1,1)
