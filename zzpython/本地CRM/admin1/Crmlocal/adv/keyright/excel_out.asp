...
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  ...
  If trim(request("keywords"))<>"" Then
	sqls=sqls&"and keywords like '%"&trim(request("keywords"))&"%'"
	sear=sear&"&keywords="&trim(request("keywords"))
end if
...
if request("ADStopDate_to")<>"" then
  ...
  sql="select * from adv_keyofferlist_right where "&sqls&" order by id desc "
...
'导出excel文件
  ...
  Response.AddHeader "Content-disposition","attachment;filename=搜索关键字右侧广告.xls"
%>
