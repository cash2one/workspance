		...
		while not rsc.eof
			...
			if comtel1<>"" then
				if len(comtel1)>=6 then
					sqlteltemp=sqlteltemp&" callee like '%"&comtel1&"%' or"
				end if
			end if
			if comtel2<>"" then
				if len(comtel2)>=8 then
					sqlteltemp=sqlteltemp&" callee like '%"&comtel2&"%' or"
				end if
			end if
			if comtel3<>"" then
				if len(comtel3)>=6 then
					sqlteltemp=sqlteltemp&" callee like '%"&comtel3&"%' or"
				end if
			end if
			if comtel4<>"" then
				if len(comtel4)>=8 then
					sqlteltemp=sqlteltemp&" callee like '%"&comtel4&"%' or"
				end if
			end if
			if sqlteltemp<>"" then
			...
			if notel=0 then
				RecordNo=""
				sqlt="select recId,caller,incomeFlg,beginTime,endTime,fileName,callee from T_RecordList where ("&sqlteltemp&")"
				if mytel<>"" then
				...
				if not rst.eof or not rst.bof then
					while not rst.eof
						RecordNo=rst("recId")
						CallerId=rst("caller")
						CallType=rst("incomeFlg")
						startTime=rst("beginTime")
						endTime=rst("endTime")
						'recordTime=rst("recordTime")
						recordTime=""
						filePath=rst("fileName")
						Dtmf=rst("callee")
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	if request("Dtmf")<>"" then
		sql=sql&" and callee like '%"&request("Dtmf")&"%'"
	...
	if request("fromsc")<>"" then
		sql=sql&" and DATEDIFF(second,beginTime,endtime)>='"&request("fromsc")*60&"'"
	...
	With oPage
		.sltFld  = "*"
		.FROMTbl = "T_RecordList"
		.sqlOrder= "order by recId desc"
...
while not rs.eof %>
  <tr>
    <td bgcolor="#FFFFFF"><%=rs("recId")%></td>
    <td bgcolor="#FFFFFF"><%=rs("caller")%></td>
    
    <td bgcolor="#FFFFFF"><%=left(rs("callee"),50)&"..."%></td>
    <td bgcolor="#FFFFFF">
	<%
	if rs("incomeFlg")="0" then response.Write("呼出")
	if rs("incomeFlg")="1" then response.Write("打入")
	%></td>
    <td bgcolor="#FFFFFF"><%=rs("beginTime")%></td>
    <td bgcolor="#FFFFFF"><%=rs("endTime")%></td>
    <td bgcolor="#FFFFFF"><%=DateAdd("s",DATEDIFF("s", rs("beginTime"), rs("endtime")),0)%></td>
    <td bgcolor="#FFFFFF"><a href="lyshow.asp?lyUrl=<%=rs("fileName")%>" target="_blank">录音</a></td>
    <td bgcolor="#FFFFFF">
    <%
	lyUrl=replace(rs("fileName"),"E:\","")
	%>
