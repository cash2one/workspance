    ...
    <input type="submit" name="Submit" value="搜索">
		...
		<table width="95%"  border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#7386A5">
              ...
              <tr bgcolor="#FFFFFF">
				...
				set rsstar=nothing
				%>
				<td rowspan="2" align="center">明日<br>
			    安排联系</td>
			  ...
			  if session("userid")="10" then
			    sqlu="select * from users where closeflag=1 and userid=22 "
			  else
			  	sqlu="select * from users where closeflag=1 and userid=22 and id="&session("personid")
			  end if
			...
			if cstr(fromdate)<>cstr(todate) then
			    ...
			    if fromdate<>"" and todate<>"" then
					sql=sql&" and teldate>='"&fromdate&"' and teldate<='"&cdate(fromdate)+1&"'"
				end if
			end if
			sqlmy="select count(com_id) from v_salescomp where EXISTS(select com_id from comp_tel where personid="&rsu("id")&" and com_id=v_salescomp.com_id "&sql&" and telflag=1) and com_webrank='"&rsstar("code")&"' and webcontacttype='"&rscate("code")&"' "&sql&""
			...
			<%
				...
				sqlm="select count(0) from temp_salescomp where com_id in (select com_id from comp_websales where contactnext_time>='"&date()+1&"' and contactnext_time<='"&date()+2&"' and com_id in (select com_id from crm_AssignWeb where personid="&rsu("id")&"))"
				'response.Write(sqlm)
          ...
          <tr bgcolor="#FFFFFF">
			...
			<%
				...
				if todate<>"" then
					...
					if fromdate<>"" and todate<>"" then
						sqlcou=sqlcou&" and teldate>='"&fromdate&"' and teldate<='"&cdate(fromdate)+1&"'"
					end if
				end if
				sqlcoucount="select count(com_id) from temp_salescomp where contacttype='"&rscate("code")&"' and com_rank='"&rsstar("code")&"' and EXISTS(select com_id from comp_tel where personid="&session("personid")&" and com_id=temp_salescomp.com_id "&sqlcou&" and telflag=0) "
		...
		<br>
		<br>
		<a href="admin_tel_comp2.asp?adminuser=<%=session("personid")%>&contacttype=&from_date=<%=fromdate%>&to_date=<%=todate%>&com_rank=">查看今天的所有电话记录</a><br></td>
      </tr>
    </table></td>
  </tr>
  <tr>
