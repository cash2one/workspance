...
<%
	...
	sqlTemp=sqlTemp&" and not EXISTS(select null from crm_OtherComp where com_id=v_salescomp.com_id)"
	sqlTemp=sqlTemp&" and not EXISTS(select null from crm_InsertCompWeb where com_id=v_salescomp.com_id and saletype=2)"
	sqlTemp=sqlTemp&" and not EXISTS(select null from crm_webPipeiComp where com_id=v_salescomp.com_id )"
	sqlTemp=sqlTemp&" and not EXISTS(select null from Crm_Active_CompAll where com_id=v_salescomp.com_id and ActivePublic=0 and not exists (select null from Crm_Active_ActiveComp where com_id=Crm_Active_CompAll.com_id))"
