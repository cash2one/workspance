  ...
  <tr>
    <td height="100%" valign="top"><table width="98%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <%end if%>   
		  <table width="100%" border="0" cellspacing="0" cellpadding="0">
		    <form name="form2" method="post" action="admin_user_list_lt.asp"><tr>
		      <td align="center">
		        姓名
...
群
<input name="chat_department" type="text" id="chat_department" size="10">
              ...
              <form action="admin_user_list.asp" method="post" name=form1>
                <%
					...
					if request.QueryString("userid")<>"" then
						sql=sql&" and chat_userid="&userid&""
					end if
					if request("chat_department")<>"" then
						sql=sql&" and chat_department like '%"&request("chat_department")&"%'"
						sear=sear&"&chat_department="&request("chat_department")
					end if
					if request("button")="" then
						if request("closeflag")="0" then
							sql=sql&" and chatclose=1"
							sear=sear&"&closeflag="&request("closeflag")
						end if
					end if
					if request("closeflag")="1" then
						sql=sql&" and chatclose=0"
						sear=sear&"&closeflag="&request("closeflag")
					end if
					...
					if session("userid")="10" then
						...
						sql=sql&" and  chat_userid in ("&ywadminid&")"
					end if
					'response.Write(sql)
                ...
                <tr bgcolor="#ffffff" >
                  ...
                  <td align="right" nowrap><%=rs("usertel")%><%if session("userid")="10" then%><a href="?personid=<%=rs("id")%>&urtel=1">删除</a><%end if%></td>
                  <td align="right" nowrap>
                  <%response.Write(rs("recordflag"))%>
                    <%if rs("chatclose")="1" then%>
                    <a href="admin_adminuser_closechat.asp?id=<%=rs("id")%>&flag=0&userid=<%=userid%>&closeflag=<%=request("closeflag")%>">聊天开通</a>
                    <%else%>
                    <a href="admin_adminuser_closechat.asp?id=<%=rs("id")%>&flag=1&userid=<%=userid%>&closeflag=<%=request("closeflag")%>"><font color="#FF0000">聊天关闭</font></a>
                    <%end if%>
