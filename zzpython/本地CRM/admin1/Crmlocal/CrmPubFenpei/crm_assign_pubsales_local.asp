...
'if request("search")<>"" then
	sql1=""
	sql1=sql1&" and agent=0 and com_id not in (select com_id from temp_salescomp where vipflag=2 and vip_check=1) and com_id not in (select com_id from crm_assign where Cushion<>-1) and com_id not in (select com_id from comp_sales where com_type='13') and (adminuser=0 or logincount>0) and com_id not in (select com_id from comp_sales where com_Especial='1')  and com_id not in (select com_id from Crm_PublicComp)"
	sqlmain="select count(com_id) from temp_salescomp where 1=1 "&sql1
	response.Write(sql1)
	set rsmain=conn.execute(sqlmain)
	compcount=rsmain(0)
	response.Write(compcount)
	rsmain.close
	set rsmain=nothing
'end if
if request("user")<>"" then
 ...
 i=0
	...
	sql1=sql1&" and agent=0 and com_id not in (select com_id from temp_salescomp where vipflag=2 and vip_check=1) and com_id not in (select com_id from crm_assign where Cushion<>-1) and com_id not in (select com_id from comp_sales where com_type='13') and (adminuser=0 or logincount>0) and com_id not in (select com_id from comp_sales where com_Especial='1')  and com_id not in (select com_id from Crm_PublicComp)"
				   ...
				   sqlcom=""
				   sqlcom="select com_id from temp_salescomp where 1=1 "&sql1&" order by vipflag DESC, viptype DESC, adminuser DESC, com_id DESC"
				   ...
				   next
				   'conn.execute("EXEC zz91_salescomp ''")
