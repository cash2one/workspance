...
sqld="delete from temp_telid where telid"
    sqlc="select top 100 id,com_id,personid from comp_tel where TelDate>'"&date&"' and not exists(select telid from temp_telid where comp_tel.id=telid) order by id desc"
...
'				sqlteltemp=sqlteltemp&" called like '%"&comtel1&"%' or"
'			end if
'		end if
'		if comtel2<>"" then
'			if len(comtel2)>=8 then
'				sqlteltemp=sqlteltemp&" called like '%"&comtel2&"%' or"
'			end if
'		end if
'		if comtel3<>"" then
'			if len(comtel3)>=6 then
'				sqlteltemp=sqlteltemp&" called like '%"&comtel3&"%' or"
'			end if
'		end if
'		if comtel4<>"" then
'			if len(comtel4)>=8 then
'				sqlteltemp=sqlteltemp&" called like '%"&comtel4&"%' or"
'			end if
'		end if
		'if sqlteltemp<>"" then
...
'		end if
		...
		if sqlteltemp<>"" then
			...
			response.Write(notel&rsc("id")&"<br>")
			'-------读取录音电话
	...
	if application("ppcount")="" then application("ppcount")=1
	application("ppcount")=application("ppcount")+1
	response.Write(application("ppcount"))
%>
