  ...
  <tr>
    <td>
					...
					if dotype="my" and docach<>"-1" then
						...
						sql=sql&" and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and Cushion<>-1 "
						else
					...
					if dotype="today" then
						if session("userid")="10" then
							...
							sql=sql&" and contactnext_time>'"&date()&"' and contactnext_time<'"&date()+1&"' and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) "
						else
							sql=sql&" and personid="&session("personid")&" and contactnext_time>'"&date()&"' and contactnext_time<'"&date()+1&"' and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id)"
						end if
						if request("subdotype")="continue" then
							sql=sql&" and vip_dateto<'"&date()+90&"' and vip_dateto>='"&date()&"'"
							sear=sear&"&subdotype="&request("subdotype")
					...
					sql=sql&" and personid="&session("personid")&" and EXISTS(select com_id from Crm_Assign where Emphases=1 and com_id=v_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************我的重点客户/end
					'******************我的再生通客户/start
					...
					if dotype="myyescontact" then
						sql=sql&" and personid="&session("personid")&"  and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and EXISTS (select com_id from v_groupcomid where  com_id=v_salescomp.com_id and EXISTS (select id from comp_tel where personid="&session("personid")&" and id=v_groupcomid.telid))"
						sear=sear&"&dotype="&dotype
						if request("subdotype")="continue" then
							sql=sql&" and vip_dateto<'"&date()+90&"' and vip_dateto>='"&date()&"'"
							sear=sear&"&subdotype="&request("subdotype")
					...
					if session("userid")="10" or session("personid")="32" then
							...
							sql=sql&" and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and not EXISTS (select com_id from v_groupcomid where  com_id=v_salescomp.com_id)"
					else
							sql=sql&" and personid="&session("personid")&"  and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and not EXISTS (select com_id from v_groupcomid where  com_id=v_salescomp.com_id)"
					end if
					if request("subdotype")="continue" then
						sql=sql&" and vip_dateto<'"&date()+90&"' and vip_dateto>='"&date()&"'"
						sear=sear&"&subdotype="&request("subdotype")
					...
					'sql=sql&" and not EXISTS(select com_id from c1 where com_id>44025 and com_id<44233 and com_id=v_salescomp.com_id)"
					'sear=sear&"&dotype="&dotype
					end if
				   ...
				   With oPage
				     .sltFld  = "com_tel,com_mobile,com_email,com_province,com_id,com_name,com_pass,vip_date,vip_datefrom,vipflag,viptype,vip_check,contactnext_time,com_rank,com_regtime,viewcount,logincount,lastlogintime,teldate,vip_dateto"
					 .FROMTbl = "v_salescomp"
				     .sqlOrder= sqlorder
        ...
        <td><table width="100%" class=ee id=ListTable border="0" cellpadding="2" cellspacing="0">
              <form action="crm_allcomp_list.asp" method="post" name=form1>
                <tr class="topline"> 
                  ...
                  类型</td>
                  <td nowrap bgcolor="#DFEBFF">确认单</td>
...
等级</td>
                ...
                <tr style="cursor:hand" bgcolor="<%=cbg%>"> 
				  ...
				  %>                  </td>
                  <td nowrap><iframe src="http://www.zz91.com/admin1/compinfo/hasConfirm.asp?com_id=<%= rs("com_id") %>" frameborder='0' width="100%" height="100%" scrolling="no"></iframe></td>
...
function CheckAll(form)
  ...
  for (var i=0;i<form.elements.length;i++)
	...
	else
	{
	  if (confirm("开通这些公司吗?"))
	  {
	  window.open ("http://www.zz91.com/admin1/compinfo/cominfo_openConfirm.asp?selectcb="+selectcb.substr(2),"_a","width=500,height=500")
