...
<table width="100%" id=secTable border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	%>
    <%
	sqlaa="select code,meno from crm_category where code like '1005____' and closeflag=0 and type="&atype&""
	set rsaa=conn.execute(sqlaa)
	if not rsaa.eof or not rsaa.bof then
	while not rsaa.eof 
	%>
    <td nowrap class="<%=topselectbutton(doaction,"zhongdiankh"&rsaa("code"))%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&lmaction=<%response.Write(lmaction)%>&cach=<%response.Write(docach)%>&action=zhongdiankh<%=rsaa("code")%>&lmcode=<%=lmcode%>&typecode=<%=rsaa("code")%>'"><%=rsaa("meno")%></td>
	<%
	rsaa.movenext
	wend
	end if
    ...
    <td nowrap class="<%=topselectbutton(doaction,"ldb")%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&lmaction=<%response.Write(lmaction)%>&cach=<%response.Write(docach)%>&action=ldb&lmcode=<%=lmcode%>'">来电宝客户</td>
    ...
    <td align="left" nowrap class="unselect" >
	 <a href="http://adminasto.zz91.com/companyadd/?addtype=zst&personid=<%=session("personid")%>" target="_blank">手动录入</a></td>
    <td align="left" nowrap class="unselect">
	 <a href="/admin1/huangye2016/add.asp" target="_blank">2016黄页客户录入</a></td>
      ...
      <td height="30" align="left" bgcolor="#FFFFFF">
        ...
        <tr>
          ...
          <td>
            ...
            <input name="typecode" type="hidden" id="typecode" value="<%=request.QueryString("typecode")%>">
...
<font id="mainprovince1"></font><font id="maincity1"></font><font id="mainGarden">无园区</font>
    ...
    <tr>
      ...
      <select name="companytype" id="companytype" style="width:120px;">
        ...
        <option value="gonghainocs">公海客户（无服务者）</option>
        ...
        <option value="gq90120">90-120过期客户</option>
        <option value="gq120180">120-180过期客户</option>
        <option value="ghhuangjin">黄金客户</option>
      </select>
...
<%if session("Partadmin")<>"0" and (dotype="allall" or dotype="MyContinueBm" or dotype="allbm" or dotype="bmzd" or dotype="all" or dotype="bmzd" or dotype="3daynodo" or (dotype="my" and session("userid")="10") or (dotype="today" and session("userid")="10") or (dotype="contact" and session("userid")="10") or (lmaction="outcontact" and session("userid")="1315") or (dotype="nocontact" and session("userid")="10") or (dotype="MyContinue" and session("userid")="10") or (dotype="myzst" and session("userid")="10") or (dotype="xm") or session("personid")="93" or session("personid")="227" or session("personid")="449" or dotype="vappaycomp") or session("userid")="10" or session("userid")="13" or lmaction="allbm"  then%>
...
'***********搜索/start
					...
					'---------公海客户（无服务者）
					if request("companytype")="gonghainocs" then
						sql=sql&" and not EXISTS(select company_id from crm_cs where company_id=v_salescomp.com_id)"
						sear=sear&"&companytype="&request("companytype")
					end if
					'---------过期90天客户
					...
					if request("companytype")="gq90120" then
						sql=sql&" and (vip_dateto<='"&date-90&"' and vip_dateto>'"&date-120&"') "
						sear=sear&"&companytype="&request("companytype")
					end if
					if request("companytype")="gq120180" then
						sql=sql&" and (vip_dateto<='"&date-120&"' and vip_dateto>'"&date-180&"') "
						sear=sear&"&companytype="&request("companytype")
					end if
					if request("companytype")="ghhuangjin" then
						sql=sql&" and vip_dateto<='"&date()+10*30&"' and vip_dateto>='"&date()&"'"
						sear=sear&"&companytype="&request("companytype")
					end if
  ...
  <tr>
    <td>
      <%
					...
					'-----------------塑交会客户
					if dotype="sujiaohui" then
						sql=sql&" and exists(select com_id from comp_regfrom1 where com_id=v_salescomp.com_id)"
						sear=sear&"&dotype="&dotype
					end if
					'******************所有未联系的客户/start
					...
					if dotype="gonghaiassign" then
						if session("userid")="10" then
							...
							sql=sql&" and EXISTS(select com_id from crm_assign_gonghai where com_id=v_salescomp.com_id and outflag=0) "
						else
							sql=sql&" and EXISTS(select com_id from crm_assign_gonghai where com_id=v_salescomp.com_id and outflag=0) "
							sql=sql&" and personid="&session("personid")&" "
						end if
					...
					if dotype="allbm" then
						if session("userid")="13" then 
							...
							sql=sql&" and exists(select id from users where userid in ("&ywadminid&") and id=v_salescomp.personid and closeflag=1)"
						end if
					...
					if dotype="myzst" then
						...
						sql=sql&" and EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id and closeflag=0)"
						...
						if dotype<>"TodayContinue" and dotype<>"MyContinueBm" and dotype<>"ContactContinue" and dotype<>"dropTosea" and dotype<>"today" and doaction<>"allzst" and dotype<>"contact" and dotype<>"allall" and dotype<>"allcontinue" and dotype<>"allExpired" and request("subdotype")<>"continue" and dotype<>"myShare" and dotype<>"MyContinue" and dotype<>"NoContinue" and dotype<>"ZoContinue" and dotype<>"allZST" and dotype<>"fromService" and dotype<>"allzstGh" and dotype<>"AllContinue" and dotype<>"AutoTogonghai" and dotype<>"大客户列表" And Left(dotype,3)<>"vap" and dotype<>"CustomIn" then
							sql=sql&" and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id and closeflag=0)"
						end if
					end if
					'-------大客户列表列表
...
'						sear=sear&"&zhuce="&request("zhuce")
					...
					'-----------重点客户准备  2012-2-10
					if left(doaction,11)="zhongdiankh" then
						typecode=request.QueryString("typecode")
						sql=sql&" and exists(select property_id from crm_category_info where v_salescomp.com_id=property_id and property_value='"&typecode&"')"
						sear=sear&"&action="&doaction&"&typecode="&typecode
					end if
					'-----------我的品牌通  2012-2-1
					...
					'******************一星客户/end
					'******************来电宝客户/start
					if doaction="ldb" then
						sql=sql&" and EXISTS(select company_id from company_service where company_id=v_salescomp.com_id and crm_service_code in ('1007','1008','1009','1010')  and online=1)"
						sear=sear&"&action="&doaction
					end if
					'******************一星客户/end
					...
					if dotype="allZST" then
					    sql=sql&" and exists(select com_id from comp_zstinfo where com_id=v_salescomp.com_id and com_check=1 and closeflag=0"
...
'						end if
					...
					if dotype="allzstGh" then
						...
						if subType<>"" then
							if subType="allCurrentZstGh" then
								sql=sql&"  and not EXISTS(select com_id from crm_assign where com_id=v_salescomp.com_id) and not EXISTS(select com_id from crm_continue_assign where com_id=v_salescomp.com_id) and exists(select com_id from comp_zstinfo where com_check=1 and com_id=v_salescomp.com_id and closeflag=0)"
								sear=sear&"&subType="&subType
							
							'未过期再生通的客户 /end
							'公海过期再生通的客户 /star
							elseif subType="allExpireZstGh" then
								sql=sql&"  and not EXISTS(select com_id from crm_assign where com_id=v_salescomp.com_id) and not EXISTS(select com_id from crm_continue_assign where com_id=v_salescomp.com_id)  and exists(select com_id from comp_zstinfo where com_check=0 and com_id=v_salescomp.com_id and closeflag=0)" '临时将时间限定为已过期30天内
								sear=sear&"&subType="&subType
							end if
						else
							sql=sql&"  and not EXISTS(select com_id from crm_assign where com_id=v_salescomp.com_id) and exists(select com_id from comp_zstinfo where com_id=v_salescomp.com_id and closeflag=0)"
						end if
						sear=sear&"&dotype="&dotype
					end if
					'公海再生通的客户 /end
					...
					if dotype="noassign" then
						sql=sql&" and not EXISTS(select com_id from Crm_Assign where com_id=v_salescomp.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id and closeflag=0) and not EXISTS(select com_id from crm_publiccomp where com_id=v_salescomp.com_id) "
						sear=sear&"&dotype="&dotype
					end if
					if dotype="tnoassign" then
						sql=sql&" and not EXISTS(select com_id from Crm_Assign where com_id=v_salescomp.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id and closeflag=0) and com_regtime>'"&date()-1&"' and com_regtime<'"&date()&"'"
						sear=sear&"&dotype="&dotype
					end if
					...
					'------放待开通未分配客户
					if dotype="vapnoassign" then
						sql=sql&" and exists(select com_id from crm_openConfirm where v_salescomp.com_id=com_id and  assignflag=0) "
						sear=sear&"&dotype="&dotype
					end if
					if dotype="vapcomp" or dotype="vappaycomp" or dotype="vapallcomp" then
						...
						if lmaction<>"kaitongdan" and lmaction<>"seo" and lmaction<>"nocontact" and lmaction<>"ldb" and lmaction<>"ldball" then
						...
						if lmaction="zstguoqi" then
							sql=sql&" and exists(select com_id from comp_zstinfo where v_salescomp.com_id=com_id and com_check=0 and closeflag=0 and vip_dateto<'"&date()&"')"
						end if
						'-------------------
						'---------①今天安排联系的客户
						...
						if lmaction="nocontact" then
							...
							sql=sql&" and vpersonid<>(select top 1 personid from comp_tel where com_id=v_salescomp.com_id and telflag=4 order by teldate desc) and teldate>'1900-1-1'"
							'sql=sql&" and teldate>'1900-1-1' and not EXISTS(select com_id from comp_tel where   com_id=v_salescomp.com_id"
						...
						if lmaction="outcontact" then
							...
							'sear=sear&"&doperson="&request("doperson")
								...
								sql=sql&" and vpersonid=(select top 1 personid from comp_tel where personid=v_salescomp.vpersonid and com_id=v_salescomp.com_id and contacttype='13' order by teldate desc)"
								'--------------begin
								'不包含公海挑入的客户
								'sql=sql&" and Not EXISTS(select com_id from comp_tel where com_id=v_salescomp.com_id"							
								if request("doperson")<>"" then
						...
						'******************部门所有客户/end
						if request("companytype")="bisha" or request("companytype")="gq90180" or  request("companytype")="gq90120" or  request("companytype")="gq120180" then
							if lmaction="gonghai" then
							else
								if session("userid")="13" or session("userid")="10" then
								else
						...
						if lmaction="bmzd" then
							...
							sql=sql&")"
							sear=sear&"&action="&request("action")
						end if
						if lmactoin="zhongdian" then
							sql=sql&" and EXISTS(select com_id from Crm_Assignvap where Emphases=1 and com_id=v_salescomp.com_id)"
						end if 
						' 自定义客户类型
						if lmaction="zdykehu" then
							if session("userid")="13" or session("userid")="10" then
								if request("doperson")<>"" and not isnull(request("doperson")) then
									'sql=sql&" and vpersonid="&request("doperson")&""
								else
						...
						if lmaction="seo" then
							...
							sql=sql&" and EXISTS(select company_id from company_service where company_id=v_salescomp.com_id and crm_service_code='10001002' and apply_status='1' and online=1)"
						end if
						'----已合作来电宝客户
						if lmaction="ldb" then
							if session("userid")="13" or session("userid")="10" then
								if request("doperson")<>"" and not isnull(request("doperson")) then
									'sql=sql&" and vpersonid="&request("doperson")&""
								else
						...
						if lmaction="nomember" then
							...
							'sql=sql&" and EXISTS(select company_id from company_service where company_id=v_salescomp.com_id and crm_service_code in (1002,1003,1004,1005,10001000,10001002,10001004) and apply_status='1')"
							sql=sql&" and not EXISTS(select company_id from company_service where company_id=v_salescomp.com_id and crm_service_code in ('1000','1006') and apply_status='1')"
						end if
						'******************部门重点客户/end
						'---------------------------
						'*录入客户管理
				   ...
				   With oPage
					 ...
					 .FROMTbl = "v_salescomp"
				     .sqlOrder= sqlorder
				     .sqlWhere= "WHERE not EXISTS (select com_id from crm_block where com_id=v_salescomp.com_id) "&sql
				     .keyFld  = "com_id"    '不可缺少
				     .pageSize= 10
				     .getConn = conn
				     Set Rs  = .pageRs
...
if left(dotype,3)<>"vap" then
	if dotype="MyContinue" then
	sqlp="select count(0) from comp_zstinfo where com_id in (select com_id from comp_info where vip_datefrom>'"&date-35&"') and com_id in (select com_id from Crm_Assign where personid="&session("personid")&") and closeflag=0"
	set rsp=conn.execute(sqlp)
	count35=rsp(0)
	sqlp="select count(0) from v_salescomp where not exists(select com_id from Crm_Active_CompAll where com_id=v_salescomp.com_id and ActivePublic=0 and not exists (select com_id from Crm_Active_ActiveComp where com_id=Crm_Active_CompAll.com_id)) and TelDate<='"&date-25&"' and exists(select com_id from comp_zstinfo where com_id=v_salescomp.com_id and closeflag=0) and personid="&session("personid")&""
	'response.Write(sqlp)
	set rsp=conn.execute(sqlp)
	count25=rsp(0)
	rsp.close
...
所有者</td>
                ...
                <tr style="cursor:hand" bgcolor="<%=cbg%>"> 
				  ...
				  set rsp=nothing
				  '-------------------------判断参加活动客户
				  sqlP="select top 1 fromname from comp_regfrom1 where com_id="&rs("com_id")&" order by id desc"
				  set rsp=conn.execute(sqlP)
				  if not rsp.eof or not rsp.bof then
...
'					set rsd=nothing
					...
					if not isnull(vip_dateto) and vip_dateto<>"" then
						...
						if cdate(vip_dateto)<=cdate(date()-90) and cdate(vip_dateto)>date()-120 then
							response.Write("<br><font color=#ff0000>(90-120过期客户)</font>")
						end if
						if cdate(vip_dateto)<=cdate(date()-120) and cdate(vip_dateto)>date()-180 then
							response.Write("<br><font color=#ff0000>(120-180过期客户)</font>")
						end if
						if cdate(vip_dateto)<=cdate(date()-180) and cdate(vip_dateto)>cdate("1900-5-5") then
					...
					set rsd=nothing
					'---------------判断来电宝客户
					sqld="select id from company_service where crm_service_code in ('1007','1008','1009','1010') and company_id="&rs("com_id")
					set rsd=conn.execute(sqld)
					if not rsd.eof or not rsd.bof then
						response.Write("<br><font color=#f80400>来电宝客户</font>")
					end if
					rsd.close
					set rsd=nothing
				end if
                ...
                <div id="laiyuan<%=rs("com_id")%>"></div>
                  ...
                  <td nowrap >
				  <a href="../crmlocal/crm_cominfoedit.asp?idprod=<%response.Write(rs("com_id"))%>&dotype=<%response.Write(dotype)%>&lmcode=<%=lmcode%>" target=_blank>
				  <%response.Write(rs("com_name"))%></a>
				...
				lastlogintime=rs("lastlogintime")
				if rs("contactnext_time")<>"1900-1-1" or rs("vcontactnext_time")<>"1900-1-1" then
					if left(dotype,3)="vap" then
				...
				%></a>
                  ...
                  <%'if dotype="大客户列表" or left(dotype,3)="vap" then%>
                  <td align="center" nowrap>
                  	<iframe src="http://adminasto.zz91.com/getCsuserName/?com_id=<%= com_id %>" scrolling="no" frameborder="0" width="100" height="30"></iframe>
                  </td>
			...
			loop  
			  ...
			  %>
                ...
                <tr bgcolor="#FFFFFF"> 
                  <td colspan="37"> 全选 
                    ...
                    <%
					if (((ywadminid<>"" and left(session("userid"),2)="13") or session("userid")="10") and dotype="CustomIn") or session("personid")="571" then
					%>
             ...
             <%if session("userid") ="10" or session("userid")="13" or ((ywadminid<>"0" and ywadminid<>"" and (not isnull(ywadminid)) and (dotype="allbm" or dotype="all" or dotype="bmzd" or dotype="contact" or dotype="nocontact" or dotype="my" or dotype="sbcomp" or dotype="MyContinue" or dotype="outgonghai" or lmaction="gonghai" or lmaction="allbm" or lmaction="mycomp" or dotype="vapallallcomp" or dotype="vappaycomp" or dotype="vapallcomp" or dotype="vapnoassign")))  then%>
              ...
              <%
				...
				<%
                    ...
                    <%if left(dotype,3)="vap" and (lmaction="seo" or lmaction="mycomp") then%>
                    <input name="Submit5" type="button" class="button" value="SEO下线" onClick="postAll(this.form,'确实要放到已下线SEO库吗?','seounline')">
                    <input name="Submit5" type="button" class="button" value="SEO上线" onClick="postAll(this.form,'确实要放到已上线SEO库吗?','seoonline')">
                    <%end if%>
					...
					<%end if%>
                    <input name="Submit5" type="button" class="button" value="加入黑名单" onClick="postAll(this.form,'确认加入黑名单吗?','compblock')">
