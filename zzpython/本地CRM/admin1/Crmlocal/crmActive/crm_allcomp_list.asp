...
page=1 
 ...
 if selectcb<>"" and dostay<>"" then
    response.Redirect("crm_assign_save.asp?selectcb="&request("selectcb")&"&dostay="&request("dostay"))&"&personid="&request("personid")
 end if
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="searchtable">
  ...
  <tr>
	  ...
	  <input class="<%if request("zhuce")="zhudong" then response.Write("pl") else response.Write("button") end if%>" type="button" name="Submit" value="主动申请客户" onClick="window.location='?dotype=<%response.Write(dotype)%>&action=<%response.Write(doaction)%>&zhuce=zhudong'">
&nbsp;&nbsp;
<input type="button" class="<%if request("zhuce")="shouji" then response.Write("pl") else response.Write("button") end if%>" name="Submit" value="收集的客户" onClick="window.location='?dotype=<%response.Write(dotype)%>&action=<%response.Write(doaction)%>&zhuce=shouji'">      </td>
  </tr>
  <%end if%>
    <tr>
...
<SCRIPT type=text/javascript>
	...
	new Dron_City("province","city",'<%=request("province")%>|<%=request("city")%>').init();
        ...
        <tr>
          ...
          <td>Email:</td>
          <td><input name="com_email" type="text" class=text id="com_email" style="background-color:#fff;" size="15"><script language=javascript>createDatePicker("regtimefrom",true,"<%=regtimefrom%>",false,true,true,true)</script></td>
          </tr>
      </table></td>
    </tr>
    ...
    <tr>
      ...
      <%if dotype="allall" or dotype="all" or dotype="3daynodo" or (dotype="my" and session("userid")="10") or (dotype="today" and session("userid")="10") or (dotype="contact" and session("userid")="10") or (dotype="nocontact" and session("userid")="10") or (dotype="myzst" and session("userid")="10") or (dotype="xm") then%>
            <select name="doperson" class="button" id="doperson" >
              <option value="" >请选择--</option>
                        <%
						sqlu="select realname,id from users where closeflag=1 and adminuserid<>'' and userid=21"
...
'***********搜索/start
					...
					sql=sql&" and EXISTS (select com_id from Comp_ComTrade where com_id=v_active_salescomp.com_id and TradeID="&request("clscb")&")"
  ...
  <tr>
    <td>
					...
					if dotype="my" and docach<>"-1" then
						...
						sql=sql&" and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and Cushion<>-1 "
						else
						sql=sql&" and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and personid="&session("personid")&" "
							if docach="" then
						       sql=sql&" and Cushion<>-1 "
							end if
						end if
						sear=sear&"&dotype="&dotype
						cach=docach
						if docach<>"" then
						'sql=sql&" and Cushion="&cach&""
						sql=sql&" and com_id not in(select top 450 com_id from v_SalesTelTime where not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_SalesTelTime.com_id) and not EXISTS(select com_id from comp_active_sales where com_Especial='1' and com_id=v_SalesTelTime.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_SalesTelTime.com_id)"
						    if session("userid")="10" and request("doperson")<>"" then
					...
					if dotype="contact" then
						if session("userid")="10" or session("personid")="32" then
							...
							sql=sql&" and contactnext_time<'"&date()&"' and contactnext_time<>'1900-1-1' and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id)"
						else
							sql=sql&" and personid='"&session("personid")&"' and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and contactnext_time<'"&date()&"' and contactnext_time<>'1900-1-1'"
						end if
					sear=sear&"&dotype="&dotype
					end if
					'******************所有安排联系的客户/end
					...
					if dotype="today" then
						if session("userid")="10" then
							...
							sql=sql&" and contactnext_time>'"&date()&"' and contactnext_time<'"&date()+1&"' and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) "
						else
							sql=sql&" and personid="&session("personid")&" and contactnext_time>'"&date()&"' and contactnext_time<'"&date()+1&"' and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id)"
						end if
					sear=sear&"&dotype="&dotype
					end if
					'******************今天安排联系的客户/end
					...
					if dotype="nocontact" then
						if session("userid")="10" then
							...
							sql=sql&" and teldate is not null and com_id=v_active_salescomp.com_id) "
							sql=sql&" and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) "
						else
							sql=sql&" and not EXISTS(select com_id from v_SalesTelTime where personid="&session("personid")&" and teldate is not null and com_id=v_active_salescomp.com_id) and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and personid="&session("personid")&" and Cushion<>-1 "
						end if
					sear=sear&"&dotype="&dotype
					end if
					'******************所有未联系的客户/end
					...
					sql=sql&" and personid="&session("personid")&" and EXISTS(select com_id from v_groupcomid where telid in (select id from comp_tel where personid<>"&session("personid")&") and com_id=v_active_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************公海挑入客户/end
					'******************三天内未处理客户/start
					...
					sql=sql&" and not EXISTS(select com_id from v_groupcomid where com_id=v_active_salescomp.com_id) and EXISTS(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id and fdate<'"&date()-2&"')"
					else
					sql=sql&" and personid="&session("personid")&" and not EXISTS(select com_id from v_groupcomid where com_id=v_active_salescomp.com_id) and EXISTS(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id and fdate<'"&date()-2&"')"
					end if
					sear=sear&"&dotype="&dotype
					end if
					'******************三天内未处理客户/end
					...
					sql=sql&" and personid="&session("personid")&" and EXISTS(select com_id from Crm_active_Assign where Emphases=1 and com_id=v_active_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************我的重点客户/end
					'******************我的再生通客户/start
					...
					if dotype<>"today" and doaction<>"allzst" and dotype<>"contact" and dotype<>"allall" then
					sql=sql&" and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_active_salescomp.com_id)"
					end if
					end if
					'******************我的再生通客户/end
					'******************公海客户/start
					if dotype="all" then
						    ...
						    'sql=sql&" and com_id not in (select com_id from crm_assign where Cushion<>-1) and com_id not in (select com_id from comp_sales where com_type='13' ) and (adminuser=0 or com_email in (select com_email from comp_login)) "
							'sql=sql&" and personid is null and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and (com_id in (select com_id from comp_logincount)) and not EXISTS(select com_id from crm_assignVIP where com_id=v_salescomp.com_id)"
							sql=sql&" and not exists(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id)  and not exists(select com_id from Crm_Assign where com_id=v_active_salescomp.com_id)  and not exists(select com_id from Crm_sleep where com_id=v_active_salescomp.com_id)  and not exists(select id from comp_logincount where com_id=v_active_salescomp.com_id and fdate>='"&date()-90&"' and fdate<'"&date()&"') and com_regtime<'"&date()-90&"'"
							sear=sear&"&dotype="&dotype
					end if
					'******************公海客户/end
					'******************所有客户/start
					if dotype="allall" then
						...
						sql=sql&" and (adminuser>0 and com_id not in (select com_id from comp_logincount))"
						sear=sear&"&zhuce="&request("zhuce")
						end if
						sear=sear&"&dotype="&dotype
					end if
					...
					'******************设备中心客户/star
				    if dotype="sbcomp" then
					sql=sql&" and EXISTS(select com_id from Sb_Com_Cls where com_id=v_active_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************设备中心客户/end
					'******************vip客户/star
				    if dotype="myvip" then
					sql=sql&" and EXISTS(select com_id from crm_assignVIP where com_id=v_active_salescomp.com_id and personid="&session("personid")&") and not exists(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************vip客户/end
					'******************特殊意向客户/start
					if dotype="Especial" then
					sql=sql&" and EXISTS(select com_id from comp_active_sales where com_Especial='1' and com_id=v_active_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					else
					'sql=sql&" and com_Especial<>'1' "
					sql=sql&" and not EXISTS(select com_id from comp_active_sales where com_Especial='1' and com_id=v_active_salescomp.com_id)"
					end if
					'******************特殊意向客户/end
					'******************新客户分配/start
					if dotype="fenpei" then
					sql=sql&" and com_regtime>='"&date()-10&"' and com_regtime<='"&date()&"' and com_id not in (select com_id from Crm_active_Assign where Cushion<>-1) and (adminuser=0 or com_id in (select com_id from comp_logincount))"
					sear=sear&"&dotype="&dotype
						if request("zhuce")="zhudong" then
					...
					sql=sql&" and personid="&session("personid")&"  and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and EXISTS (select com_id from v_groupcomid where  com_id=v_active_salescomp.com_id and EXISTS (select id from comp_tel where personid="&session("personid")&" and id=v_groupcomid.telid))"
					sear=sear&"&dotype="&dotype
					end if
					'******************所有正在联系的客户/
					'******************所有未联系的客户/
					...
					if session("userid")="10" or session("personid")="32" then
							...
							sql=sql&" and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and not EXISTS (select com_id from v_groupcomid where  com_id=v_active_salescomp.com_id)"
					else
							sql=sql&" and personid="&session("personid")&"  and not EXISTS(select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) and not EXISTS (select com_id from v_groupcomid where  com_id=v_active_salescomp.com_id)"
					end if
					...
					sql=sql&" and personid="&request("personid")&" and not exists(select personid from v_active_salescomp where v_active_salescomp.personid is null and v_active_salescomp.com_id=com_id)) and contactnext_time>'"&cdate(request("datetime"))&"' and contactnext_time<'"&cdate(request("datetime"))+1&"' and teldate<'"&cdate(request("datetime"))&"'"
					...
					'******************所有待激活的客户/
					if dotype="allactive" then
					sql=sql&" and not exists(select com_id from Crm_Assign where com_id=v_active_salescomp.com_id) and not exists(select id from comp_logincount where com_id=v_active_salescomp.com_id and fdate>='"&date()-90&"' and fdate<'"&date()&"') and com_regtime<'"&date()-90&"'"
					sear=sear&"&dotype="&dotype
					end if
					'******************所有待激活的客户/
					...
					sql=sql&" and not EXISTS(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_active_salescomp.com_id) and not EXISTS(select com_id from crm_active_publiccomp where com_id=v_active_salescomp.com_id) "
					sear=sear&"&dotype="&dotype
					end if
					if dotype="tnoassign" then
					sql=sql&" and not EXISTS(select com_id from Crm_active_Assign where com_id=v_active_salescomp.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_active_salescomp.com_id) and com_regtime>'"&date()-1&"' and com_regtime<'"&date()&"' and (adminuser=0 or com_id in (select com_id from comp_logincount))"
					sear=sear&"&dotype="&dotype
					end if
					if dotype="vipnoassign" then
					sql=sql&" and (vipflag=1 and vip_check=1) and not EXISTS (select com_id from Crm_active_Assign where Cushion<>-1 and com_id=v_active_salescomp.com_id) and not EXISTS (select com_id from crm_assignVIP where  com_id=v_active_salescomp.com_id) and not EXISTS (select com_id from comp_active_sales where com_type='13' and com_id=v_active_salescomp.com_id) "
					sear=sear&"&dotype="&dotype
					end if
					if dotype="chongtu1" then
					sql=sql&" and com_id>44025 and com_id<44233"
					sear=sear&"&dotype="&dotype
					else
					sql=sql&" and not EXISTS(select com_id from c1 where com_id>44025 and com_id<44233 and com_id=v_active_salescomp.com_id)"
					'sear=sear&"&dotype="&dotype
					end if
				...
				'	if session("userid")="10" then
				   ...
				   With oPage
				     .sltFld  = "com_tel,com_mobile,com_email,com_province,com_id,com_name,com_pass,vip_date,vip_datefrom,vipflag,viptype,vip_check,contactnext_time,com_rank,com_regtime,viewcount,logincount,lastlogintime,teldate"
					 .FROMTbl = "v_active_salescomp"
				     .sqlOrder= sqlorder
				     .sqlWhere= "WHERE not EXISTS (select com_id from Agent_ClientCompany where com_id=v_active_salescomp.com_id) and not exists(select com_id from crm_sleep where com_id=v_active_salescomp.com_id) "&sql
				     .keyFld  = "com_id"    '不可缺少
				     .pageSize= 10
				     .getConn = conn
				     Set Rs  = .pageRs
        ...
        <td><table width="100%" class=ee id=ListTable border="0" cellpadding="2" cellspacing="0">
              <form action="crm_allcomp_list.asp" method="post" name=form1>
                <tr class="topline"> 
                  ...
                  类型</td>
                  <!--<td nowrap bgcolor="#DFEBFF"><span class="style1 STYLE2">操作</span></td>-->
...
等级</td>
                ...
                <tr style="cursor:hand" bgcolor="<%=cbg%>"> 
                  <td height="24"><input name="cbb<%=i%>" type="checkbox" title="<%response.Write(rs("com_email"))%>" value="<%response.Write(rs("com_id"))%>"></td>
                  <SPAN onmouseover=DoHL();  onDblClick="javascript:DoSL();javascript:oow('../crmActive/crm_cominfoedit.asp?idprod=<%response.Write(rs("com_id"))%>&cemail=<%response.Write(rs("com_email"))%>&dotype=<%response.Write(dotype)%>');" onmouseout=DoLL();>
				  <td nowrap>
                  ...
                  <!--<td nowrap><a href="#kang" onClick="window.open('../admin_vip_change.asp?com_id=<%=rs("com_id")%>','','width=500,height=500')">操作</A></td>-->
                   
                  <td nowrap> <%response.Write(rs("com_name"))%></td>
                  <td>
                    <%
				...
				<%  
				'sqlvcount="select viewcount,logincount,lastlogintime from comp_login where com_email='"&rs("com_email")&"'"
				'set rsvcount=conn.execute(sqlvcount)
				'if not rsvcount.eof or not rsvcount.bof then
				vcount=rs("viewcount")
				...
				'else
				        'sqls="select top 1 contactnext_time,contacttype from comp_tel where com_id="&rs("com_id")&" order by id desc"
						'set rss=server.CreateObject("adodb.recordset")
						'rss.open sqls,conn,1,2
						'if not rss.eof then
						'contactnext_time=rss("contactnext_time")
						'contacttype=rs("contacttype")
						'else
						'contactnext_time=""
						'contacttype=""
						'end if
				...
				'end if
				  ...
				  <%
				  sqluser="select a.personid,b.realname,a.Waiter_Open from crm_active_assign as a,users as b where a.com_id="&rs("com_id")&" and a.personid=b.id"
...
function CheckAll(form)
  ...
  for (var i=0;i<form.elements.length;i++)
	...
	else
	{
	  if (confirm("开通这些公司吗?"))
	  {
	  window.open ("http://www.zz91.com/admin1/compinfo/cominfo_open.asp?selectcb="+selectcb.substr(2),"_a","width=500,height=500")
...
-->
                ...
                <tr bgcolor="#FFFFFF"> 
                  <td colspan="25"> 全选 
					...
					if dotype<>"all" and dotype<>"xm" and dotype<>"Especial" and dotype<>"allall"  then
					%>
                    ...
                    <%if dotype<>"my" and dotype<>"allall" then%>
					...
					<!--<input type="button" name="Submit" class="button" value="放到待删除区" onClick="selectdelcom(this.form)">-->
					<input type="button" name="Submit2" class="button" value="放到废品池" onClick="selectwastecom(this.form)">
					<%end if%>
					<%if dotype<>"allall" then%>
					<%'if dotype="myvip" then%>
					<!--<input type="button" name="Submit" class="button" value="关闭VIP试用" onClick="selectclosevip(this.form)">-->
					<%'end if%>
<!--					<input type="button" name="Submit" class="button" value="放入待开通再生通" onClick="selectopenzst(this.form)">-->
                    <input name="Submit5" type="button" class="button" value="取出公海" onClick="outselectcrm(this.form)">
					<%end if%>
<%if session("userid") ="10" then%>
<input name="Submit5" type="button" class="button" value="放入待分配" onClick="outselectcrmtt(this.form)">
<%end if%>
