...
page=1 
 ...
 if dostay="selec1tcrm" then
	...
	for i=0 to ubound(arrcom)
		...
		set rst=nothing
	next
	response.redirect "crm_allcomp_list_conflict.asp?dotype=conflict&lmcode=3926"
 end if
...
<SCRIPT type=text/javascript>
    ...
    <tr>
	  ...
	  <% =session("Partadmin") %>
      <%if session("Partadmin")<>"0" and (dotype="allall" or dotype="allbm" or dotype="bmzd" or dotype="all" or dotype="bmzd" or dotype="3daynodo" or (dotype="my" and session("userid")="10") or (dotype="today" and session("userid")="10") or (dotype="contact" and session("userid")="10") or (dotype="nocontact" and session("userid")="10") or (dotype="conflict" and session("userid")="10") or (dotype="myzst" and session("userid")="10") or (dotype="xm") or session("personid")="93" or session("personid")="227") then%>
    ...
    <td>
					...
					if dotype="conflict" then
						...
						'else
							'sql=sql&" and EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id)"
						'end if
						sql=sql&" and com_id in(select a.com_id from crm_assign a,crm_continue_assign b where a.com_id=v_salescomp.com_id and a.com_id=b.com_id and a.personid<>b.personid)"
    ...
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" class=ee id=ListTable border="0" cellpadding="2" cellspacing="0">
              <form action="crm_allcomp_list_conflict.asp" method="post" name=form1>
                <tr class="topline"> 
...
-->
                ...
                <tr bgcolor="#FFFFFF"> 
                  <td colspan="31"> 全选 
					...
					<%if session("userid") ="10" or (ywadminid<>"0" and ywadminid<>"" and (not isnull(ywadminid)) and (dotype="conflict" or dotype="all" or dotype="bmzd" or dotype="my"))  then%>
