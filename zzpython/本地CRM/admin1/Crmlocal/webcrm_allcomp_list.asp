...
page=1 
 ...
 if selectcb<>"" and dostay<>"" then
    response.Redirect("webcrm_assign_save.asp?selectcb="&request("selectcb")&"&dostay="&request("dostay"))&"&personid="&request("personid")
 end if
...
<table width="100%" id=secTable border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<%end if%>
	<%end if%>
    <td align="left" nowrap class="unselect" onClick="window.open('webcrm_comp_add.asp?dotype=<%response.Write(dotype)%>&cach=<%response.Write(docach)%>&action=sqzst','_blank','width=700,height=500')">
	 手动录入</td>
	 <td align="left" class="unselect" onClick="window.open('http://admin.zz91.com/admin1/adv/ggpos/index.html','_blank','')">
	 广告位示意图</td>
	 <!--
...
<SCRIPT type=text/javascript>
    ...
    <tr>
      ...
      <select name="compkind" id="compkind">
        <option value="">请选择...</option>
        <%
		sql="select code,meno from cate_webcompType where code like '__'"
        ...
        <%
		sql1="select code,meno from cate_webcompType where code like '"&rs("code")&"__'"
      ...
      <script>selectOption("compkind",<%=request("compkind")%>)</script>
      ...
      <%if dotype="allall" or dotype="all" or dotype="3daynodo" or (dotype="my" and session("userid")="10") or (dotype="today" and session("userid")="10") or (dotype="contact" and session("userid")="10") or (dotype="nocontact" and session("userid")="10") or (dotype="myzst" and session("userid")="10") or (dotype="xm") then%>
            <select name="doperson" class="button" id="doperson" >
              <option value="" >请选择--</option>
                        <%
						sqlu="select realname,id from users where closeflag=1 and userid=22"
...
'***********搜索/start
					...
					sql=sql&" and com_webrank='"&trim(request("com_rank"))&"'"
					...
					if request("compkind")<>"" then
						sql=sql&" and exists(select null from crm_WebcompKind where com_id=v_salescomp.com_id and com_type like '"&request("compkind")&"%')"
  ...
  <tr>
    <td>
					...
					if dotype="my" and docach<>"-1" then
						...
						sql=sql&"  and com_id in (select com_id from crm_assignWeb)"
						else
						sql=sql&" and com_id in (select com_id from crm_assignWeb where personid="&session("personid")&")"
						end if
						sear=sear&"&dotype="&dotype
						cach=docach
						
						    if request("doperson")<>"" then
								sql=sql&" and wpersonid="&request("doperson")&""
					...
					if dotype="myShare" then
						sql=sql&" and com_id in (select com_id from crm_comp_share where salesType=1) and wpersonid is null"
						sear=sear&"&dotype="&dotype
					end if
					'****************我的共享客户 /end
					'******************所有安排联系的客户/start
					if dotype="contact" then
						if session("userid")="10" or session("personid")="32" then
							...
							sql=sql&" and contactnext_time<'"&date()&"' and contactnext_time<>'1900-1-1' and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id)"
						else
							sql=sql&" and personid='"&session("personid")&"' and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and contactnext_time<'"&date()&"' and contactnext_time<>'1900-1-1'"
						end if
					...
					if dotype="today" then
						if session("userid")="10" then
						    if request("doperson")<>"" then
								sql=sql&" and wpersonid="&request("doperson")&" and Cushion<>-1"
								sear=sear&"&doperson="&request("doperson")
						    end if
							sql=sql&" and webcontactnext_time>'"&date()&"' and webcontactnext_time<'"&date()+1&"' and (wpersonid is not null or wpersonid<>'')"
						else
							sql=sql&" and wpersonid="&session("personid")&" and webcontactnext_time>'"&date()&"' and webcontactnext_time<'"&date()+1&"'"
						end if
						if request("subdotype")="continue" then
					...
					if dotype="nocontact" then
						if session("userid")="10" then
							...
							sql=sql&" and not EXISTS(select com_id from v_SalesTelTime where personid="&session("personid")&" and teldate is not null and com_id=v_salescomp.com_id) and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and personid="&session("personid")&" and Cushion<>-1 "
						end if
					sear=sear&"&dotype="&dotype
					end if
					'******************所有未联系的客户/end
					...
					sql=sql&" and personid="&session("personid")&" and EXISTS(select com_id from v_groupcomid where telid in (select id from comp_tel where personid<>"&session("personid")&") and com_id=v_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************公海挑入客户/end
					'******************三天内未处理客户/start
					...
					sql=sql&" and not EXISTS(select com_id from v_groupcomid where com_id=v_salescomp.com_id) and EXISTS(select com_id from Crm_Assign where com_id=v_salescomp.com_id and fdate<'"&date()-2&"')"
					else
					sql=sql&" and personid="&session("personid")&" and not EXISTS(select com_id from v_groupcomid where com_id=v_salescomp.com_id) and EXISTS(select com_id from Crm_Assign where com_id=v_salescomp.com_id and fdate<'"&date()-2&"')"
					end if
					sear=sear&"&dotype="&dotype
					end if
					'******************三天内未处理客户/end
					...
					if session("userid")="10" then
					sql=sql&" and EXISTS(select com_id from Crm_Assignweb where Emphases=1 and com_id=v_salescomp.com_id)"
					else
					sql=sql&" and wpersonid="&session("personid")&" and EXISTS(select com_id from Crm_Assignweb where Emphases=1 and com_id=v_salescomp.com_id and personid="&session("personid")&")"
					end if
					sear=sear&"&dotype="&dotype
					end if
					'******************我的重点客户/end
					...
					if dotype="all" then
							...
							'sql=sql&" and wpersonid is null and teldate<'"&date()-60&"' and com_regtime<'"&date()-30&"' and com_id not in (select com_id from comp_zstinfo)"
							'********再生通私海
							'sql=sql&" and not exists(select null from Crm_Assign where com_id=v_salescomp.com_id)"
							''调整
							sql=sql&" and exists(select null from crm_web_compall where com_id=v_salescomp.com_id) and not exists(select null from crm_assignweb where com_id=v_salescomp.com_id)"
							'*******排出激活部
					...
					'普通人员限制地区
						if session("Partadmin")="0" and session("userid")<>"10" then
							sqlpt="select provinceID from crm_webProvince_assgin where personid="&session("personid")&""
							set rspt=conn.execute(sqlpt)
							if not rspt.eof or not rspt.bof then
								provinceID=rspt(0)
							else
								provinceID="0"
							end if
							rspt.close
							set rspt=nothing
							sql=sql&" and exists(select null from comp_ProvinceID where com_id=v_salescomp.com_id and province in ("&provinceID&"))"
						end if
					'---------------
					end if
					'******************再生通客户/start
					if dotype="allall" then
						...
						'if session("Partadmin")="0" then
'							if session("userid")<>"10" then
'								response.Write("所有客户列表暂时关闭")
'								response.End()
'							end if
'						end if
					elseif dotype<>"xm" then
						sql=sql&" and not EXISTS(select com_id from crm_webwaste where waste=1 and com_id=v_salescomp.com_id)"
					end if
					'****************青岛展会客户
					...
					'******************废品池/start
					if dotype="xm" then
					sql=sql&" and com_id in (select com_id from crm_webwaste) "
					        if request("doperson")<>"" then
								sql=sql&" and wpersonid="&request("doperson")&" "
								sear=sear&"&doperson="&request("doperson")
						    end if
					...
					sql=sql&" and EXISTS(select com_id from crm_assignVIP where com_id=v_salescomp.com_id and personid="&session("personid")&") and not exists(select com_id from Crm_Assign where com_id=v_salescomp.com_id)"
					sear=sear&"&dotype="&dotype
					end if
					'******************vip客户/end
					'******************特殊意向客户/start
					...
					if dotype="myyescontact" then
						if session("userid")="10" then
						sql=sql&" and EXISTS (select com_id from  comp_tel where com_id=v_salescomp.com_id and telflag=1) and wpersonid is not null"
						else
						sql=sql&" and wpersonid="&session("personid")&"  and EXISTS (select com_id from comp_tel where personid="&session("personid")&" and com_id=v_salescomp.com_id and telflag=1)"
						end if
						sear=sear&"&dotype="&dotype
						if request("subdotype")="continue" then
							sql=sql&" and vip_dateto<'"&date()+90&"' and vip_dateto>='"&date()&"'"
					...
					if session("userid")="10" or session("personid")="32" then
							...
							sql=sql&" and not EXISTS (select com_id from v_groupcomid where  com_id=v_salescomp.com_id) and wpersonid is not null"
					else
							sql=sql&" and wpersonid="&session("personid")&"  and not EXISTS (select com_id from v_groupcomid where  com_id=v_salescomp.com_id)"
					end if
					if request("subdotype")="continue" then
						sql=sql&" and vip_dateto<'"&date()+90&"' and vip_dateto>='"&date()&"'"
						sear=sear&"&subdotype="&request("subdotype")
					...
					if dotype="tnoassign" then
						sql=sql&" and not EXISTS(select com_id from Crm_Assign where com_id=v_salescomp.com_id) and not EXISTS(select com_id from Comp_ZSTinfo where com_id=v_salescomp.com_id) and com_regtime>'"&date()-1&"' and com_regtime<'"&date()&"' and (adminuser=0 or com_id in (select com_id from comp_logincount))"
						sear=sear&"&dotype="&dotype
					end if
					'本部门录入的客户/公海 /star
					if dotype="insertComp" then
						sql=sql&" and EXISTS(select com_id from crm_InsertCompWeb where com_id=v_salescomp.com_id and saletype=2) and not EXISTS(select com_id from Crm_Assignweb where com_id=v_salescomp.com_id)"
						sear=sear&"&dotype="&dotype
					end if
					'本部门录入的客户/公海 /end
					'我录入的客户 /star
					...
					if dotype="allputong" then
						sql=sql&" and teldate<='"&date()-30&"' and not EXISTS(select com_id from Crm_Assignweb where com_id=v_salescomp.com_id) "
						sear=sear&"&dotype="&dotype
					end if
					'公海普通的客户 /end
					
					if dotype="vipnoassign" then
					sql=sql&" and (vipflag=1 and vip_check=1) and not EXISTS (select com_id from crm_assign where Cushion<>-1 and com_id=v_salescomp.com_id) and not EXISTS (select com_id from crm_assignVIP where  com_id=v_salescomp.com_id) and not EXISTS (select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) "
					sear=sear&"&dotype="&dotype
					end if
					...
					select case request("comporder")
						case "1"
						sqlorder=sqlorder&"com_webrank "&ascdesc&""
						case "2"
						sqlorder=sqlorder&" logincount "&ascdesc&""
						case "3"
						sqlorder=sqlorder&"webcontactnext_time "&ascdesc&""
						case "4"
				   ...
				   With oPage
				     .sltFld  = "com_tel,com_mobile,com_email,com_province,com_id,com_name,com_pass,vip_date,vip_datefrom,vipflag,viptype,vip_check,webcontactnext_time,com_rank,com_webrank,com_regtime,viewcount,logincount,lastlogintime,teldate,vip_dateto"
					 .FROMTbl = "v_salescomp"
				     .sqlOrder= sqlorder
    ...
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" class=ee id=ListTable border="0" cellpadding="2" cellspacing="0">
              <form action="webcrm_allcomp_list.asp" method="post" name=form1>
                <tr class="topline"> 
...
等级</td>
                  ...
                  销售</td>
                  <td align="center" class=sort id=td_address nowrap bgcolor="#DFEBFF">BD
                    <br>
                  销售</td>
                </tr>
                <%
				function showbdtype(code)
					sqlty="select meno from cate_webCompType where code='"&code&"'"
				...
				if rs("webcontactnext_time")<>"1900-1-1" then
					contactnext_time=rs("webcontactnext_time")
				else
					contactnext_time=""
				end if
				...
				lastlogintime=rs("lastlogintime")
				  ...
				  sqls="select top 1 teldate,personid from comp_tel where telflag=0 and com_id="&rs("com_id")&" and ContactType=13 order by id desc"
                ...
                <tr style="cursor:hand" bgcolor="<%=cbg%>"> 
                  ...
                  <%
				  'sqlas="select fdate from crm_AssignWeb where com_id="&rs("com_id")&""
...
'				  rsas.close
'				  set rsas=nothing
				  'sqlzs="select TowebContact from comp_sales where com_id="&rs("com_id")
...
'				  set rszs=nothing
				  ...
				  'sqlzs="select com_id from Crm_Comp_Share where com_id="&rs("com_id")&" and salesType=1"
'				  set rszs=conn.execute(sqlzs)
'				  if not rszs.eof then
'					  salesexiats=0
'				  end if
...
'				  set rszs=nothing
				  ...
				  '---------------------------
				 %>
				 <SPAN onmouseover=DoHL();  onDblClick="javascript:DoSL();javascript:oow('../crmlocal/webcrm_cominfoedit.asp?idprod=<%response.Write(rs("com_id"))%>&cemail=<%response.Write(rs("com_email"))%>&dotype=<%response.Write(dotype)%>');" onmouseout=DoLL();>
				
				  <td nowrap>
				  
                    <%
					sqltp="select com_type from crm_WebcompKind where com_id="&rs("com_id")&""
				...
				set rs0=nothing
				  ...
				  <div class="crm_tishi">
				  <%
				  sqlr="select realname from users where id="&wpersonid
				  ...
				  set rsr=nothing
				  %>
				  <%=lteldate%><%=realname%>联系过
				  </div>
				  <%
				  end if
				  if rs("vipflag")=2 and rs("vip_check")=1 then
					  if datediff("d",cdate(rs("vip_datefrom")),now)<30 or datediff("d",now,cdate(rs("vip_dateto")))<90 then
					    response.Write("<div class='crm_tishi'>")
					  	response.Write("<font color=#9966ff>此客户处于签单后一个月或续签前三个月，请匆进行品牌通销售</font>")
						response.Write("</div>")
					  end if
                  ...
                  <td>
                    <%
				  if rs("com_webrank")<>"-1" then
				  response.Write(rs("com_webrank")&"星")
				  end if
				  %>                 </td>
...
'					sqlr=sqlr&" ) and com_id<>"&com_id&""
				  ...
				  <a href="###kang" onClick="window.open('pp_comp.asp?mobile=<%response.Write(rs("com_mobile"))%>&tel=<%=rs("com_tel")%>&dotype=<%=dotype%>&com_id=<%=rs("com_id")%>','_blank','width=600,height=500')"><%response.Write(rs("com_mobile"))%></a>
				  <%else%>
...
<input name="Submit5" type="button" class="button" value="给再生通部" onClick="outselectcrmTozst(this.form)">
<%end if%>
					<input name="Submit4" type="button" class="button" value="加到共享客户库" onClick="inselectcrmToshare(this.form)">
                    <input name="Submit4" type="button" class="button" value="取消共享客户" onClick="outselectcrmToshare(this.form)">
