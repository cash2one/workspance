...
<table width="100%" id=secTable border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td class="<%if doaction="reg" then response.Write("selected") else response.Write("unselect") end if%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&cach=<%response.Write(docach)%>&action=reg'">注册客户</td>
    <td class="<%if doaction="vip" then response.Write("selected") else response.Write("unselect") end if%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&cach=<%response.Write(docach)%>&action=vip'">VIP试用客户</td>
    <td class="<%if doaction="sqvip" then response.Write("selected") else response.Write("unselect") end if%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&cach=<%response.Write(docach)%>&action=sqvip'">申请VIP试用</td>
	...
	<td class="<%if doaction="allzst" then response.Write("selected") else response.Write("unselect") end if%>" onClick="window.location='?dotype=<%response.Write(dotype)%>&cach=<%response.Write(docach)%>&action=allzst'">所有再生通</td>
	<%end if%>
	<%end if%>
...
<SCRIPT type=text/javascript>
    ...
    <tr>
      ...
      <%if dotype="allall" or dotype="all" or (dotype="my" and session("userid")="10") or (dotype="today" and session("userid")="10") or (dotype="contact" and session("userid")="10") or (dotype="nocontact" and session("userid")="10") or (dotype="myzst" and session("userid")="10") or (dotype="xm") then%>
...
'***********搜索/start
					...
					sql=sql&" and com_contactperson like '%"&trim(request("com_contactperson"))&"%'"
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
					...
					if dotype<>"today" and doaction<>"allzst" and dotype<>"contact" then
					...
					if dotype="all" then
							...
							sql=sql&" and personid is null and not EXISTS(select com_id from comp_sales where com_type='13' and com_id=v_salescomp.com_id) and (com_id in (select com_id from comp_logincount)) "
							sear=sear&"&dotype="&dotype
					end if
					'******************公海客户/end
					'******************所有未分配的客户/start
...
-->
                ...
                <tr bgcolor="#FFFFFF"> 
                  <td colspan="25"> 全选 
					...
					<%end if%>
					<%if dotype="my" or dotype="nocontact" or dotype="today" or dotype="contact" then%>
