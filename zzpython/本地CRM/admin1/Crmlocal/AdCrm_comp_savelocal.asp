...
<%
        ...
        sqlc="select com_id,personid from crm_assignad where com_id="&com_id&""
		set rsc=conn.execute(sqlc)
		if rsc.eof or rsc.bof then
		   sqlu="insert into crm_assignad(com_id,personid) values("&com_id&","&session("personid")&")"
		   conn.execute(sqlu)
		   ...
		   rs.close
		   set rs=nothing
		   sql="insert into crm_InsertCompWeb(com_id,personid,saletype) values("&com_id&","&session("personid")&",4)"
		   conn.execute(sql)
