...
<table width="100%" border="0" cellspacing="1" cellpadding="5" bgcolor="#666">
  <tr>
    ...
    <td align="center" nowrap="nowrap" bgcolor="#FFFFFF"><a href="/admin1/compinfo/Telcomp.asp">本地CRM号码错误客户</a></td>
    <td align="center" bgcolor="#CCCCCC">无人接听，停机，关机</td>
  </tr>
</table>
<%
sql=""
sear="n="
	   ...
	   With oPage
		 ...
		 '.sqlWhere= "WHERE exists(select com_id from crm_compcontactInfo where com_id=temp_salescomp.com_id and c_NoContact<>2 and c_NoContact>0) and not exists(select com_id from crm_compcontactInfo where  c_Type=13 and com_id=temp_salescomp.com_id) and not exists(select com_id from comp_comType where com_id=temp_salescomp.com_id)"&sql
		 .sqlWhere= "WHERE exists(select com_id from comp_comTypeCheck where com_id=temp_salescomp.com_id and Fcheck=0 and comType=11) "&sql
		 .keyFld  = "com_id"    '不可缺少
		 .pageSize= 10
		 .getConn = conn
		 Set Rs  = .pageRs
...
<table width="100%" border="0" cellspacing="1" cellpadding="5" bgcolor="#666666">
  ...
  <tr>
    ...
    <td colspan="5" bgcolor="#FFFFFF">全选
      ...
      <script>selectOption("comType","11")</script>   
