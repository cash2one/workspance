...
<%
		...
		ckeywords=request.Form("ckeywords")
		com_id=idprod
if request("re")<>"" then
		'*************判断二级域名重复
		...
		'********************
		  ...
		  '***************主营方向
			if request("salesType")<>"" then
				...
				if rsreg.eof or rsreg.bof then
					sqlinreg="insert into comp_salestype(id,com_id,salestype,salestext,buytext) values(0,"&com_id&",'"&request.Form("salestype")&"','"&replacequot(request.Form("salestext"))&"','"&replacequot(request.Form("buytext"))&"')"
		  ...
		  if not rs.eof or not rs.bof then
			...
			conn.execute(sqlt)
		  	...
		  	sqlt="insert into comp_salestype(id,com_id,cmeet) values(0,"&com_id&",'"&CMeet&"')"
			conn.execute(sqlt)
		  end if
		  rs.close
		  set rs=nothing
			...
			response.write "<script language='javascript'>parent.document.getElementById('form1').action='http://admin.zz91.com/admin1/compinfo/crm_comedit_re.asp?frompage="&frompage&""&frompagequrstr&"';parent.document.getElementById('form1').target='';parent.document.getElementById('form1').submit()</script>"'从postcheck.asp 引入的
			response.end
end if
%>
