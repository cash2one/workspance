...
<table width="100%" border="0" cellspacing="1" cellpadding="5" bgcolor="#666">
  <tr>
    ...
    <td align="center" nowrap="nowrap" bgcolor="#CCCCCC"><a href="TelcompYX.asp">发生有效联系的客户</a></td>
  </tr>
</table>
<%
sql=""
sql=sql&" and exists(select com_id from comp_sales where contacttype=13 and com_id=temp_salescomp.com_id)"
sear="n="
	   ...
	   With oPage
		 ...
		 .sqlWhere= "WHERE not exists(select com_id from comp_comTypeCheck where com_id=temp_salescomp.com_id) and not exists(select com_id from comp_comType where com_id=temp_salescomp.com_id) "&sql
		 .keyFld  = "com_id"    '不可缺少
		 .pageSize= 10
		 .getConn = conn
		 Set Rs  = .pageRs
