...
Class clsPageRs
  ...
  Public Function getTotalPage
	...
	cmdcout.commandtext = "zz91_getviporderpage" 
	...
	cmdcout.Parameters.Append cmdcout.CreateParameter("@sql",adVarChar,adParamInput,2000," "&fFromTbl&" as a where "&fSqlWhere)
  ...
  Public Function pageRs
	...
	cmd.commandtext = "zz91_viporderpage" 
	...
	-->
	 ...
	 <%
	  if request("searchname")="" then
		 if SafeRequest("id1",1)<>"" and SafeRequest("id2",1)="" then
		   sql="select cbcount from cls_b where cb_id="&SafeRequest("id1",1)
		   set rscb1=conn.execute(sql)
		   abcount=rscb1(0)
		   rscb1.close
		   set rscb1=nothing
		 end if
		 if SafeRequest("id2",1)<>"" then
		   sql="select cs_count from cls_s where cs_id="&SafeRequest("id2",1)
		   set rscb1=conn.execute(sql)
		   abcount=rscb1(0)
		   rscb1.close
		   set rscb1=nothing
     ...
     <%If fCurPage>1 Then %>
        <a class=gray id=TopPageNavPre href="<%=urlPrefix%>_p1.html">首页</a>&nbsp; <a class=gray id=TopPageNavPre href="<%=urlPrefix%>_p<%=fCurPage-1%>.html">上一页</a>
        <%else%>
      首页&nbsp;上一页
     <%end if%>
     <%If fCurPage-fTotalPage<0 Then %>
     <a class=gray id=TopPageNavPre href="<%=urlPrefix%>_p<%=fCurPage+1%>.html">下一页</a>&nbsp;&nbsp;<a class=gray id=TopPageNavPre href="<%=urlPrefix%>_p<%=fTotalPage%>.html">末页</a>
     <%else%>&nbsp;下一页&nbsp;末页
     <%end if%></div>
    <%
  End Sub
