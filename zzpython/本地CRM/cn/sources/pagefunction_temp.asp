...
Class clsPageRs2
  ...
  Public Property Let searprovince(byRef val)
    Fsearprovince=val
  End Property
  ...
  Public Property Let FromTbl(byRef val)
    fFromTbl=val
  End Property
  Public Property Let Searchname(byRef val)
    Fsearchname=val
  End Property
  
  Public Property Let Sqlsearch(byRef val)
    Fsqlsearch=val
  End Property
  ...
  Public Function getTotalPage
    sqlTotalCount="SELECT COUNT(1) FROM "& FromTbl &" "& sqlWhere & Fsqlsearch
  ...
  Public Function pageRs
    ...
    Else
      ...
      'Else  '联合查询等时最好使用Not In
        pageSql = sqlStart &" NOT IN "& sqlEnd &" "
      'End If
    End If
    'response.Write pageSql
	'response.Write(Fsearprovince)
    Set pageRs=fConn.Execute("EXEC zz91_temp '"&pageSql&"','"&fSqlOrder&"','"&Fsearchname&"','"&Fsearprovince&"'")
  End Function
...
<div align="right" id="pagetop"><font color="#666666">
  <%If fCurPage>1 Then %>
        <a href="<%=pageUrl%>&page=1">首页</a>&nbsp; <a href="<%=pageUrl%>&page=<%=fCurPage-1%>">上一页</a>
        <%else%>
  首页&nbsp;上一页
  <%end if%>
  <%If fCurPage-fTotalPage<0 Then %>
  <a href="<%=pageUrl%>&page=<%=fCurPage+1%>">下一页</a>&nbsp;&nbsp;<a href="<%=pageUrl%>&page=<%=fTotalPage%>">尾页</a>
  <%else%>
  &nbsp;下一页&nbsp;尾页
<%end if%></div>
