...
%>
      <%if page>1 then%>
      &nbsp;&nbsp;&nbsp;&nbsp; <a href="<%response.Write(towardpage)%>_p1.html">首页</a>&nbsp;<a href="<%response.Write(towardpage)%>_p<%=page-1%>.html">上一页</a>
      <%else%>&nbsp;&nbsp;&nbsp;&nbsp;首页&nbsp;上一页&nbsp;<%end if%>
  <%if beg>1 then%> <a href="<%response.Write(towardpage)%>_p<%=page-1%>.html"><img src="http://www.zz91.com/zz91images/icon_ar.gif" border="0" align="absmiddle" /></a>&nbsp;
    ...
    <%if enf<pg then%>&nbsp;<a href="<%response.Write(towardpage)%>_p<%=page+1%>.html"><img src="http://www.zz91.com/zz91images/icon_al.gif" border="0" align="absmiddle" /></a>
        <%end if%>
<%if totalpg-page>0 then%><a href="<%response.Write(towardpage)%>_p<%=page+1%>.html">下一页</a>&nbsp;<a href="<%response.Write(towardpage)%>_p<%=totalpg%>.html">末页</a>
<%else%>下一页 末页<%end if%>
</div>