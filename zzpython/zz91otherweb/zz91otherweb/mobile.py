...
from zz91tools import formattime,getoptionlist,date_to_strall,date_to_str
...
def pay_order(request):
    ...
    after_range_num = funpage.after_range_num(3)
    before_range_num = funpage.before_range_num(6)
    pay_orderlist=zzm.getpay_orderlist(frompageCount=frompageCount,limitNum=limitNum,out_trade_no=out_trade_no,mobile=mobile)
    listcount=0
...
def addchongzhiok(request):
    ...
    if paywid:
        ...
        sql='update pay_mobileWallet set company_id=%s,fee=%s,ftype=%s,gmt_modified=%s,paytype=%s,payfrom=%s where id=%s'
    else:
        argument=[company_id,fee,ftype,gmt_date,gmt_created,gmt_created,paytype,payfrom]
        sql='insert into pay_mobileWallet(company_id,fee,ftype,gmt_date,gmt_created,gmt_modified,paytype,payfrom) values(%s,%s,%s,%s,%s,%s,%s,%s)'
    zzm.dbc.updatetodb(sql,argument)
    return HttpResponseRedirect(request_url)
...
def sunfeechart(request):
    ...
    if ',' in datall:
        ...
        ftype=datall
        argument=[ftype]
    sql='select sum(fee) from pay_mobileWallet where ftype=%s'+sqlarg
