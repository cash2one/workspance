...
def price_category_field(request):
    ...
    pricectattrlist=zzprice.getpricectfieldlist(frompageCount,limitNum,price_category_id)
...
#        pricectattrlist2=zzprice.getpricectattrlist2()
        ...
        return render_to_response('price/add_pricefield.html',locals())
    if priceattrid:
        sql='update price_category_field set price_category_id=%s,name=%s,field=%s,sortrank=%s where id=%s'
        zzprice.dbc.updatetodb(sql,[price_category_id,name,field,sortrank,priceattrid])
    else:
        sql='insert into price_category_field(price_category_id,name,field,sortrank) values(%s,%s,%s,%s)'
        zzprice.dbc.updatetodb(sql,[price_category_id,name,field,sortrank])
    return HttpResponseRedirect(request_url)
