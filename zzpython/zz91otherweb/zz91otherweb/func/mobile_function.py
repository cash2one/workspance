...
class mshop:
    def __init__(self):
        self.dbc=dbc
    def getshop_product_ranklist(self,frompageCount,limitNum,paytype=''):
    ...
    def getshop_productlist(self,frompageCount,limitNum,paytype=''):
        sqlarg=' from shop_showphone where id>0'
        argument=[]
        if paytype:
...
class mobile:
    def __init__(self):
        self.dbc=dbc
    def getpay_orderlist(self,frompageCount,limitNum,out_trade_no='',mobile=''):
        ...
        sql1='select count(0)'+sqlarg
        sql='select id,buyer_email,buyer_id,is_success,notify_id,notify_time,out_trade_no,subject,total_fee,trade_no,trade_status,contact,mobile,gmt_created'+sqlarg
        sql+=' order by id desc limit '+str(frompageCount)+','+str(limitNum)
        count=self.dbc.fetchnumberdb(sql1,argument)
        resultlist=self.dbc.fetchalldb(sql,argument)
        listall=[]
        for result in resultlist:
            ...
            list={'id':id,'buyer_email':buyer_email,'buyer_id':buyer_id,'is_success':is_success,'notify_id':notify_id,'notify_time':notify_time,'out_trade_no':out_trade_no,'subject':subject,'total_fee':total_fee,'trade_no':trade_no,'trade_status':trade_status,'contact':contact,'mobile':mobile,'gmt_created':gmt_created}
            listall.append(list)
        return {'list':listall,'count':count}
    ...
    def getoutfeelist(self,frompageCount,limitNum,type='',gmtdate='',gmt_begin='',gmt_end='',paytypeid='',company_id='',company_name='',account='',mobile='',paytypem=''):
        ...
        sql='select a.id,a.company_id,a.fee,a.ftype,a.gmt_created,b.name,c.account,c.mobile,a.payfrom,a.paytype,a.forcompany_id,a.product_id'+sqlarg
        ...
        for result in resultlist:
            ...
            list={'id':id,'company_id':company_id,'company_name':company_name,'account':account,'mobile':mobile,'fee':fee,'ftype':ftype,'ftypename':ftypename,'gmt_created':gmt_created,'paytype':paytype,'payfrom':payfrom,'forcompany_id':forcompany_id,'product_id':product_id}
            listall.append(list)
    ...
    def getftypefee(self,id):
        ...
        if result:
            return result[0]
    def getpaytypelist(self,frompageCount=0,limitNum=20,paytype='',subid=''):
        sqlarg=''
        if paytype==1:
            sqlarg+=' and fee>=0'
        elif paytype==2:
