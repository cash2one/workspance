...
class zz91price:
    ...
    def getpricefielddetail(self,id):
        sql='select price_category_id,name,field,sortrank from price_category_field where id=%s'
        result=self.dbc.fetchonedb(sql,[id])
        list={'price_category_id':'','price_category_label':'','name':'','field':'','sortrank':''}
        if result:
            ...
            field=result[2]
            sortrank=result[3]
            list={'price_category_id':price_category_id,'price_category_label':price_category_label,'name':name,'field':field,'sortrank':sortrank}
        return list
...
#            listall=listallcache
        return listall
    def getpricecategorylist(self):
        listall=[]
        listallcache=cache.get("getpricecategorylist")
        if not listallcache:
            ...
            for result in resultlist:
                ...
                listall.append(list)
            cache.set("getpricecategorylist",listall,60*60000)
        else:
            listall=listallcache
        return listall
    ...
    def getpricenextcategory2(self,id):
        ...
        return listall
    def getpricectfieldlist(self,frompageCount='',limitNum='',price_cid=''):
        sql1='select count(0) from price_category_field where id>0'
        sql='select id,price_category_id,name,field,sortrank from price_category_field where id>0'
        argument=[]
        if price_cid:
            sql1=sql1+' and price_category_id=%s'
            sql=sql+' and price_category_id=%s'
        ...
        for result in resultlist:
            ...
            field=result[3]
            sortrank=result[4]
            list={'id':id,'price_category_id':price_category_id,'price_category_label':price_category_label,'name':name,'field':field,'sortrank':sortrank}
            listall.append(list)
        return {'list':listall,'count':count}