...
def showppccomplist2(request):
    ...
    keywords = request.GET.get("keywords")
    listall=getppccomplist(int(mm),keywords,int(page),adposition=adposition)['listall']
    return render_to_response('script_showppccomplist2.html',locals())
...
def showppcscript2(request):
    ...
    listall=ppclist['listall']
    lastpage=ppclist['lastpage']
    return render_to_response('app/ppc_ad2.html',locals())
...
## 来电宝 隐藏号码页面
def showppctxtadscript2(request):
...
def getdomainhtml(request,company_id):
    html="<div class=weixin_code style='margin-left:-160px;width:auto'>"
    sqll="select id from crm_company_service where company_id=%s and crm_service_code ='10001007' and apply_status=1 and DATEDIFF(CURDATE(),`gmt_end`)<=0"
    wzresult=dbc.fetchonedb(sqll,[company_id])
    if wzresult:
        html=html+"<div style='text-align:center;color:#fff;background-color: #6CB72F'>用手机查看我的微站</div><img src='http://pyapp.zz91.com/getdomainpic"+str(company_id)+".html'>"
    sqll="select id from crm_company_service where company_id=%s and crm_service_code ='10001009' and apply_status=1 and DATEDIFF(CURDATE(),`gmt_end`)<=0"
    wzresult=dbc.fetchonedb(sqll,[company_id])
    if wzresult:
        html=html+"<div style='text-align:center;color:#fff;background-color: #6CB72F'>我的工厂秀</div><img src='http://pyapp.zz91.com/getqiyexiupic"+str(company_id)+".html'>"
    html=html+"</div>"
...
def getqiyexiupic(request,company_id):
    ...
    if domain:
        ...
        return None
#-----我的名片
def getvcardpic(request,company_id):
    ...
    if returnresult:
        ...
        domain="http://m.zz91.com/company/detail"+str(company_id)+".html"
    if domain:
        qr = qrcode.QRCode(
                        version=1,
                        error_correction=qrcode.constants.ERROR_CORRECT_L,
...
#判断是否可以抽奖（完成3个操作的用户）
def choujiangstaus(request):
    company_id=request.GET.get("company_id")
    s=getchoujiangstaus(company_id)
    if s:
        return HttpResponse('var _suggest_result_='+str(s))
#获得抽奖次数
def choujiangcount(request):
    company_id=request.GET.get("company_id")
    sql="select sum(bnum) from subject_choujiang where company_id=%s"
    result=dbc.fetchonedb(sql,[company_id])
    if result:
        bnum=result[0]
        if not bnum:
            bnum=0
        return HttpResponse('var _suggest_result_={"count":"'+str(bnum)+'"}')
#2016砸金蛋活动
def zajindanstaus(request):
    company_id=request.GET.get("company_id")
    s=getzajindanstaus(company_id)
    if s:
        return HttpResponse('var _suggest_result_='+str(s))
#砸金蛋次数
def zajindancount(request):
    company_id=request.GET.get("company_id")
    sql="select count(*) from subject_choujiang where company_id=%s and DATEDIFF(CURDATE(),gmt_created)<1 and btype=4"
    result=dbc.fetchonedb(sql,[company_id])
    if result:
        bnum=result[0]
        if not bnum:
        ...
        return HttpResponse('var _suggest_result_={"count":"'+str(bnum)+'"}')
#刮刮乐次数
def guagualecount(request):
    company_id=request.GET.get("company_id")
    sql="select count(*) from subject_choujiang where company_id=%s and DATEDIFF(CURDATE(),gmt_created)<1 and btype=5"
    result=dbc.fetchonedb(sql,[company_id])
    if result:
        bnum=result[0]
        if not bnum:
        ...
        bnum=1-bnum
        return HttpResponse('var _suggest_result_={"count":"'+str(bnum)+'"}')
def guagualestaus(request):
    company_id=request.GET.get("company_id")
    s=getguagualestaus(company_id)
    if s:
        return HttpResponse('var _suggest_result_='+str(s))
