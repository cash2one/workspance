...
def getcompinfo(pdtid,cursort,keywords,company_id=''):
	...
	if (productsinfo==None):
		...
		if productlist:
			...
			#---接听率
			sql1="select phone_rate,level from ldb_level where company_id=%s"
...
def replaceurl(content):
    ...
    return content
#判断是否可以抽奖（完成3个操作的用户）
def getchoujiangstaus(company_id):
	gmt_created=getNow()
	bnum=-1
	btype=0
	sql="select id from subject_choujiang where company_id=%s and btype in (0,1,2,3)"
	result=dbc.fetchonedb(sql,[company_id])
	list={"firstchoujiang":"0","choujiangcount":"0"}
	if result:
		list["firstchoujiang"]="1"
	...
	h6=range(20,22)+[6,0]
	h5=range(60,130)+[5,0]
	h4=range(140,150)+[4,0]
	h2=range(190,200)+[2,0]
	#h7=range(250,300)
	alllist.append(h0)
	alllist.append(h6)
	alllist.append(h5)
	...
	haoma=[0,2,4,5,6,7]
	hlist=alllist[random.choice(range(0,5))]
	
	
	list["list"]=hlist
	xuanzhong=random.choice(hlist)
	list["xuanzhong"]=xuanzhong
	if xuanzhong not in haoma:
		list["xuanzhong"]=7
	choujiangcount=0
	#获得抽奖次数
	sql="select sum(bnum) from subject_choujiang where company_id=%s and btype in (0,1,2,3)"
	result=dbc.fetchonedb(sql,[company_id])
	if result:
		if result[0]:
			choujiangcount=result[0]
			
	if choujiangcount>0:
		#保存抽中记录
		jiangpin=list["xuanzhong"]
		...
		list["choujiangcount"]=str(int(choujiangcount)-1)
	return list

def getzajindanstaus(company_id):
	gmt_created=getNow()
	bnum=1
	btype=4
	...
	try:
		...
		r= requests.get("http://app.zz91.com/qianbao/qianbaobaoblance.html",params=payload)
		if r:
			qianbaobaoblance=r.text
		else:
			qianbaobaoblance=0
	except:
		qianbaobaoblance=0
	...
	list={"qianbaobaoblance":float(qianbaobaoblance),"choujiangcount":"0"}
	...
	choujiangcount=0
	#获得抽奖次数
	if float(qianbaobaoblance)>10:
		sql="select sum(bnum) from subject_choujiang where company_id=%s and DATEDIFF(CURDATE(),gmt_created)<1 and btype=4"
		result=dbc.fetchonedb(sql,[company_id])
		if result:
			if result[0]:
				choujiangcount=result[0]
		...
		if choujiangcount<3:
			...
			jiangpin=list["xuanzhong"]
			try:
				payload={'company_id':str(company_id),'ftype':'31','fee':'-10'}
				r= requests.get("http://app.zz91.com/payfee.html",params=payload)
				sql="insert into subject_choujiang(btype,gmt_created,bnum,company_id,jiangpin) values(%s,%s,%s,%s,%s)"
				dbc.updatetodb(sql,[btype,gmt_created,bnum,company_id,jiangpin])
				list["choujiangcount"]=str(int(choujiangcount)-1)
			except:
	...
	return list
#刮刮乐
def getguagualestaus(company_id):
	gmt_created=getNow()
	bnum=1
	btype=5
	list={"choujiangcount":"0"}
	...
	haoma=[1,2,3,4]
	hlist=alllist[random.choice(range(0,4))]
	...
	choujiangcount=0
	#获得抽奖次数
	sql="select sum(bnum) from subject_choujiang where company_id=%s and DATEDIFF(CURDATE(),gmt_created)<1 and btype=5"
	result=dbc.fetchonedb(sql,[company_id])
	if result:
		if result[0]:
			choujiangcount=result[0]
	...
	if choujiangcount<1:
		...
		try:
			if fee>0:
				payload={'company_id':str(company_id),'ftype':'32','fee':fee,'more':1}
				r= requests.get("http://app.zz91.com/sendfee.html",params=payload)
			sql="insert into subject_choujiang(btype,gmt_created,bnum,company_id,jiangpin) values(%s,%s,%s,%s,%s)"
			dbc.updatetodb(sql,[btype,gmt_created,bnum,company_id,jiangpin])
			list["choujiangcount"]="1"
		except:
			a=1
	return list
