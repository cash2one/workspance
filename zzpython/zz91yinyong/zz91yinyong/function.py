...
def getindexcategorylist(code,showflag):
	catelist=cache.get("mobile_cate1"+str(code))
	if (catelist==None):
		if (showflag==2):
		...
		cache.set("mobile_cate1"+str(code),listall_cate,60*60000)
	else:
...
def keywordsTop(keywords):
	keytop=cache.get("mobile_keywordstop"+str(keywords))
	if keytop:
		return keytop
	...
	cache.set("mobile_keywordstop"+str(keywords),results,60*60)
	return results
def keywords_fix(keywords):
	keyfix=cache.get("mobile_keywordsfix"+str(keywords))
	if keyfix:
		return keyfix
...
def get_typenews(typeid,typeid2,cursor_news):
	sql="select id,title from dede_archives where typeid=%s and typeid2=%s order by pubdate desc limit 0,5"
	resultlist=dbn.fetchonedb(sql,[typeid,typeid2])
	listall=[]
	if resultlist:
		for result in resultlist:
