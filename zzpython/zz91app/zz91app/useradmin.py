    ...
    if plist:
        ...
        if list:
            ...
            if str(company_id)=="0":
                ...
                sqlc="select id from oauth_access where open_id=%s and target_account=%s and open_type='app.zz91.com'"
                list=dbc.fetchonedb(sqlc,[str(appid),account])
                if list:
                    sql="update oauth_access set closeflag=0 where id=%s"
                    dbc.updatetodb(sql,[list[0]])
                    error="suc"
                else:
                    sqld="select target_account,id from oauth_access where open_id=%s and open_type='app.zz91.com'"
                    listd=dbc.fetchonedb(sqld,[str(appid)])
                    if listd:
                        oid=listd[1]
                        target_account=listd[0]
                        if not target_account or str(target_account)=="None":
                            sql="update oauth_access set closeflag=0,target_account=%s where id=%s"
                            dbc.updatetodb(sql,[account,oid])
                            error="suc"
                        else:
                        ...
                        firstinstall=1
                #首次安装APP送20 再生钱包#
                zzq.getsendfee(company_id,20,12)
                #首次下载APP并登录
                if firstinstall==1:
                    choujiang(company_id=company_id)
        else:
            error="用户名或邮箱 还未在ZZ91注册！"
    else:
    ...
    if loginflag:
        if error=="suc":
            if firstinstall==1:
                ...
                qvalue=['感谢您的安装','',gmt_created,content,company_id]
            ...
            if datatype=="json":
                messagedata={'err':'false','errkey':'','result':result,'token':token}
            else:
                messagedata={'err':'false','errkey':'','result':company_id,'token':token}
            return HttpResponse(simplejson.dumps(messagedata, ensure_ascii=False))
        else:
...
def tokeninfo(request):
    ...
    sql="select token from app_token where company_id=%s and token=%s"
    listd=dbc.fetchonedb(sql,[token])
    if not listd:
...
def auth_bindingmobile(request):
    ...
    if (plist):
        ...
        if list:
            errkey="改手机号码已经绑定，你可以用改手机号码直接登陆！1"
            err="true"
            messagedata={'err':err,'errkey':errkey}
            return HttpResponse(simplejson.dumps(messagedata, ensure_ascii=False))
        ...
        if list:
            errkey="改手机号码已经绑定，你可以用改手机号码直接登陆！2"
            err="true"
            messagedata={'err':err,'errkey':errkey}
            return HttpResponse(simplejson.dumps(messagedata, ensure_ascii=False))
...
def myinfosave(request):
    ...
    if company_id:
        ...
        userlist=dbc.fetchonedbmain(sqlh,[company_id])
        if (userlist==None):
            value=[company_id,account,contact,email,tel,mobile,qq,contact,gmt_modified,gmt_created]
            sqlu="insert into bbs_user_profiler(company_id,account,nickname,email,tel,mobile,qq,real_name,gmt_modified,gmt_created) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            dbc.updatetodb(sqlu,value);
        #保存头像
        if faceurl:
        ...
        value=[comname,industryCode,business,serviceCode,areaCode,address,addresszip,company_id]
        sql="update company set name=%s,industry_code=%s,business=%s,service_code=%s,area_code=%s,address=%s,address_zip=%s where id=%s"
