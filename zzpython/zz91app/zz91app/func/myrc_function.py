...
class zmyrc:
    ...
    def getfavoritelist(self,frompageCount="",limitNum="",company_id="",checkStatus=""):
        ...
        if  checkStatus=='2':
            sqlarg=" (favorite_type_code=10091002 or favorite_type_code=10091003 or favorite_type_code=10091004) "
        if   checkStatus=='3':
        ...
        if alist:
            listcount=alist[0]           
        sql2="select favorite_type_code,content_id,content_title,id from myfavorite where company_id=%s and "+sqlarg+" order by gmt_created desc limit "+str(frompageCount)+","+str(frompageCount+limitNum)+""
    ...
    #询盘留言列表 翻页
    def getleavewordslist(self,frompageCount,limitNum,company_id,sendtype):
        port = spconfig['port']
        cl = SphinxClient()
    ...
    def getmyproductsbyid(self,id):
        ...
        if list:
            ...
            if validity=="10":
                vali="十天"
            alist={'id':list[0],'title':list[1],'quantity':list[2],'quantity_unit':list[3],'price':list[4],'price_unit':list[5],'details':list[6],'categorycode':list[7],'products_type_code':list[8],'validity':vali}
            return alist
    #产品图片
    def getmyproductspic(self,id):
        sql1="select pic_address,check_status from products_pic where product_id=%s"
        ...
        if productspic:
            for p in productspic:
                ...
                picurl={'images':pdt_images,'images_big':pdt_images_big,'check_status':check_status}
    ...
    def getmyaddressbooklist(self,frompageCount="",limitNum="",company_id=""):
        ...
        if alist:
            listcount=alist[0]           
        sql2="select forcompany_id,bz from company_addressbook where company_id=%s "+sqlarg+" order by id desc limit "+str(frompageCount)+","+str(frompageCount+limitNum)+""
        aalist=self.dbc.fetchalldb(sql2,[company_id])
        listall=[]
        if alist:
            for list in aalist:
