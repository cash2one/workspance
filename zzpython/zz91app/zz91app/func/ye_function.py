...
class zye:
    ...
    def getmarketlist(self,frompageCount='',limitNum='',industry='',category='',province=''):
        ...
        sql=sql+' order by company_num desc limit '+str(frompageCount)+','+str(limitNum+frompageCount)
    ...
    def is_in_market(self,market_id,company_id):
        sql='select company_id,is_quit from market_company where market_id=%s and company_id=%s'
        resultlist=self.dbc.fetchonedb(sql,[market_id,company_id])
        if resultlist:
            return 1
        else:
            return 0
        ...
        if res:
            if res.has_key('matches'):
                ...
                for match in tagslist:
                    ...
                    list1={'pdt_id':list['pdt_id'],'pdt_images':list['pdt_images'],'pdt_kind':list['pdt_kind'],'pdt_name':list['pdt_name'],'pdt_time_en':list['pdt_time_en'],'com_province':list['com_province'],'pdt_price':list['pdt_price']}
        ...
        if res:
            if res.has_key('matches'):
                ...
                for match in tagslist:
                    ...
                    list=zzc.getcompanydetail(company_id)
                    list1={'company_id':list['company_id'],'name':list['name'],'business':subString(list['business'],180)}
                    listall_comp.append(list1)
                listcount_comp=res['total_found']
        return {'list':listall_comp,'count':listcount_comp}
