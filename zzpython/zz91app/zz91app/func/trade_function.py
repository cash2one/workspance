...
class zztrade:
    ...
    def getcompinfo(self,pdtid,keywords,company_id=''):
        ...
        cache.set("app_productsinfo"+str(pdtid),productsinfo,60*120)
        return productsinfo
    #----相关供求类别(你是不是还想找?)
...
#                    company_numb=self.getpricelist_company_count(title)
    ...
    def getldbphone(self,company_id):
        if company_id:
            ...
            return None
    #公司供求信息 翻页
    def getcompanyproductslist(self,kname="",frompageCount="",limitNum="",company_id="",pdt_type=""):
        port = spconfig['port']
        cl = SphinxClient()
