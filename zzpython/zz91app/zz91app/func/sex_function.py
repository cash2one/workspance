...
class zznews:
    ...
    def __init__(self):
        self.dbn=dbn
    def getnewscolumn(self,mid="",deviceId=""):
        sql='select id,typename,keywords from dede_arctype where id not in (1,6,16,21,31,26) '
        if deviceId:
            ...
            sql=sql+' order by id asc limit 0,10'
        resultlist=self.dbn.fetchalldb(sql)
        if resultlist:
            listall=[{'id':0,'typeid':0,'typename':'推荐','numb':0,'url':''}]
            #listall=[]
            numb=0
            ...
            return [{'id':0,'typeid':0,'typename':'推荐','numb':0,'url':''}]
        ...
        if res:
            if res.has_key('matches'):
                ...
                for match in tagslist:
                    id=match['id']
                    attrs=match['attrs']
                    subcontent=self.getsubcontent(id,80)
                    ...
                    list1={'title':title,'picone':picone,'picmore':picmore,'click':click,'id':id,'pubdate':pubdate2,'pcount':pcount,'fcount':fcount,'typename':typename,'murl':murl,'litpic':litpic,'subcontent':subcontent.decode('utf-8','ignore')}
                    if type:
    ...
    def getsubcontent(self,id,len):
        ...
        if alist:
            content=alist[0]
            if content:
                content=filter_tags(content)[:len]
        return content
        """
    ...
    def get_newslittlecontent(self,id):
        ...
        if alist:
            ...
            click=alist[2]
            typeid=alist[3]
            typename=self.get_typename(typeid)
    ...
    def getnewscontent(self,id):
        ...
        if (newscontent==None):
            ...
            newscontent={'title':title.decode('utf-8','ignore'),'pubdate':pubdate,'content':content.decode('utf-8','ignore'),'typename':typename,'click':click,'pcount':pcount,'fcount':fcount}
...
#------以下是dedecms数据
    ...
    #获得收藏夹列表(根据userid获得)
    def get_memebe_stom(self,mid="",frompageCount="",limitNum="",allnum='',deviceId=''):
        """
        if mid:
            argument=[mid]
        ...
        if deviceId:
            argument=[deviceId]
            sql1="select count(*) from dede_member_stow where deviceId=%s"
            sql="select id,mid,aid,title from dede_member_stow where deviceId=%s order by id desc limit "+str(frompageCount)+","+str(limitNum)+""
        listall=[]
    ...
    def get_feedback(self,aid=None,mid=None,frompageCount="",limitNum="",allnum='',deviceId=''):
        ...
        if aid:
            ...
            argument.append(aid)
        if deviceId:
            sqlarg=' and deviceId=%s'
            argument.append(deviceId)
        """
        ...
        sql='select id,aid,mid,msg,bad,good,username,dtime from dede_feedback where id>0'+sqlarg+' order by id desc limit '+str(frompageCount)+','+str(limitNum)
        count=self.dbn.fetchnumberdb(sql1,argument)
        resultlist=self.dbn.fetchalldb(sql,argument)
        listall=[]
        if resultlist:
            for result in resultlist:
                ...
                bad=result[4]
                good=result[5]
                feedbackname=result[6]
                dtime=result[7]
                dtime=time.strftime('%Y-%-m-%d &nbsp;%-H:%M', time.localtime(dtime))
                list={'id':id,'aid':aid,'bad':bad,'good':good,'msg':msg,'feedbackname':feedbackname,'dtime':dtime}
                listall.append(list)
        return {'list':listall,'count':count}
    #最近浏览
    def get_history(self,mid=None,frompageCount="",limitNum="",allnum='',deviceId=''):
        argument=[]
        sqlarg=''
        """
        if mid:
        ...
        sql='select id,aid,mid from view_history where id>0'+sqlarg+' order by id desc limit '+str(frompageCount)+','+str(limitNum)
        count=self.dbn.fetchnumberdb(sql1,argument)
        resultlist=self.dbn.fetchalldb(sql,argument)
        listall=[]
        if resultlist:
