...
class zzinquiry:
    ...
    def getleavewordnew(self,account):
        ...
        if result:
            return result[0]
    #询盘留言列表 翻页
    def getleavewordslist(self,frompageCount=0,limitNum=1,scomid='',rcomid='',comtype='',is_viewed=''):
        listall=[]
        listcount=0
        port = spconfig['port']
        cl = SphinxClient()
    ...
    def getchatlist(self,frompageCount=0,limitNum=1,scomid='',rcomid=''):
        ...
        sql=sql+' order by id asc limit '+str(frompageCount)+','+str(limitNum)
    ...
    def getcompanyname(self,uname):
        ...
        if (newcode):
            ...
            if clist:
                #设置缓存
                list={'company_id':company_id,'companyname':clist[0],'contact':contact}
