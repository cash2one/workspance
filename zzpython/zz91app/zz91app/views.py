...
def messagesindex(request):
    ...
    list={'count0':count0,'count1':count1,'count2':count2,'count3':count3}
...
def index_new(request):
    ...
    #头条资讯
    hotnewslist=zzn.getnewslist(frompageCount=0,limitNum=3,flag='h')
    hotnews=[]
    for list in hotnewslist['list']:
        ll={'id':list['id'],'title':list['title'],'content':list['subcontent']}
        hotnews.append(ll)
    datatype=request.GET.get("datatype")
    if datatype=="json":
        resultlist={"pricelist":pricelist,'adlist':adlist,'onenewslist':onenewslist,'onehuzhulist':onehuzhulist,'adlistall':adlistall,'qiandao':qiandao,'hotnews':hotnews}
        return HttpResponse(simplejson.dumps(resultlist, ensure_ascii=False))
...
def leavewordslist(request):
    ...
    #----获得最新询盘
    leavewordsdata=zzi.getchatlist(rcomid=company_id,scomid=forcompany_id,frompageCount=0,limitNum=20)
...
def feedbacksave(request):
    ...
    if not content or not contact:
        messagedata={'err':'true','errkey':'请填写标题和联系方式','type':'leavewords'}
        return HttpResponse(simplejson.dumps(messagedata, ensure_ascii=False))
