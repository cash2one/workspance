/*!
 * =====================================================
 * Mui v0.5.2 (https://github.com/dcloudio/mui)
 * =====================================================
 */
/**
 * MUI核心JS
 ...
 * @type Function|_L4.$
 */
var nowurl = "http://app.zz91.com/index.html";
var win=1;
	...
	var handle = function(event, touch) {
		if (event.type === $.EVENT_END || event.type === $.EVENT_CANCEL) {
			...
			if (touch.distance < options.tabMaxDistance && touch.touchTime < options.tapMaxTime) {
				if (touch.lastTarget && (touch.lastTarget === event.target)) { //same target
		...
		if (container) {
			...
			if ($container.length === 1) {
				if ($.options.optimize && $.os.plus && $.os.android) { //android and optimize
					...
					$container.plus_pulldownRefresh(pullRefreshOptions.up,pullRefreshOptions.down);
				} else {
...
(function($) {
	 ...
	 * @returns {undefined}
	 */
	$.get = function(url, successCallback, errorCallback,arg) {
		ajax('GET', url, successCallback, errorCallback,arg);
	};
	/**
	 * mui.post
	 * @param {type} url
	 ...
	 * @returns {undefined}
	 */
	$.post = function(url, successCallback, errorCallback,arg) {
		ajax('POST', url, successCallback, errorCallback,arg);
	};
...
(function($, window, document, name, undefined) {
	...
	var toggleOffCanvas = function(container, anchor,tragfx) {
		if (container && anchor) {
			var type;
			...
			if (classList.contains(CLASS_OFF_CANVAS_LEFT)) {
				...
				if (tragfx=="show" && container.classList.contains(CLASS_LEFT)==false){
					container.classList.toggle(CLASS_LEFT);
				}
				if (tragfx=="hide" && container.classList.contains(CLASS_LEFT)==true){
					container.classList.toggle(CLASS_LEFT);
				}
	...
	window.addEventListener('tap', function(event) {
		...
		toggleOffCanvas($.targets._container, $.targets.offcanvas,"show");
	});
		...
		this.each(function() {
			...
			if (args[0] === 'show' || args[0] === 'hide' || args[0] === 'toggle') {
				...
				if (classList.contains(CLASS_OFF_CANVAS_LEFT) || classList.contains(CLASS_OFF_CANVAS_RIGHT)) {
					...
					if (container) {
						toggleOffCanvas(container, this,tragfx);
					}
				}
			}
		});
...
(function($, window) {
	...
	$.back = function() {
		...
		if (nowurl) {
			if (nowurl == "http://app.zz91.com/index.html") {
				if (winDrag==false){
					if (confirm('确认退出？')) {
		...
		if (nowurl) {
			if (nowurl != null && isBack == true && winDrag==false) {
				winback();
				isBack = false;
			}
		}
	...
	$.plusReady(function() {
		plus.key.addEventListener('backbutton', function() {
			var errbutton = document.getElementById("errbutton");
			if (errbutton != null) {
				$.back();
			} else {
				nowurl=null;
