...
function openoverlay(url,title,html,overlayheight)
{
	...
	if (overlayheight=="100%"){
		overlayheight=$(window).height()-130;
	}
	var simpleWindown_html = new String;
	simpleWindown_html ="<div id=\"TB_overlayBG\" onclick='closeoverlay()'>&nbsp;</div>";
	...
	$(".overlaybox").css({
		...
		top:($(window).height()-overlayheight)/2+$(window).scrollTop()-20+"px",
		display:"block"
	});
