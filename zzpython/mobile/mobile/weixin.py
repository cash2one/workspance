			...
			if request.body:
				...
				if (content==None  and EventKey=='qiandao'):
					...
					if waccount:
						...
						titlelist=[{'title':'立即绑定帐号，签到送大礼！','Description':'您还未绑定您的ZZ91再生网帐号，点此立即绑定。','PicUrl':'http://m.zz91.com/images/gangtan1.jpg','Url':'http://m.zz91.com/weixin/login.html?weixinid='+str(toUserName)+''}]
				...
				if (content==None  and EventKey=='binding'):
					...
					if waccount:
						gmt_created=datetime.datetime.now()
						morpic="http://img0.zz91.com/zz91/weixin/images/more.jpg"
						titlelist=[{'title':'2016年中国再生资源商务大全广告火爆招商中,招商电话：0571-56612345','Description':'招商电话：0571-56612345 56611111','PicUrl':'http://www.zz91.com/zt/2016yellapp/images/imgbnd.jpg','Url':'http://www.zz91.com/zt/2016yellapp/index.html'}]
						#titlelist=[]
						wcompany_id=getcompanyid(waccount)
						qcout=getleavewordscount(wcompany_id)
						hcount=getmyhuzhureplaycoutno(waccount)
...
def postsms(mobile,username,userid):
	...
	if (int(plist[0])<5):
		if (int(plist[0])<3):
			...
			if (o['message']=='发送成功' or o['code']=="9001"):
				return True
			else:
				return o['message']
		else:
...
def loginsave(request):
	...
	if plist:
		...
		if list:
			...
			#return HttpResponse(str(company_id)+"p")
			if list:
				aaa="b"
