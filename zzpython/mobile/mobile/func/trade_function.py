...
#            isbuycontact=self.getbuycontact(company_id,pdtid)
        productsinfo=cache.get("mobile_productscompinfo"+str(pdtid))
        if (productsinfo==None):
            ...
            productsinfo=list1
            cache.set("mobile_productscompinfo"+str(pdtid),list1,60*30)
...
#                    title=getlightkeywords(cl,[title],kname,"offersearch_new")
    ...
    def getproductdetail(self,id):
        productdetail=cache.get("mobile_productsdetail"+str(id))
        if productdetail:
        ...
        plist=self.dbc.fetchonedbmain(sql,[id])
        if plist:
            company_id=plist[0]
            title=plist[1]
            if not title:
            ...
            if clist:
                ...
                if (viptype == '10051000'):
                    ...
                    arrviptype['vipcheck']=1
                #来电宝客户
                sqll="select id from crm_company_service where company_id="+str(company_id)+" and crm_service_code in(1007,1008,1009,1010,1012) and apply_status=1 and DATEDIFF(CURDATE(),`gmt_end`)<=0"
            ...
            cache.set("mobile_productsdetail"+str(id),list,60*60)
            return list
    #----相关供求类别(你是不是还想找?)
...
#            return res
            if res.has_key('matches'):
                ...
                for match in tagslist:
                    ...
                    mobileurl='/jiage/detail'+str(td_id)+'.html'
                    attrs=match['attrs']
    ...
    def getpricelist_company(self,kname='',frompageCount='',limitNum='',maxcount=100000):
        ...
        if res:
            if res.has_key('matches'):
                ...
                for match in tagslist:
                    td_id=match['id']
                    mobileurl='/jiage/cdetail'+str(td_id)+'.html'
                    attrs=match['attrs']
...
#                    company_numb=self.getpricelist_company_count(title)
    ...
    def getdomain_zz91(self,id):
        ...
        if result:
            return result[0]
    def getcategorylabel(self,pinyin):
        sql="select label from category_products where pinyin=%s"
        results = self.dbc.fetchonedb(sql,[pinyin])
        if results:
            return results[0]
    #置顶广告
