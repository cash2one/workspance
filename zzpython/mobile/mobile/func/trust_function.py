...
class zztrust:
    ...
    def getcaigou(self,frompageCount,limitNum):
        ...
        for result in resultlist:
            ...
            differ_time=s/60
            """
            list={'id':sid,'company_id':company_id,'status':statustxt,'title':title,'useful':useful,'quantity':quantity,'price':price,'area_code':area_code,'area':area,'buy_no':buy_no,'color':color,'level':level,'differ_time':differ_time,'dealertel':dealertel,'dealername':dealername,'bjdisable':bjdisable}
            listall.append(list)
        return {'list':listall,'count':count}
    
    #获取最大的id号
    def getmaxid(self,table='',company_id=""):
        sqlidmax='select max(id) from '+table+' where company_id=%s'
        result=self.dbc.fetchonedb(sqlidmax,[company_id])
        if result:
            maxid= result[0]
        return result[0]
    #根据buy_no获得交易员id
