...
//	测试环境请求基础地址
	private $API_Pay_Base_Url = 'http://ok.yeepay.com/testpayapi/api/';
	private $API_Mobile_Pay_Base_Url = 'http://ok.yeepay.com/paymobile/';
	private $API_PC_Pay_Base_Url = 'http://ok.yeepay.com/payweb/';
	private $API_Merchant_Base_Url = 'http://ok.yeepay.com/merchant/';
...
// 	private $API_Merchant_Base_Url = 'https://ok.yeepay.com/merchant/';
	...
	/**
	 * 返回PC网页支付跳转URL地址(pc网页收银台由易宝支付提供，包括信用卡支付、储蓄卡支付和绑卡支付，具体可以看一键支付提供的接口调用示例http://mobiletest.yeepay.com/demo)
	 * 
	 ...
	 * @return string
	 */
	public function pcWebPay($order_id,$transtime,$amount,$product_catalog,$identity_id,$identity_type,$user_ip,$user_ua,$terminaltype,$terminalid,$paytypes='1|2',$orderexp_date=60,$callbackurl='',$fcallbackurl='',$currency=156,$product_name='',$product_desc=''){
		$query = array(
				'orderid'	=>	$order_id,
			'transtime'	=>	$transtime,
			'currency'	=>	$currency,
	...
	/**
	 * 返回移动终端网页支付跳转URL地址(移动终端网页收银台由易宝支付提供，包括信用卡支付、储蓄卡支付和绑卡支付，具体可以看一键支付提供的接口调用示例http://mobiletest.yeepay.com/demo)
	 * 
	 * @param string $order_id
	 * @param string $transtime
	 * @param int $amount
	 ...
	 * @return string
	 */
	public function webPay($order_id,$transtime,$amount,$product_catalog,$identity_id,$identity_type,$user_ip,$user_ua,$callbackurl='',$fcallbackurl='',$currency=156,$product_name='',$product_desc='',$terminaltype,$terminalid,$orderexp_date=60){
		$query = array(
			'orderid'	=>	$order_id,
			'transtime'	=>	$transtime,
			'currency'	=>	$currency,
		...
		return $this->getUrl(YEEPAY_MOBILE_API, 'api/pay/request', $query);
	}
	...
	/**
	 * 绑卡支付请求。在绑卡支付请求后，会有同步返回说明接口是否请求成功，如果请求成功然后进行请求发送短信验证码接口。
	 */
