...
def regsecond_save(request):
	...
	if company_id:
		value=[companyname,industryCode,serviceCode,areaCode,address,addresszip,company_id]
		sql="update company set name=%s,industry_code=%s,service_code=%s,area_code=%s,address=%s,address_zip=%s where id=%s"
