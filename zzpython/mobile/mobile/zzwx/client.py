...
class Client(object):
    ...
    """
    def __init__(self, appid, appsecret,weixinid=""):
        if weixinid[0:3]=="okX":
            self.appid = "wx2891ef70c5a770d6"
            self.appsecret = "d3f9436cfc50cd9e4f62f96893a1ee0c"
        else:
            self.appid = "wxb3a1f99915ac43ed"
            self.appsecret = "6514984261ac291bfd6ef38ab150fcfb"
        self.appid = appid
    ...
    def request(self, method, url, **kwargs):
        ...
        r.raise_for_status()
        #print r.content
        jsona = json.loads(r.content)
        if check_error(jsona):
            return jsona
    ...
    def get_sign(self):
        ...
        ret = {
            ...
            'jsapi_ticket': self.get_ticket()['ticket'],
            'timestamp': self.__create_timestamp(),
            'url': 'http://pyapp.zz91.com/test.html'
        }
    ...
    @property
    def token(self):
        weixintoken=cache.get("weixintoken")
        if weixintoken:
            return weixintoken
        json = self.grant_token()
        cache.set("weixintoken",json["access_token"],7200)
    ...
    def send_text_message(self, user_id, content):
        ...
        :return: 返回的 JSON 数据包
        """
        datava=json.dumps({"touser": str(user_id),"msgtype": "text","text": {"content": content}},ensure_ascii=False,indent=2)
        #datava['text']=content
        return self.post(
            url="https://api.weixin.qq.com/cgi-bin/message/custom/send",
            #params="{\"touser\":\""+str(user_id)+"\",\"msgtype\":\"text\",\"text\":{\"content\":\""+content+"\"}}" 
            data=datava,
