	...
	favoritedel: function(id, obj) {
		...
		mui.confirm('确认要删除吗？', '提示', btnArray, function(e) {
			if (e.index == 0) {
				...
				arg += "&id=" + id;
				zzajax1("post", zzweburl+"/myrc/favoritedel.html", arg, function(data) {
					if (!isJson(data)){
...
var messages = {
	views: function(urlp, obj, isview) {
		...
		if (isview != "1" && obj.style.color == "") {
			...
			if (mainwin){
				mainwin.evalJS('messagesupdate('+messagenum.toString()+');');
...
var downloader = {
	down: function(durl, srvVer) {
		...
		//var durl = zzweburl+"/app/zz91-1.0.2.apk";
		var options = {
			method: "GET"
		};
		...
		dtask.addEventListener("statechanged", function(task, status) {
			switch (task.state) {
				...
				case 3: // 已接收到数据
					...
					if(ds){ds.innerText = parseInt(task.downloadedSize / 1024) / 1000 + "M";}
					break;
				case 4: // 下载完成
					//closeoverlay();
...
(function($) {
	$.ready(function() {
		$('body').on('tap', 'a', function(e) {
			...
			var localflag=this.getAttribute('localflag');
			if (localflag){localflag=localflag}else{localflag=0}
			if (localpath){
				gotourl(id, {'localpath':localpath,'type':'localhtml','localflag':localflag});
				return;
			}
...
function payopen(paytype) {
	...
	var weburl = zzweburl+"/qianbao/chongzhi.html?company_id=" + company_id.toString() + "&appsystem=" + appsystem + "&paytype=" + paytype + "&ttt=" + (new Date()).getTime().toString()+"&usertoken="+usertoken;
...
function showheader(wintype) {
	...
	if (wintype == "login" || wintype == "error" || wintype == "reg") {
		var appbottom=document.getElementById("appbottom")
		if (appbottom){
			appbottom.innerHTML = "";
		}
	} else {
		if (!document.getElementById("bottomcheck")){
			var appbottom=document.getElementById("appbottom")
			if (appbottom){
				appbottom.innerHTML = botttomstr;
			}
		}
	}
	if (wintype == "huzhu") {
...
function loadappbody() {
	...
	if (company_id == null || company_id.toString() == "0") {
		...
		zzajax1("get", zzweburl+"/logininfo1.html?appid=" + clientid + "&appsystem=" + appsystem + "&userimei=" + userimei + "&ttt=" + (new Date()).getTime().toString(), '', function(data) {
			if (!isJson(data)){
				var j = JSON.parse(data);
			}else{
				var j=data;
...
function regloadbody(cid) {
	...
	if (winflag == 2) {
		UserInfo.companyid(company_id.toString());
	...
	if (url.indexOf(zzweburl+"") < 0) {
		if (url.indexOf("?") < 0) {
			url = zzweburl+"/" + url + "?t=" + (new Date()).getTime().toString() + "&company_id=" + company_id.toString() + "&win=" + win.toString() + "&clientid=" + clientid + "&appsystem=" + appsystem + "&visitoncode=" + visitoncode;
		} else {
			url = zzweburl+"/" + url + "&t=" + (new Date()).getTime().toString() + "&company_id=" + company_id.toString() + "&win=" + win.toString() + "&clientid=" + clientid + "&appsystem=" + appsystem + "&visitoncode=" + visitoncode;
		}
	}
...
function reloadloginall(j,username,pwd_hash,company_id){
	//alert(username)
	UserInfo.onSuccess(j,username,pwd_hash,company_id);
}
...
function zz91login(frm) {
	...
	if (window.plus) {
		nWaiting = plus.nativeUI.showWaiting("正在登录...");
	}
	...
	var pwd_hash = UserInfo.get_pwd_hash(passwd);
			...
			if (j.err == "false") {
				...
				if (UserInfo){
					...
					if (wvs){
						for (var i = 0; i < wvs.length; i++) {
							//alert(wvs[i])
							//wvs[i].evalJS('reloadloginall("'+j+'","'+username +'","' + pwd_hash +'","' + company_id.toString() + '");');
						}
					}
					UserInfo.onSuccess(j,username,pwd_hash,company_id.toString());
				}
...
function changeaccount() {
	...
	var arg = "company_id=" + company_id;
	arg += "&clientid=" + clientid;
	mui.get(zzweburl+"/changeaccount.html?" + arg, '', function(data) {
		if (!isJson(data)){
			var j = JSON.parse(data);
		}else{
			var j=data;
...
function myFun(result) {
	...
	$.getScript('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js', function(_result) {
        if (remote_ip_info.ret == 1) {
        	...
        	if (!searchname){
        		searchname=remote_ip_info.province
        	}
...
function gotoopen(url,wintype){
	var trueurl=url;
	var localtype=0;
	var localpath;
	if (isJson(wintype)){
		localtype=wintype.localflag;
		localpath=wintype.localpath
		wintype=wintype.type;
	}
	if (wintype != "blank") {
		nowwintype = wintype;
	}
	if (wintype == "blank" || wintype == "reg" || wintype=="localhtml") {
		var extras;
		if (wintype=="localhtml"){extras={"localpath":localpath,"localhtmlflag":"1","localtype":localtype}}
		url = url.replace("&", "TandT").replace("?", "wenhao")
		mui.openWindow({
			id: url,
			url: "/blank.html?url2=" + url + "&wintype=" + wintype + "&company_id=" + company_id.toString() + "&clientid=" + clientid + "&appsystem=" + appsystem + "&visitoncode=" + visitoncode+"&usertoken="+usertoken,
			extras:extras,
			preload: false //TODO 等show，hide事件，动画都完成后放开预加载
		});
		return;
	...
	mui.get(url,{'company_id':company_id,'clientid':clientid,'appsystem':appsystem,'visitoncode':visitoncode,'usertoken':usertoken},function(data){
		if (content){
			content.innerHTML = data;
		}
		if (nWaiting) {
		...
		if (UserInfo!={}){
			mui.plusReady(function() {
				if (UserInfo.has_login()) {
					...
					if (company_id){}else{company_id="0";}
					gotoopen(url,wintype)
					return false;
				}else{
					gotoopen(url,wintype)
				}
...
function getmessagesnum(cid) {
	if (cid) {
		...
		mui.get(zzweburl+"/messagescount.html?company_id=" + cid.toString(),
			function(data) {
			if (!isJson(data)){
				var j = JSON.parse(data);
			}else{
...
function FreshTime() {
	var pendtype=document.getElementById("payend_Time")
	if (pendtype && pendtype!="0000-00-00 00:00:00"){
		var endtime = new Date(pendtype.value); //结束时间
		...
		var timediv = document.getElementById("LeftTime");
		var havenum=document.getElementById("havenum");
		if (timediv && havenum) {
			timediv.innerHTML = d + "天" + h + "小时" + m + "分" + s + "秒";
			var buybutton=document.getElementById("buybutton");
			if (lefttime <= 0 || parseInt(havenum.value)<=0) {
				timediv.innerHTML = "活动已结束";
				if (buybutton){buybutton.style.display="none";}
				clearInterval(sh);
			}else{if (buybutton){buybutton.style.display="";}}
		}
	}
...
function huanye_pay() {
	...
	var arg = "baoming='联系方式：" + contact + "联系人：" + person + "联系地址：" + address + "'&paytype=20&company_id=" + company_id.toString()
	zzajax1("get", zzweburl + "/qianbao/qianbaopay.html?" + arg, arg, function(data) {
		if (data == 'baoming') {
			dismiss();
			openfloatdiv(event, 'sucess1');
		} else if (data == '0') {
			dismiss();
			openfloatdiv(event, 'terror');
		} else if (data == 'nomoney') {
			dismiss();
			openfloatdiv(event, 'bal');
		} else if (data == 'havepay') {
			dismiss();
			openfloatdiv(event, 'havepay');
		}
	}, function(data) {
		alert("错误!请重试.");
	});
...
var retry = 3;
function qiandao(){
    var func_url = zzweburl + "/qianbao/qiandao.html";
    if (UserInfo.has_login()) {
	}else{
		gotourl("/login/", "blank");
		return;
    ...
    usertoken=UserInfo.token();
    defaultqueryString['usertoken']=usertoken;
    if (func_url){
	    	mui.ajax(func_url, {
	        data:defaultqueryString,
	        dataType:'json',
	        type:'get',
	        timeout:3000,
	        success:function(data){
	            ...
	            //plus.ui.alert("签到成功！感谢您的");
	            mui.toast('再生钱包  + 0.5元');
	        },
	        error:function(xhr,type,errorThrown){
	            retry--;
	            if(retry > 0) return qiandao();
	            retry=3;
	            mui.toast('网络错误，请重试。');
	        }
	    })
