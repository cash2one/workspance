...
from zz91tools import date_to_str,date_to_strall,filter_tags

class zz91huzhu:
    def __init__(self):
        self.db_huzhu=companydb()
    def updatedb(self,is_del,postid):
        sql='update bbs_post set is_del=%s where id=%s'
        self.db_huzhu.updatetodb(sql,[is_del,postid])
    def update_bbs_post(self,bbs_post_category_id,title,content,check_status,gmt_created,postsource,litpic,postid):
        ...
        self.db_huzhu.updatetodb(sql,[bbs_post_category_id,title,content,content_query,check_status,gmt_created,postsource,postid])
        if litpic:
    ...
    def add_bbs_post(self,company_id,account,bbs_post_category_id,title,content,check_status,gmt_created,postsource,litpic):
        content_query=filter_tags(content)
        sql='insert into bbs_post(company_id,account,bbs_post_category_id,title,content,content_query,check_status,gmt_created,postsource) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        self.db_huzhu.updatetodb(sql,[company_id,account,bbs_post_category_id,title,content,content_query,check_status,gmt_created,postsource])
        if litpic:
...
#            argument.append(ispush)
    ...
    def getbbs_post_category(self,id):
        sql='select name from bbs_post_category where id=%s'
        result=self.db_huzhu.fetchonedb(sql,[id])
        if result:
            return result[0]
    def getcategorylist(self):
        sql='select id,name from bbs_post_category order by id desc'
        resultlist=self.db_huzhu.fetchalldb(sql)
        listall=[]
        if resultlist:
            for result in resultlist:
