...
from zz91conn_read import database_huanbao_read

class huanbaodb:
    def __init__(self):
        self.conn_huanbao=database_huanbao()
        self.cursor_huanbao=self.conn_huanbao.cursor()
        self.conn_huanbao_read=database_huanbao_read()
        self.cursor_huanbao_read=self.conn_huanbao_read.cursor()
    #----更新到数据库
    ...
    def fetchalldb(self,sql,argument=''):
        if argument:
            self.cursor_huanbao_read.execute(sql,argument)
        else:
            self.cursor_huanbao_read.execute(sql)
        resultlist=self.cursor_huanbao_read.fetchall()
        if resultlist:
            return resultlist
        else:
            return []
    ...
    def fetchonedb(self,sql,argument=''):
        if argument:
            ...
            self.cursor_huanbao_read.execute(sql)
        result=self.cursor_huanbao_read.fetchone()
        if result:
            return result
    #----查询一条总数
    ...
    #----关闭连接
    def closedb(self):
        self.conn_huanbao_read.close()
        self.conn_huanbao.close()
