...
def getindexcategorylist(code,showflag):
    ...
    #设置缓存
    cache.set("zz91cp_getindexcategorylist"+str(code)+str(showflag),listall_cate,60*10)
    return listall_cate
#获得所有地区
def getarealist(code):
    ...
    for b in catelist:
        ...
        listall.append(list)
    #设置缓存
    cache.set("zz91cp_getarealist"+str(code),listall,60*10)
    return listall
...
def getproducstcategorylist(code):
    ...
    for b in catelist:
        ...
        listall.append(list)
    #设置缓存
    cache.set("zz91cp_getproducstcategorylist"+str(code),listall,60*10)
    return listall
#记录点击次数
    ...
    if res:
        if res.has_key('matches'):
            ...
            for match in itemlist:
                ...
                list={'id':pid,'title':title,'gmt_time':pdt_date,'gmt_time_m':pdt_datem,'fulltitle':attrs['ptitle'],'pdt_images':pdt_images}
    ...
    if res:
        if res.has_key('matches'):
            ...
            for match in itemlist:
                ...
                list={'id':id,'comname':comname,'business':business,'area_province':area_province,'domain_zz91':domain_zz91,'business100':business100,'url':url}
