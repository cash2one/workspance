...
def getproductsinfo(pdtid,keywords):
	...
	if productlist:
		...
		#来电宝客户
		if (productlist[0]):
			sqll="select id from crm_company_service where company_id="+str(productlist[0])+" and crm_service_code in(1007,1008) and apply_status=1"
			#cursor.execute(sqll)
		...
		,'arrcompinfo':arrcompinfo,'listmore':listmore}
	else:
		list1=None
	...
	if (list1 == None):
		...
		cache.set("zz91cp_pro"+str(pdtid),list1,60*20)
	return list1
