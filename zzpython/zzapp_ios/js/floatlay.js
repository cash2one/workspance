...
function openoverlay(url,title,html,overlayheight,obj)
{
	...
	if (title!=""){
		simpleWindown_html += "<h2 onclick=closeoverlay()><span id=otitle>"+title+"</span><a class=\"overlayclose\" href=\"javascript:closeoverlay();\">×</a></h2>";
	}else{
		simpleWindown_html += "<h2 onclick=closeoverlay()><a class=\"overlayclose\" href=\"javascript:closeoverlay();\">×</a></h2>";
	}
...
function openimglayer(html,overlayheight){
	...
	var simpleWindown_html = new String;
	simpleWindown_html="";
	simpleWindown_html +="<div id=\"TB_overlayBG1\">&nbsp;</div>";
	...
	if (!document.getElementById("TB_overlayBG1"))
	{
		$("body").append(simpleWindown_html);
	}else{
		if(html!=0){
...
{
	$("#TB_overlayBG1").css("display","none");
