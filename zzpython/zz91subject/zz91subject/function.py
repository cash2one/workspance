...
def getwarmwinter(pkind):
	sql="select ad_content,ad_target_url,DATE_FORMAT(gmt_plan_end,'%Y') as ayear,DATE_FORMAT(gmt_plan_end,'%m') as amouth,DATE_FORMAT(gmt_plan_end,'%d') as aday,id from ad where position_id="+str(pkind)+" and gmt_plan_end>='"+str(date.today()+timedelta(days=1))+"' and review_status='Y' order by sequence asc"
...
def getOrderadlist(pkind,e):
	sql="select ad_content,ad_target_url,DATE_FORMAT(gmt_plan_end,'%Y') as ayear,DATE_FORMAT(gmt_plan_end,'%m') as amouth,DATE_FORMAT(gmt_plan_end,'%d') as aday,id,ad_title,ad_description from ad where position_id="+str(pkind)+" and gmt_plan_end>='"+str(date.today()+timedelta(days=0))+"' and review_status='Y' and sequence='"+str(e)+"' order by gmt_start asc"
	cursor_ad.execute(sql)
...
def getadpostionNum(akind):
	sql="select max_ad from ad_position where id="+str(akind)+""
	cursor_ad.execute(sql)
	alist = cursor_ad.fetchone()
	if alist:
		return alist[0]
