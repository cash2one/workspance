...
#        arg_url=re.findall(urls_pat,html)
    ...
    def getpriceattrpinyin(self,label):
        ...
        if result:
            return result[0]
        return ''
    def getpricearealist(self,categoryid,parent_id,page_type='',morelist=''):
        #获得缓存
        ...
        for result in resultlist:
            ...
            list={'label':label,'pinyin':pinyin,'numb':numb,'page_type':page_type}
            listall.append(list)
        #设置缓存
    ...
    def getpricefield(self,priceid):
        catelist=cache.get("price_field"+str(priceid))
        if catelist:
            return catelist
...
#            listall.append(list)
        ...
        return listdir
    def getpricefield2(self,categoryid):
        catelist=cache.get("price_field2"+str(categoryid))
        catelist=None
        if catelist:
            return catelist
        sql='select DISTINCT field,name from price_category_field where price_category_id=%s order by id asc'
        ...
        listdir={'listname':listname,'listfield':listfield}
        if listname and listfield:
            cache.set("price_field2"+str(categoryid),listdir,60*20)
            return listdir
...
#                    company_numb=self.getpricelist_company_count(title)
        ...
        return {'list':listall_baojia,'count':listcount_baojia}
    #----最终页price_list数据
    def getdetailpricelist(self,priceid,listfield):
        #获得缓存
        ...
        if len(listfield)>1:
            ...
            for result2 in resultlist2:
                ...
                list2={'id':id,'postdate':postdate,'curl':curl,'company_numb':company_numb,'hexptitle':hexptitle,'listvalue':listvalue}
...
#                            label=attrs['plabel1']
    ...
    #                            sqlarg='id,postdate,label,label1,'+listfield2
                                sql2='select '+listfield2+' from price_list where id=%s'
    ...
    #                                curl='/chart/'+str(id)+'.html'
        ...
        return {'list':listall,'count':count}
#----走势图数据
    def getchartpricelist(self,keywords='',frompageCount='',limitNum='',timedate='',gmt_begin='',gmt_end='',area='',group='',attrbute='',maxcount=500,categoryid=""):
...
#                            listchart.append(list)
                            ...
                            if price.isdigit()==True and price1.isdigit()==True:
                                ...
                                aveprice=(lowprice+heiprice)/2
    #                            list['aveprice']=aveprice
                        list={'postdate2':postdate,'aveprice':aveprice}
                        listchart.append(list)
                count=res['total_found']
        return listchart
