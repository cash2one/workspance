...
def get_new(title,typeid,typeid2):
#    time3d=time1-3600*24*3
#    time3d=str(time3d)
    conn=database()
    cursor=conn.cursor()
    sql='select title from dede_archives where title=%s and typeid=%s and typeid2=%s'
    cursor.execute(sql,[title,typeid,typeid2])
    resultlist=cursor.fetchone()
    close(conn,cursor)
    return resultlist
