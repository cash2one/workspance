...
#!抓取标题后去掉空格
        ...
        if arg_title==0:
            ...
            if main_url=='http://www.chinascrap.com/':
                title=a.get_text()[1:]
            result=get_new(title,typeid,typeid2)
            if not result:
    ...
    #                      'http://www.xkxm.com/AspCodeHtml/News/':len(a)<48
                ...
                if son_urls[main_url]:#根据长度赛选需要的子url
                    ...
                    try:
                        ...
                        txt='invalid link' + a + str(typeid)+time3
    ...
    #                            content=content+content2
                        ...
                        txt="content error "  + a + 'typeid: ' + str(typeid)+time3
                        f=open('print/'+ time2 +'-out.txt','ab')
                        print >>f,txt
                        f.close()
                    try:
                        ...
                        txt="hand content error "  + a + time3
                        f=open('print/'+ time2 +'-out.txt','ab')
                        print >>f,txt
                        f.close()
                    if not result:
                        try:
                            ...
                            txt="pass " + a + 'typeid: ' + str(typeid)+time3
                            f=open('print/'+ time2 +'-out.txt','ab')
                            print >>f,txt
                            f.close()