	...
	if (comtype == 1):
		...
		if (newcode == None):
			...
			if(PPTGrade == '0'):
				...
				return 1
def getcom_namecommon(com_id):
	sql="select com_subname,com_fqr,com_name from comp_info where com_id="+str(com_id)
	cursor1.execute(sql)
	newcode=cursor1.fetchone()
	return newcode
def getcom_subname(com_id):
	...
	if (newcode == None):
		...
		return newcode[0]
def getcom_fqr(com_id):
	sql="select com_fqr from comp_info where com_id="+str(com_id)
	cursor1.execute(sql)
	newcode=cursor1.fetchone()
	if (newcode == None):
		return ''
...
def getpdt_price(pdt_id):
	sql="select pdt_price,pdt_price_min,pdt_price_max,pdt_unit from products_customerPrice where pdt_id="+str(pdt_id)+" and not exists(select pdt_id from products_notShowPrice where pdt_id=products_customerPrice.pdt_id)"
	cursor1.execute(sql)
	newcode=cursor1.fetchone()
	if (newcode == None):
		return ''
...
def geclsvaluelist(oldidstr):
	...
	if (notpdtid4!='0'):
		notpdtid+=notpdtid4
	if (notpdtid1==changezhongwen('废旧轮胎与废橡胶')):
		notpdtid=notpdtid.replace(changezhongwen('废旧轮胎与废橡胶'),changezhongwen('废橡胶'))
	if (notpdtid2==changezhongwen('废轮胎')):
		notpdtid=notpdtid.replace(changezhongwen('废橡胶'),changezhongwen('废旧轮胎与废橡胶'))
	
	if (notpdtid1==changezhongwen('废纺织品与废皮革')):
		notpdtid=notpdtid.replace(changezhongwen('废纺织品与废皮革'),changezhongwen('废纺织品'))
	if (notpdtid2==changezhongwen('皮革')):
		notpdtid=notpdtid.replace(changezhongwen('废纺织品'),changezhongwen('废皮革'))
	return notpdtid
...
#是否暂停发布
def getstoppost(id):
	sql="select pdt_id from products_stop where pdt_id="+str(id)+""
	cursor1.execute(sql)
	oldidstr=cursor1.fetchone()
	if (oldidstr == None):
		return '0'
...
def getcompinfo(pdtid,cursort):
	...
	if productlist:
		...
		if (pdt_images == '' or pdt_images == '0'):
			...
			pdt_images='http://image01.zz91.com/images.php?picurl=http://img.zz91.com/upimages/'+str(pdt_images)+'&width=100&height=75'
			pdt_imagesflag=1
...
sql = "select com_id,0,pdt_id,pdt_kind,pdt_name,pdt_com_province,pdt_detail,pdt_time_en,pdt_check,pdt_keywords,convert(bigint,gmt_editdate),pdt_price"
sql +=" from products where convert(bigint,gmt_editdate)>(select updatetime from import_table where tablename='"+updatetablename+"') order by gmt_editdate asc"
#print sql
cursor1.execute(sql)
results = cursor1.fetchmany(10000)

#sql="update import_table set updatetime=(select convert(bigint,max(gmt_editdate)) from products) where tablename='"+updatetablename+"'"
#cursor1.execute(sql)
#conn1.commit()
...
for a in results:
	...
	namecommon=getcom_namecommon(com_id)
	if (namecommon!=None):
		com_subname=namecommon[0]
		if (com_subname==None):
			com_subname=''
		com_fqr=namecommon[1]
		if (com_fqr==None):
			com_fqr=''
		com_name=changezhongwen(namecommon[2])
		if (com_name==None):
			com_name=''
	...
	value=[com_id,com_name,pdt_id,pdt_kind,pdt_name,com_province,pdt_detail,pdt_time_en,pdt_check,com_subname,viptype,pdt_images,pdt_imagesflag,pdt_keywords,pdt_price,pdt_price_min,pdt_price_max,pdt_unit,clslist,searchkeywords,offerstaus,country]
	valueupdate=[com_id,com_name,pdt_kind,pdt_name,com_province,pdt_detail,pdt_time_en,pdt_check,com_subname,viptype,pdt_images,pdt_imagesflag,pdt_keywords,pdt_price,pdt_price_min,pdt_price_max,pdt_unit,clslist,searchkeywords,offerstaus,country]
	...
	oldidcount=oldid[0]
	if (oldidcount>0):
		sql="update products set com_id=%s,com_name=%s,pdt_kind=%s,pdt_name=%s,com_province=%s,pdt_detail=%s,pdt_time_en=%s,pdt_check=%s,com_subname=%s,viptype=%s,pdt_images=%s,pdt_imagesflag=%s,pdt_keywords=%s,pdt_price=%s,pdt_price_min=%s,pdt_price_max=%s,pdt_unit=%s,clslist=%s,searchkeywords=%s,offerstaus=%s,country=%s where pdt_id="+str(pdt_id)
		...
		sql="insert into products(com_id,com_name,pdt_id,pdt_kind,pdt_name,com_province,pdt_detail,pdt_time_en,pdt_check,com_subname,viptype,pdt_images,pdt_imagesflag,pdt_keywords,pdt_price,pdt_price_min,pdt_price_max,pdt_unit,clslist,searchkeywords,offerstaus,country)"
...
try:
	#读取最新更新时间
	if (editdate==0):
		sql="select convert(bigint,max(gmt_editdate)) from products"
		cursor1.execute(sql)
		oldid=cursor1.fetchone()
		editdate=oldid[0]
	print str(editdate)
	updatelastid(updatetablename+"date",str(editdate))
	sql="update import_table set updatetime="+str(editdate)+" where tablename='"+updatetablename+"'"
	cursor1.execute(sql)
	conn1.commit()
	...
	print changezhongwen("更新成功"+str(gmt_created)+"共更新了"+str(i)+"条信息")
	conn1.close()
	conn2.close()
	connt.close()
except Exception, e:
