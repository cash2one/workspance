...
def country():
	...
	for row in results:
		...
		sql="update company_account set company_id="+str(id2)+" where gmt_created='"+gmt_created+"' and gmt_modified='"+gmt_modified+"' and gmt_register='"+gmt_register+"' and (company_id=0 or company_id=1 or company_id is null) "
		cursor_to.execute(sql)
		print id2
	#__close()
country()
