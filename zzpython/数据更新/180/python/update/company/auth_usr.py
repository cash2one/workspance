...
#sql = "select Com_UserName,Com_PWsafe,Com_Email,com_id,convert(bigint,gmt_editdate) from comp_loading where convert(bigint,gmt_editdate)>(select updatetime from import_table where tablename='"+updatetablename+"')  order by gmt_editdate asc"
sql="select Com_UserName,Com_PWsafe,Com_Email,com_id,convert(bigint,gmt_editdate) from comp_loading where exists(select com_id from comp_info where com_regtime>'2011-11-16' and com_id=comp_loading.com_id)"
cursor1.execute(sql)
