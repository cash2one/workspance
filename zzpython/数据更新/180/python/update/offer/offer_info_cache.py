...
updatetablename="offer_info_cache"
execfile("/usr/tools/inc.py") 
def _key_to_file(key):
	#key1=str(key)[0:len(key)-2]
...
sql = "select pdt_id"
sql +=" from products where pdt_time_en>=dateadd(d,-3,getdate()) order by pdt_id asc"
cursor1.execute(sql)
...
for a in results:
	...
	print changezhongwen(str(pdt_id)+"成功"+str(i))
