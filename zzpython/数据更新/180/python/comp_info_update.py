...
updatetablename="comp_info_update"
...
def getprovincecode(strvalue):
	...
	sql11="select code from category where label like '%"+str(area1)+"%'"
...
def getclassified_code(com_id):
	sql="select comType from comp_comType where com_id="+str(com_id)+""
	cursor1.execute(sql)
	newcode=cursor1.fetchone()
	if (newcode == None):
		return '0'
...
sql = "select top 100000 com_id,com_keywords"
...
for a in results:
	...
	#com_productslist_en=changezhongwen(a[1])
	#sales_buy=getsalesandbuy(com_id)
	value=[industry_name,garden_name]
	valueupdate=[industry_name,garden_name]
	...
	if (oldidcount>0):
		sql="update company set industry_name=%s,garden_name=%s where com_id="+str(com_id)
