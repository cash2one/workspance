...
updatetablename="offer_info_re1day"
...
sql +=" from products where pdt_time_en>=dateadd(d,-1,getdate()) order by gmt_editdate asc"
cursor1.execute(sql)
...
for a in results:
	...
	if (oldidcount>0):
		...
		getcompinfo(pdt_id,cursort)
		updateflag="新增"
	print changezhongwen(updateflag+"成功"+str(pdt_time_en))
	updatelastid(updatetablename,pdt_id)
	editdate=str(a[10])
	i+=1
