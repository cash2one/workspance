...
#sql +=" from products where convert(bigint,gmt_editdate)>'"+maxupdate+"' and pdt_time_en>=dateadd(d,-1,getdate()) order by pdt_time_en desc"
sql="select com_id,com_name,pdt_id,pdt_kind,pdt_name,com_province,pdt_detail,pdt_time_en,pdt_check,com_subname,pdt_keywords,pdt_images,pdt_price,pdt_unit from temp_productsUpdate"
cursor1.execute(sql)
results = cursor1.fetchmany(10000)

sql="update p set gmt_updatetime=(select convert(bigint,max(gmt_editdate)) from products_temp) where id=1"
cursor1.execute(sql)
conn1.commit()
...
for a in results:
	...
	pdt_check=str(a[8])
	com_subname=str(a[9])
	pdt_keywords=changezhongwen(a[10])
...
try:
	...
	print changezhongwen("更新成功"+str(i)+"条"+str(gmt_created))
	conn1.close()
	conn2.close()
	connt.close()
except Exception, e:
