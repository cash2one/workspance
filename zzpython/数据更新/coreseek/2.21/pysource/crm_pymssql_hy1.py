...
class mainSource(object):
	...
	def Connected(self):   #如果是数据库，则在此处做数据库连接
		if self.conn==None:	   
			...
			sql = "SELECT  com_id,com_email,com_name,com_tel,com_province,com_mobile,vipflag,com_rank,com_regtime,lastlogintime,teldate,contacttype,com_ctr_id,com_zip,com_contactperson,com_desi,com_add,com_productslist_en FROM temp_salescomp where teldate>'2011-10-1' and com_rank>=3 and not exists (select com_id from comp_zstinfo where com_id=temp_salescomp.com_id) and not exists (select com_id from crm_assign where com_id=temp_salescomp.com_id)"
