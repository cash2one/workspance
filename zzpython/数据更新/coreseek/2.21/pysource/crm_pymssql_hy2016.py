...
class mainSource(object):
	...
	def Connected(self):   #如果是数据库，则在此处做数据库连接
		if self.conn==None:	   
			...
			sql = "SELECT  id,com_email,newemail,membertype,com_id,cname,comkeywords,js1,js2,sl1,sl2,province,cproductslist,ccontactp,ctel,cmobile,cadd,pcheck,huangye_qukan,qt1,qt2 FROM huangye_list where huangye_qukan='2016' and pcheck=1"
	...
	def NextDocument(self):   #取得每一个文档记录的调用
		if self.idx < len(self.data):
			...
			if (item[19]):
				self.qt1 = item[19].decode('GB18030','ignore').encode('utf-8')
			else:
				self.qt1 = ''
			if (item[20]):
				self.qt2 = item[20].decode('GB18030','ignore').encode('utf-8')
			else:
				self.qt2 = ''
