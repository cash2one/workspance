...
from news_artical.items import benwang
import sys
reload(sys)
...
class dmozSpider(CrawlSpider):
    ...
    start_urls=["http://news.steelcn.cn/yuanchuang.html",]
    ...
    def parse(self, response):
        ...
        titleurllist=sel.xpath("//*[@class='list']/ul/li/a/@href").extract()
        title_zh=sel.xpath("//*[@class='list']/ul/li/a/text()").extract()
        for n in xrange(0,len(titleurllist)):
            ...
            #判断是否是今天的news
                ...
                if '中钢网' not in title_zh[n].decode('utf-8') and '国际要闻概览' not in title_zh[n].decode('utf-8') and '钢铁消费行业摘要' not in title_zh[n].decode('utf-8') and '宏观经济新闻摘要' not in title_zh[n].decode('utf-8') and '闻早报' not in title_zh[n].decode('utf-8'):
                    #判断是否有上述关键字
