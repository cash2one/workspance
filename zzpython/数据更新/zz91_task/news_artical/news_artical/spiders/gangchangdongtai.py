...
from news_artical.items import gangchangdongtai
import sys
reload(sys)
sys.setdefaultencoding('utf8')
from function import savedb,getimglist
class dmozSpider(CrawlSpider):
    name = "gangchangdongtai"
    download_delay = 1
    allowed_domains = ["prcsteel.com"]
    start_urls=["http://news.prcsteel.com/gangchangxinxi/gangchangdongtai.html",]
    ...
    def into(self,url,pubdate):
        ...
        title=''
        artical=''
        pattern_all=re.compile(r'<div class="detail_con">(.*?)<div style="clear:both;width:100%;">',re.S)
        res_all=re.findall(pattern_all,html)
        if res_all:
            pattern_title=re.compile(r'<div class="detail_tit">(.*?)<div class="detail_tit_info">',re.S)
            ...
            pattern_artical=re.compile(r'<div id="newsContent">(.*?)</div>',re.S)
    ...
    def parse(self, response):
        ...
        #current_date='2015-08-13'
        pubdatelist=sel.xpath("//*[@class='newslist_con2']/li/span/text()").extract()
        titllist=sel.xpath("//*[@class='newslist_con2']/li/a/@href").extract()
