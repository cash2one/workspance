...
from news_artical.items import zgly1
import sys
reload(sys)
...
class dmozSpider(CrawlSpider):
    name = "zgly1"
    download_delay = 1
    allowed_domains = ["alu.cn"]
    start_urls=["http://www.alu.cn/aluNews/NewsList_2.html",]
    ...
    def into(self,url,pubdate):
        ...
        title=''
        artical=''
        pattern_all=re.compile(r'<div class="newsinfo">(.*?)<div class="shangxia btline">',re.S)
        res_all=re.findall(pattern_all,html)
        if res_all:
        
            pattern_title=re.compile(r'<h2 class="red">(.*?)</h2>',re.S)
            res_title=re.findall(pattern_title,res_all[0])
            if res_title:
                title=re.sub("<[^>]*?>","",res_title[0]).strip()
                #print title
                
            pattern_artical=re.compile(r'</span></strong></a></strong>(.*?)</p></div>',re.S)
    ...
    def parse(self, response):
        ...
        #current_date='(15/08/24)'
        pubdatelist=sel.xpath("//*[@class='hot_news_list']/ul/li/em/text()").extract()
        titllist=sel.xpath("//*[@class='hot_news_list']/ul/li/a/@href").extract()
        ...
        for pubdate in pubdatelist:
            ...
            if pubdate>=current_date:
                ...
                titleurl='http://www.alu.cn/aluNews/'+titleurl
                print a
                print 'ook' 
                print pubdate
                print titleurl
