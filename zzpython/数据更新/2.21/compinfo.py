...
try:
	...
	sql = sql +" from company as a inner join company_account as c on c.company_id=a.id where DATEDIFF(CURDATE(),a.gmt_modified)<=10  and DATEDIFF(CURDATE(),a.gmt_modified)>=0 and regfrom_code!='10031040' order by a.id asc"
	print sql
	cursorserver.execute(sql)
	...
	for a in results:
		...
		if (oldidcount>0):
			...
			print sql
			print com_id
			cursor.execute(sql,(com_id,com_subname, com_name,com_add,com_zip, com_province, com_ctr_id, com_tel, com_mobile, com_fax, com_email, com_email_back, com_website,com_contactperson,com_desi,com_station,str(com_intro), com_productslist_en, com_regtime,num_login,gmt_last_login));
