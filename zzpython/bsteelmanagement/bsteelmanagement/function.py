...
def getarticallist(frompageCount,limitNum,isdelete='',typeid='',stitle='',sort='',begintime='',endtime=''):
    seartext=''
    if typeid and typeid!='':
        seartext=seartext+' and typeid='+typeid
    if stitle and stitle!='':
        seartext=seartext+' and title like "%'+stitle+'%"'
    if isdelete:
        seartext=seartext+' and isdelete='+isdelete
    if begintime and begintime!='':
        seartext=seartext+' and gmt_created>="'+begintime+'"'
    if endtime and endtime!='':
        seartext=seartext+' and gmt_created<="'+endtime+'"'
    
    sql1='select count(0) from bsteel_artical where id>0'
    sql1+=seartext
    count=fetchnumberdb(sql1+seartext)
