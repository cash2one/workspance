...
def mynavlist(request):
    ...
    mycolumn=zzn.getnewscolumn(mid=mid,deviceId=deviceId)
...
def show_feedback(request):
    ...
    after_range_num = funpage.after_range_num(5)
    before_range_num = funpage.before_range_num(4)
    newslist=zzn.get_feedback(aid=aid,mid=mid,frompageCount=frompageCount,limitNum=limitNum,allnum='')
    listcount=0
    if (newslist):
        listall=newslist['list']
        listcount=newslist['count']
...
def insert_dede_feedback(request):
    ...
    username=""
    if mid:
        username=zzn.getmemberinfo(mid)['uname']
    if not mid:
        mid=0
