...
def getcompinfo(pdtid,keywords,company_id=''):
    ...
    if (productsinfo==None):
        ...
        if productlist:
            ...
            #---接听率
            sql1="select phone_rate,level from ldb_level where company_id=%s and is_date=0"
...
def getcompany_id(cname,regtime):
    sql="select id from company where gmt_created=%s limit 0,1"
    newcode=dbc.fetchonedbmain(sql,[regtime])
    if (newcode == None):
        return '0'
    else:
        return newcode[0]
