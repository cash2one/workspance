...
//		cacheClient.delete(CacheConst.CACHE_PARAM_IS_OLD);
	...
	@Override
	public String getValueBylogisticsCompany(String logisticsCompany,Integer index){
		String val = "";
		if(StringUtils.isEmpty(logisticsCompany) || index==null){
			return val;
		}
		do {
			String s=paramDao.queryParamByKey(logisticsCompany).getValue();
			String[] s1 = s.split(",");
			val=s1[index];
		} while (false);
		return val;
