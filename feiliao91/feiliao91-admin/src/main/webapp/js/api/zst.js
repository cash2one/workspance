Ext.namespace("ast.ast1949.api.zst");

ast.ast1949.api.zst.SvrApplyDetailForm=Ext.extend(Ext.form.FormPanel,{
...
//				}
			...
			buttons:[{
				...
				handler:function(btn){
					...
					var form=this;
					Ext.MessageBox.confirm(Context.MSG_TITLE, "再生通开通服务关系会员的各种功能使用，请务必谨慎操作！<br />现在您确定信息都填写无误，"+info+"开通再生通服务吗？", function(btn){
						if(btn != "yes"){
							return false;
						}
						form.getForm().submit({
							url : Context.ROOT+Context.PATH+"/api/zst/doOpen.htm",
							method : "post",
		...
		ast.ast1949.api.zst.SvrApplyDetailForm.superclass.constructor.call(this,c);
	},
	loadOpenDetail:function(data){
		...
		this.findById("zstNoExist").setValue(data["zstNoExist"]);
...
ast.ast1949.api.zst.SvrApplyDetailEditorForm=Ext.extend(Ext.form.FormPanel,{
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
