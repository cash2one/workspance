Ext.namespace("ast.ast1949.crm.cs");

ast.ast1949.crm.cs.CSMAP={};

ast.ast1949.crm.cs.Fields=[
	...
	{name:"pv", mapping:"pv"},
	{name:"crmname", mapping:"crmname"}
];

ast.ast1949.crm.cs.ACCOUNTFIELD=[
...
ast.ast1949.crm.cs.REFRESHPRODUCTSFIELD = [
     {name : "id",mapping : "id"},
     {name : "companyId" , mapping : "companyId"},
     {name : "refreshDate" , mapping :  "refreshDate"},
     {name : "refreshRate" , mapping : "refreshRate"}
 ];
ast.ast1949.crm.cs.Grid = Ext.extend(Ext.grid.GridPanel,{
	queryUrl:Context.ROOT +Context.PATH+  "/crm/cs/queryCoreCompany.htm",
	constructor:function(config){
		...
		var _store = new Ext.data.JsonStore({
			...
			remoteSort:true,
			fields:ast.ast1949.crm.cs.Fields,
			url:_url,
			autoLoad:false
		});
		...
		var _cm=new Ext.grid.ColumnModel( [_sm,{
			...
			renderer:function(value, metadata, record, rowIndex, colIndex, store){
				if(value!=null && value.length>0){
					if(typeof ast.ast1949.crm.cs.CSMAP[value] != "undefined"){
						return ast.ast1949.crm.cs.CSMAP[value];
					}else{
						return value;
					}
				}else{
...
//					url: "http://192.168.2.21/getusername/?com_id="+record.get("company_id"),
//					params:{"com_id":record.get("company_id")},
//					method: 'GET',
//					data:'json',
//					success: function (response) {
//						var jsonData = eval('('+response.responseText+')');
//						record.set("name",jsonData.name);
//		            }
//		        });
//				return record.get("name");
				return "<a href='http://192.168.2.21/getusername/?com_id="+record.get("company_id")+"' target='_blank'>查看</a>";
				
//				return record.get("crmname");
			...
			tbar:[{
				...
				menu:[{
					...
					handler:function(btn){
						...
						ast.ast1949.crm.cs.updateCompanyAccount(rows.get("company_id"));
					}
				},{
					text:"开通400",
					iconCls:"add",
					...
					handler:function(btn){
						...
						Ext.MessageBox.confirm(Context.MSG_TITLE,"确定解绑 '"+" "+rows.get("companyName")+"' 的微信?",function(btn){
						if(btn!="yes"){
							return ;
						}else{
							Ext.Ajax.request({
								url:Context.ROOT+Context.PATH+ "/crm/cs/deleteWeiXin.htm",
								params:{
									"cid":companyId
								},
								method:"post",
								...
								success:function(response,opt){
								   ...
								   if(obj.success){
								      ast.ast1949.utils.Msg(obj.data,"解绑已成功");
								   }else{
								   	  ast.ast1949.utils.Msg(obj.data,"解绑失败");
								   }
								}
							});
						}
					...
					handler : function (btn) {
						var rows=grid.getSelectionModel().getSelected();
						ast.ast1949.crm.cs.productsAutoRefresh(rows.get("company_id"));
					}
				}
				]
			},{
				...
				handler:function(btn){
					...
					ast.ast1949.crm.cs.sendSMS(ids)
				}
			}
//			,{
//				text:"放入抽奖箱",
			...
			,{
				...
				listeners:{
					"blur":function(field){
						...
						if(B["outbusiness"]==field.getValue()){
							return false;
						}
						B["outbusiness"]=field.getValue();
				...
				store:new Ext.data.JsonStore( {
					fields : [ "account", "name" ],
					autoLoad:true,
					url : Context.ROOT + Context.PATH+ "/crm/cs/queryCs.htm"
				}),
				listeners:{
					"blur":function(field){
						...
						var B=_store.baseParams;
						if(B["csAccount"]==field.getValue()){
							return false;
						}
						B["csAccount"]=field.getValue();
						_store.reload({params:{"startIndex":0, "pageSize":Context.PAGE_SIZE}});
					}
				}
			},'-',{
...
//			}
		...
		ast.ast1949.crm.cs.Grid.superclass.constructor.call(this,c);
	},
	loadDefault:null,
	contextmenu:null
});
 ...
 * 重新分配客户信息
 * */
ast.ast1949.crm.cs.reassign=function(grid, oldassignArr, cs, csname){
 ...
 * 放置废品池
 * */
ast.ast1949.crm.cs.updateOutbusiness=function(grid, companyId,type){
 ...
 * 从公海捞客户
 * */
ast.ast1949.crm.cs.reassignByHighSea=function(grid, oldassignArr, cs, csname){
	//一次可以分配多个客户给一个用户，需要最后确认，如确定要将x位客户分配给某某某吗
	if(oldassignArr.length<=0){
		return false;
	}
 ...
 * 丢公海
 * */
ast.ast1949.crm.cs.dropUser=function(grid, ids){
	//一次可以丢一组用户，但需要最后确认
 ...
 * 发送短信
 * */
ast.ast1949.crm.cs.sendSMS=function(ids){
	//一次可以丢一组用户，但需要最后确认
	Ext.MessageBox.confirm(Context.MSG_TITLE, "您确定要将选中的"+ids.length+"个客户发送短信吗？<br />", function(btn){
        ...
        var form = new ast.ast1949.crm.cs.SMSForm({
    		id:"SMS_FORM",
    		region:"center"
    	});
...
ast.ast1949.crm.cs.SMSForm=Ext.extend(Ext.form.FormPanel, {
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
...
//						disabled:"true",
		...
		ast.ast1949.crm.cs.SMSForm.superclass.constructor.call(this,c);
	},
	mystore:null,
 ...
 * 显示客户信息，在新页面打开
 * */
ast.ast1949.crm.cs.showInfo=function(rows){
	if(rows.length>1){
		//询问是否要一次打开全部客户信息
...
ast.ast1949.crm.cs.SearchForm = Ext.extend(Ext.form.FormPanel,{
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
...
//					B["csAccount"]=form.findById("cs").getValue();
		...
		ast.ast1949.crm.cs.SearchForm.superclass.constructor.call(this,c);
	},
	initForm:null
});

//用户名信息修改
ast.ast1949.crm.cs.updateCompanyAccount=function(companyId){
	var form=new ast.ast1949.crm.cs.UpdateCompanyAccountForm({
	});
...
ast.ast1949.crm.cs.UpdateCompanyAccountForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
			...
			buttons:[{
				...
				handler:function(btn){
					if(form.getForm().isValid()){
						var grid = Ext.getCmp(ast.ast1949.crm.cs.ACCOUNTFIELD);
						form.getForm().submit({
		...
		ast.ast1949.crm.cs.UpdateCompanyAccountForm.superclass.constructor.call(this,c);
	},
	loadCompanyAccount:function(companyId){
		...
		form.store = new Ext.data.JsonStore({
			fields : ast.ast1949.crm.cs.ACCOUNTFIELD,
...
//自动刷新供求设置
ast.ast1949.crm.cs.productsAutoRefresh=function(companyId){
	var form=new ast.ast1949.crm.cs.productsAutoRefreshForm({
	});
...
ast.ast1949.crm.cs.productsAutoRefreshForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		var c = {
				...
				buttons : [{
					...
					handler : function (btn) {
						if(form.getForm().isValid()){
							var grid = Ext.getCmp(ast.ast1949.crm.cs.REFRESHPRODUCTSFIELD);
							form.getForm().submit({
		...
		ast.ast1949.crm.cs.productsAutoRefreshForm.superclass.constructor.call(this,c);
	},
	loadRefreshDate:function(companyId){
		...
		form.store = new Ext.data.JsonStore({
			fields : ast.ast1949.crm.cs.REFRESHPRODUCTSFIELD,
