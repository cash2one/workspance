Ext.namespace("ast.ast1949.phoneAllBs");

ast.ast1949.phoneAllBs.INFOFIELD=[
...
ast.ast1949.phoneAllBs.PHONEINFOFIELD=[
	{name:"id",mapping:"id"},
...
ast.ast1949.phoneAllBs.Grid = Ext.extend(Ext.grid.GridPanel,{
	constructor:function(config){
		...
		var storeUrl = Context.ROOT + Context.PATH + "/phone/queryAllPhoneBs.htm";
		
		var _store= new Ext.data.JsonStore({
			root:"records",
			totalProperty: 'totalRecords',
			remoteSort:true,
			fields:ast.ast1949.phoneAllBs.INFOFIELD,
			url: storeUrl,
			autoLoad:true
		});
		var tbar = [
...
//			iconCls:"add",
//			handler:function(){
//				ast.ast1949.phoneAllBs.updatePhoneInfo(0);
//			}
//		}
		{
			text:"写小计",
			iconCls:"add",
			handler:function(){
//				ast.ast1949.phoneAllBs.addCRMLog(row.get("id"));
		...
		,"-",{
			...
			handler:function(){
				var row=grid.getSelectionModel().getSelected();
				ast.ast1949.phoneAllBs.updatePhoneInfo(row.get("id"))
			}
		},"-",{
			text:"询盘群发",
			iconCls:"copy",
		...
		ast.ast1949.phoneAllBs.Grid.superclass.constructor.call(this,c);
	},
	loadDefault:null,
	contextmenu:null,
 ...
 * 重新分配客户信息
 * */
ast.ast1949.phoneAllBs.reassign=function(grid, oldassignArr, cs, csname){
...
//账户信息修改
ast.ast1949.phoneAllBs.updatePhoneInfo=function(id,companyId){
	var form=new ast.ast1949.phoneAllBs.UpdatePhoneInfoForm({
	});
...
ast.ast1949.phoneAllBs.UpdatePhoneInfoForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
		...
		ast.ast1949.phoneAllBs.UpdatePhoneInfoForm.superclass.constructor.call(this,c);
	},
	loadInit:function(id,companyId){
		...
		form.store = new Ext.data.JsonStore({
			fields : ast.ast1949.phoneAllBs.PHONEINFOFIELD,
...
ast.ast1949.phoneAllBs.SearchForm = Ext.extend(Ext.form.FormPanel,{
	targetGrid:null,
	constructor:function(config){
		config = config||{};
		Ext.apply(this,config);
			...
			items:[{
				...
				,{
					...
					listeners:{
						"change":function(field,newvalue,oldvalue){
							if(newvalue==""){
								B["monthFeeFrom"] = undefined;
							}else{
								B["monthFeeFrom"] = newvalue;
							}
							_store.baseParams = B;
						}
					}
					...
					listeners:{
						"change":function(field,newvalue,oldvalue){
							if(newvalue==""){
								B["monthFeeTo"] = undefined;
							}else{
								B["monthFeeTo"] = newvalue;
							}
							_store.baseParams = B;
						}
					}
		...
		ast.ast1949.phoneAllBs.SearchForm.superclass.constructor.call(this,c);
