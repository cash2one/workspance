Ext.namespace("ast.ast1949.admin.bbs.category");

Ext.onReady(function(){
	var _url = Context.ROOT + "/bbs/bbscategory/edit.htm";
	
	var _editPanel = ast.ast1949.admin.bbs.category.editPanel({
	...
	var _left = new Ext.Panel({
		title:  '模块信息<span style="color:red">【点击右键更多操作】</span>',
		region: 'west',
		collapsible:true,
		margins:  "0 0 2 2",
		cmargins: "0 2 2 2",
		...
		minSize:120,
		autoScroll:true,
		items:[{html:"<div id='newsmodule-tree'></div>"}]
	});
	...
	var _tree = ast.ast1949.admin.bbs.category.treePanel({el:"newsmodule-tree",rootData:""});
    ...
    function doDelete(_btn){
		...
		conn.request({
			url: Context.ROOT + Context.PATH + "/bbs/bbscategory/delete.htm?id="+_id,
			method : "get",
			scope : this,
  ...
  //更新
    ...
    function resumeForm(f){
		...
		f.findById("url").setValue("");
		f.findById("checked").setValue("");
