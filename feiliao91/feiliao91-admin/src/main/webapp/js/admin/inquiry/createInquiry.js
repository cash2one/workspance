...
//				dataIndex : "senderId"
	...
	initProducts:function(obj){
		...
		this.findById("productsTypeCode").setValue(obj.productsTypeCode);
	}
});
...
//					grid = Ext.getCmp(CREATEINQUIRY.PRODUCT_GRID);
					...
					if(Ext.get("search-products-title").dom.value==""){
						...
						B["title"]=Ext.get("search-products-title").dom.value;
					}
				...
				handler:function(btn){
					...
					if(form.getForm().isValid()){
						...
						if(_ids.length>0) {
							var _url=Context.ROOT + Context.PATH + "/admin/inquiry/createInquiryForProductsByInquiry.htm?productId="+Ext.getCmp("id").getValue()+"&productsArray="+_ids;
							form.getForm().submit({
								url:_url,
								method:"post",
								success:function(){
