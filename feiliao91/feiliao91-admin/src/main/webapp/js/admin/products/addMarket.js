...
ast.ast1949.admin.products.market.productsForm = Ext.extend(Ext.form.FormPanel,{
					...
					listeners:{
							"blur":function(field){
								if(Ext.get(field.getId()).dom.value!=""){
									Ext.getCmp("industry").setValue(field.getRawValue()); 
								}else{
					...
					listeners:{
						...
						"blur":function(field){
							if(Ext.get("areaCode").dom.value!=""){
								Ext.getCmp("areaCode").setValue(field.getRawValue()); 
								
							}else{
								Ext.getCmp("areaCode").setValue(); 
							}
    ...
    saveUrl:Context.ROOT+Context.PATH+"/admin/products/insertMarket.htm",
