...
ast.ast1949.admin.charts.category.InfoFormPanel = Ext.extend(Ext.Panel,{
	_form:null,
	constructor:function(_cfg){
		...
		var typeCombo = ast.ast1949.admin.price.priceCategoryComboTree({
			fieldLabel	: "关联报价",
			id			: "combo-relevanceId",
			name		: "combo-relevanceId",
			hiddenName	: "relevanceId",
			hiddenId	: "relevanceId",
			emptyText	: "请选择...",
			readOnly	:true,
			allowBlank	:true,
		//	itemCls 	:"required",
			width		:"100",
			el:"relevanceId-combo",
		...
		var tree;
      	parentCombo.on("expand",function(){
      		if(tree==null){
	      		tree =  ast.ast1949.admin.charts.category.treePanel({el:"category-combo",rootData:""});
		...
		ast.ast1949.admin.charts.category.InfoFormPanel.superclass.constructor.call(this,{
			title:_title,
			closeable:true,
			width:700,
			autoHeight:true,
