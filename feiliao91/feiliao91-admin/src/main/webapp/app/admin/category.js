...
Ext.define("CategoryFormModel",{
	...
	proxy:{
		type:"ajax",
		api:{
			read:Context.ROOT+"/admin/category/queryOne.htm"
		}
	}
});
...
//					
				...
				handler:function(btn,e){
					...
					var form=Ext.create("ast.ast1949.admin.category.MainForm",{
						saveUrl:Context.ROOT+"/admin/category/update.htm",
						region:"center",
						treeNode:this.getTreeNode()
					});
//					
				...
				handler:function(btn,e){
					...
					Ext.Msg.confirm(Context.MSG_TITLE,"类别删除后将无法恢复（包括子类别），您确定要删除这些类别吗？",function(o){
						...
						Ext.Ajax.request({
							url: Context.ROOT+"/admin/category/delete.htm",
...
//			}
	...
	saveUrl:Context.ROOT+"/admin/category/create.htm",
	config:{
		saveUrl:null,
		treeNode:null
	}
