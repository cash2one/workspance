...
//		Integer count = Integer.valueOf(object.getString("totals"));
		...
		do{
			...
			for (int i = 0; i < n; i++) {
				//插入
				insertData(targetDate, start, limit);
				start+=limit;
			}
	...
	private int insertData(String targetDate,Integer start,Integer limit)  throws Exception{
		Integer result=0;
		
		String responseText = HttpUtils.getInstance().httpGet(API_HOST+"/todayUpdateCompany.htm?date="+targetDate+"&start="+start+"&limit="+limit, HttpUtils.CHARSET_UTF8);
