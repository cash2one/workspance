...
public class BaseDataStatisticTask implements ZZTask {
	...
	private static final String PUBLISH_INQUIRY_EXPORT = "publish_inquiry_export";// 询盘
	...
	public boolean exec(Date baseDate) throws Exception {
		...
		// 后台导出询盘
		String sql7 = "select count(*) from inquiry where '"
			+ DateUtil.toString(startDate, "yyyy-MM-dd")
			+ "'<= gmt_created and gmt_created < '"
			+ DateUtil.toString(endDate, "yyyy-MM-dd")
			+ "'"
		...
		result.put(PUBLISH_INQUIRY_EXPORT, analysis(PUBLISH_INQUIRY_EXPORT, targetDate, sql7));
