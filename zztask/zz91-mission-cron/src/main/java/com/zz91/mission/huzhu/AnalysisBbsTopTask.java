...
public class AnalysisBbsTopTask implements ZZTask {
	...
	public boolean exec(Date baseDate) throws Exception {
		...
		// 发问牛人
		analysisCowPersonQuestion(target);
		// 牛贴
		analysisCowStickers(target);
		// 牛问
		analysisCowQuestion(target);
		return true;
	}
	...
	private void analysisCowStickers(Long gmtTarget) {
		String sql="select visited_count,id,title from bbs_post where (check_status=1 or check_status=2) and is_del='0' " +
				" and post_time > '"+startdDate+"' and post_time<'"+endDate+"' and visited_count>0 and bbs_post_category_id not in (11) order by visited_count desc limit 10";
	...
	//统计牛问
	private void analysisCowQuestion(Long gmtTarget) {
		String sql="select visited_count,id,title from bbs_post where (check_status=1 or check_status=2) and is_del='0' " + 
				" and post_time > '"+startdDate+"' and post_time<'"+endDate+"' and visited_count>0 and bbs_post_category_id = 11 order by visited_count desc limit 10";
		final List<AnalysisBbsTop> list=new ArrayList<AnalysisBbsTop>();
		DBUtils.select("ast", sql, new IReadDataHandler() {
		...
		for (AnalysisBbsTop analysisBbsTop : list) {
			insertBbsTop("question", analysisBbsTop.getTitle(), analysisBbsTop.getTargerId(), analysisBbsTop.getNum(), gmtTarget);
		}
	}
	...
	private void analysisCowPerson(Long gmtTarget) {
		String sql2="select count(*) a,bbs_user_profiler_id,account,post_time from bbs_post where is_del='0' and (check_status=1 or check_status=2) and " +
				" post_time>'"+startdDate+"' and post_time<'"+endDate+"' and bbs_user_profiler_id>0 and account is not null and bbs_post_category_id not in (11) group by account order by a desc limit 6";
		final List<AnalysisBbsTop> list =new ArrayList<AnalysisBbsTop>();
	...
	//统计发问牛人
	private void analysisCowPersonQuestion(Long gmtTarget) {
		String sql2="select count(*) a,bbs_user_profiler_id,account,post_time from bbs_post where is_del='0' and (check_status=1 or check_status=2) and " +
				" post_time>'"+startdDate+"' and post_time<'"+endDate+"' and bbs_user_profiler_id>0 and account is not null and bbs_post_category_id = 11 group by account order by a desc limit 6";
		final List<AnalysisBbsTop> list =new ArrayList<AnalysisBbsTop>();
		...
		for (AnalysisBbsTop analysisBbsTop : list) {
			...
			if (map.get("name")!=null && map.get("name")!="") {
				insertBbsTop("profileQuestion", map.get("name"),analysisBbsTop.getTargerId() , analysisBbsTop.getNum(), gmtTarget);
			} else{
				insertBbsTop("profileQuestion", analysisBbsTop.getTitle(), analysisBbsTop.getTargerId(), analysisBbsTop.getNum(),gmtTarget);
			}
		}
	}
