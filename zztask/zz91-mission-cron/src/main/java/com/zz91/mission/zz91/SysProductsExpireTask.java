 ...
 */
public class SysProductsExpireTask implements ZZTask {
	...
	private void queryCompanyId(int i, int limit, String targetDate) throws Exception {
		...
		DBUtils.select("ast",
				"select id ,company_id,title from products  where check_status='1' and is_del='0' and is_pause='0' and refresh_time<='2016-02-29' and refresh_time>='2014-12-06' and expire_time<'" + targetDate + "' order by id desc limit " + (i * limit) + "," + limit,
	...
	private int queryMaxSize(String targetDate) throws Exception {
		...
		try {
			...
			rs = conn.createStatement().executeQuery(
					"select count(0) from products where check_status='1' and is_del='0' and is_pause='0' and refresh_time<='2016-02-29' and refresh_time>='2014-12-06' and expire_time<'" + targetDate + "'");
			while (rs.next()) {
				max = rs.getInt(1);
			}
		} catch (Exception e) {
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		SysProductsExpireTask sysExpireProductInqueryTask = new SysProductsExpireTask();
