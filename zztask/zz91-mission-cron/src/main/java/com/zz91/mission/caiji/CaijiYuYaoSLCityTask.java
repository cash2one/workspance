...
public class CaijiYuYaoSLCityTask implements ZZTask{
    
    final static String PRICE_OPERTION = "price_caiji";
    final static String DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";
    CaiJiCommonOperate operate = new CaiJiCommonOperate();

    final static Map<String, String> YUYAOSLCITY_MAP = new HashMap<String, String>();
    static{
        // httpget 使用的编码 不然会有乱码
        YUYAOSLCITY_MAP.put("charset", "utf-8");
        
        //采集地址 正式
        YUYAOSLCITY_MAP.put("url", "http://www.ex-cp.com/plastic/list-47.html");
        YUYAOSLCITY_MAP.put("contentUrl", "http://www.l-zzz.com");
        YUYAOSLCITY_MAP.put("listStart", "<strong>ABS</strong>"); // 列表页开头
        YUYAOSLCITY_MAP.put("listEnd", "<div class=\"m_r f_l\">"); // 列表页结尾
        
        YUYAOSLCITY_MAP.put("contentStart", "<div class=\"content\" id=\"article\">");
        YUYAOSLCITY_MAP.put("contentEnd", "<div class=\"b10 c_b\">&nbsp;</div>");
        YUYAOSLCITY_MAP.put("split", "<li>");
        YUYAOSLCITY_MAP.put("splitLink", "\"");
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
            String content ="";
            try {
                url = YUYAOSLCITY_MAP.get("url");
                content = operate.httpClientHtml(url, YUYAOSLCITY_MAP.get("charset"));
            ...
            if(content==null){
                errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>浙江塑料城网上交易市场-余姚塑料城"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />"
                ...
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'余姚塑料城("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            Integer start = content.indexOf(YUYAOSLCITY_MAP.get("listStart"));
            Integer end = content.indexOf(YUYAOSLCITY_MAP.get("listEnd"));
            String result = content.substring(start, end);
            String[] str = result.split(YUYAOSLCITY_MAP.get("split"));
            ...
            if(list.size()<1){
                errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>浙江塑料城网上交易市场-余姚塑料城"+"("+typeId+")"+"</font></a>查看来源网站<br />"
                                + "如果有，请<a href='http://apps2.zz91.com/task/job/definition/doTask.htm?jobName=CaijiYuYaoSLCityTask&start="+DateUtil.getDate(new Date(), "yyyy-MM-dd")+" 00:00:00'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'余姚塑料城("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            for(String aStr:list){
                ...
                if(StringUtils.isEmpty(linkStr)){
                    ...
                    String[] alink = linkStr.split(YUYAOSLCITY_MAP.get("splitLink"));
                    if(alink.length>0){
                        String resultTitle = Jsoup.clean(linkStr, Whitelist.none()).replace(formatDate, format);
                        ...
                        resultContent = operate.getContent(YUYAOSLCITY_MAP, newUrl);
        ...
        if (flagFailure != 0){
            String content = "余姚塑料城"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'余姚塑料城("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public static void main(String[] args) {
        ...
        CaijiYuYaoSLCityTask js=new CaijiYuYaoSLCityTask();
        try {
        	js.exec(DateUtil.getDate("2016-02-17", "yyyy-MM-dd"));
