...
public class AnalysisCsLogTask implements ZZTask{
	...
	private void analysis(Map<String, AnalysisCsLog> map, Date startDate, int start, int limit){
		...
		for (CsLog log : list) {
			...
			if(log.getCallType()==0 && log.getSituation()==1){
				analysis.setServiceCall(analysis.getServiceCall()+1);
			}
			if(log.getCallType()==1 && log.getSituation()==1){
	...
	public static void main(String[] args) {
		...
		try {
			csLog.clear(DateUtil.getDate("2015-02-27", "yyyy-MM-dd"));
			csLog.exec(DateUtil.getDate("2015-02-27", "yyyy-MM-dd"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
