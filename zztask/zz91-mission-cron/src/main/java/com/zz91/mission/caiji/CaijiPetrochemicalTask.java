...
public class CaijiPetrochemicalTask implements ZZTask{
    ...
    static{
        ...
        //采集地址 正式
        PETRIFACTION_MAP.put("url", "http://info.1688.com/news/newsListByTags/v5003220-l14958.html");
        PETRIFACTION_MAP.put("contentUrl", "http://info.1688.com");
        PETRIFACTION_MAP.put("listStart", "<dl class=\"li23 listcontent1\">"); // 列表页开头
        PETRIFACTION_MAP.put("listEnd", "<!--分页 start-->"); // 列表页结尾
        
        PETRIFACTION_MAP.put("contentStart", "<div class=\"d-content\">");
        PETRIFACTION_MAP.put("contentEnd", "(责任编辑");
        PETRIFACTION_MAP.put("split", "<li>");
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
            String content ="";
            try {
                url = PETRIFACTION_MAP.get("url");
                content = operate.httpClientHtml(url, PETRIFACTION_MAP.get("charset"));
            ...
            if(content==null){
                errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>阿里巴巴-国内石化"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />"
                                + "如果有，请<a href='http://admin1949.zz91.com/web/zz91/auto/caiji/caiji_albb_petrifaction.htm?typeId="+typeId +"'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'国内石化("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            if(DateUtil.toString(DateUtil.getFirstDateThisWeek(), "yyyy-MM-dd").equals(DateUtil.toString(baseDate, "yyyy-MM-dd"))){
            	formatDate = DateUtil.toString(DateUtil.getDateAfterDays(baseDate, -3), "MM月dd日");
            }else{
            	formatDate = DateUtil.toString(DateUtil.getDateAfterDays(baseDate, -1), "MM月dd日");
            }
            // 检查日期是否对应今天
            List<String> list = new ArrayList<String>();
            for (String s : str) {
                if (s.indexOf(formatDate) != -1) {
                    if(s.contains("企业PVC出厂价") || s.contains("企业ABS出厂价") || s.contains("企业PP出厂价")
                            || s.contains("LLDPE出厂价") || s.contains("企业LDPE出厂价") || s.contains("企业HDPE出厂价")
                            || s.contains("企业GPPS出厂价") || s.contains("企业HIPS出厂价")) {
                        list.add(s);
                    }
                }
            }
            if(list.size()<1){
                errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>阿里巴巴-国内石化"+"("+typeId+")"+"</font></a>查看来源网站<br />"
                                + "如果有，请<a href='http://admin1949.zz91.com/web/zz91/auto/caiji/caiji_albb_petrifaction.htm?typeId="+typeId +"'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'国内石化("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            for(String aStr:list){
                ...
                if(StringUtils.isEmpty(linkStr)){
                    ...
                    if(alink.length>0){
                        ...
                        resultContent = operate.getContent(PETRIFACTION_MAP, newUrl);
                        // 获取内容
...
//                                           + "阿里巴巴</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
        ...
        if (flagFailure != 0){
            String content = "国内石化"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
                    +errContentUrl + "如果有，请<a href='http://admin1949.zz91.com/web/zz91/auto/caiji/caiji_albb_petrifaction.htm?typeId="+typeId +"'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'国内石化("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                            + "阿里巴巴</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'国内石化("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public static void main(String[] args) {
        ...
        CaijiPetrochemicalTask js=new CaijiPetrochemicalTask();
        try {
