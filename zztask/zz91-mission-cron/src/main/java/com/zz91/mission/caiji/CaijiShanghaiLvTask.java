...
public class CaijiShanghaiLvTask implements ZZTask{
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
            ...
            if(content==null){
                errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>上期所-沪铝"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />"
                                + "如果有，请<a href='http://admin1949.zz91.com/web/zz91/auto/caiji/caiji_shfe.htm?typeId="+typeId +"'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'沪铝("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            if (contentResult.indexOf(format)!= -1) {
               ...
               /*if (flg != null && flg > 0) {
                ...
                errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>上期所-沪铝"+"("+typeId+")"+"</font></a>查看来源网站<br />"
                                + "如果有，请<a href='http://admin1949.zz91.com/web/zz91/auto/caiji/caiji_shfe.htm?typeId="+typeId +"'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'沪铝("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
        ...
        if (flagFailure != 0){
            String content = "沪铝"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'沪铝("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public static void main(String[] args) {
        ...
        CaijiShanghaiLvTask js=new CaijiShanghaiLvTask();
        try {
