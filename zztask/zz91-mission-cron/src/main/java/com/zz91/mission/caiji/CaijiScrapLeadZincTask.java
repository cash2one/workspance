...
public class CaijiScrapLeadZincTask implements ZZTask {
	...
	static {
		...
		FB_MAP.put(
				"listEnd",
				"<p style=\"background:none; padding:0px 0 0 60px; float: left; \" align=\"center\">"); // 列表页结尾
            ...
            for(String urlStr:urlArray){
                ...
                if(content==null){
                    errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                            + "<font color='#FF0000'>富宝-废铅废锌"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />";
                    LogUtil.getInstance().log(
                            "caiji-auto", PRICE_OPERTION, null, "{'title':'废铅废锌("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                ...
                if(list.size()<1){
                    errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                            + "<font color='#FF0000'>富宝-废铅废锌"+"("+typeId+")"+"</font></a>查看来源网站<br />";
                    LogUtil.getInstance().log(
                            "caiji-auto", PRICE_OPERTION, null, "{'title':'废铅废锌("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                                    + "富宝</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
                    flagFailure++;
...
//                          resultContent = resultContent.toLowerCase();
                            ...
                            if(resultContent.contains("<table>")){
                            	resultContent="<p>ZZ91再生网"+formatZero+"消息</p>"+"<p>"+resultContent;
                            }
                           ...
                           if (flg != null && flg > 0) {
                               ...
                               LogUtil.getInstance().log(
                                       "caiji-auto", PRICE_OPERTION, null, "{'title':'废铅废锌("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
        ...
        if (flagFailure != 0){
            String content = "废铅废锌"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
	...
	public static void main(String[] args) {
		...
		CaijiScrapLeadZincTask js = new CaijiScrapLeadZincTask();
		try {
			js.exec(DateUtil.getDate("2014-07-04", "yyyy-MM-dd"));
