 ...
 * 给长期有效的供求普会留言
 * @author root
 *长期有效标准 9999-12-31 23:59:59
 */
public class SysLongValidityProductTask implements ZZTask{
	 private final static String LOG_DATE_FORMAT = "yyyy-MM-dd";
	 private final static String DB="ast";
	 final String targetDate=DateUtil.toString(new Date(), LOG_DATE_FORMAT);
	@Override
	...
	public boolean exec(Date baseDate) throws Exception {
		String basetime="9999-12-31 23:59:59";
		int max=queryMaxSize(basetime);
		int limit=1000;
		for(int i=0;i<(max/limit+1);i++){
			queryCompanyId(i,limit,basetime);
		}
		return true;
	}
	private void queryCompanyId(int i,int limit,String basetime)throws Exception{
		final Map<String, String>root=new HashMap<String, String>();
		//获取帐号
		final Map<String, String> accountMap = new HashMap<String, String>();
		//获取查询条件的供求（不区分普会和高会）数据maps
		...
		DBUtils.select("ast", "select id ,company_id,title from products  where  check_status='1' and is_del='0' and is_pause='0' and refresh_time<'2014-01-01' and expire_time='"+basetime+"' order by id desc limit "+(i*limit)+","+limit, new IReadDataHandler() {
			@Override
	 ...
	 * 将留言信息插入到inquiry 表中
	 * @param productMap 
	 * @param account  发布供求信息的帐号
	 */
	private void sysInquiry(Map<String, String> productMap,String account){

			String title="您发布的信息【"+productMap.get("title")+"】长时间未更新"; 
			String content="您发布的【"+productMap.get("title")+"】已经有一段时间未更新，为增加信息的活跃度、提高信息的排名，建议您及时刷新供求信息。您可以前往生意管家商机管理-管理供求页面根据提示修改相关信息";
			...
			String insertSql = "INSERT INTO inquiry (" + "title,"
					...
					+ title + "','"
					+ content + "',1,"
					+ productMap.get("companyId") + ",1,'admin','" + receiverAccount + "',0,0,now(),now(),now())";
	 ...
	 * @throws Exception
	 */
	private int queryMaxSize(String basetime) throws Exception{
		int max=0;
		Connection conn=null;
		ResultSet rs=null;
		try {
			conn=DBUtils.getConnection(DB);
			rs=conn.createStatement().executeQuery("select count(0) from products where check_status='1' and is_del='0' and is_pause='0' and refresh_time<'2014-01-01' and expire_time='"+basetime+"'");
			while(rs.next()){
				max=rs.getInt(1);
			}
		} catch (Exception e) {
	...
	public boolean clear(Date baseDate) throws Exception {
	 ...
	 public static void main(String [] args) throws ParseException, Exception{
	    	...
	    	SysLongValidityProductTask sysLongValidityProductTask= new SysLongValidityProductTask();
