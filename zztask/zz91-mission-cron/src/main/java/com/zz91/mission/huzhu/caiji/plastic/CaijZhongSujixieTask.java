...
/**
 * 互助抓取 中塑在线-知识-机械  对应bbs_post 中的类别:废料学院-行业知识-废塑料
 * 采集地址:http://info.21cp.com/zhishi/Jixie/
 * @author root
 *
 */
public class CaijZhongSujixieTask implements ZZTask {
    ...
    static{
        ...
        //采集地址 正式
        QQZSS_MAP.put("url", "http://info.21cp.com/zhishi/Jixie/");
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
        	for(Integer i=1;i<12;i++){
        		...
        		try {
        			if(i==1){
        				url="http://info.21cp.com/zhishi/Jixie/";
        			}else {
        				url="http://info.21cp.com/zhishi/ShowClass.asp?ClassID=144&page="+i;
        ...
        if (flagFailure != 0){
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'中塑在线-知识 -机械("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public void sendEmail(String url,Integer typeId,String defaultTime) throws ParseException{
    	String errContentUrl =" 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                + "<font color='#FF0000'>中塑在线-知识 -机械"+"("+typeId+")"+"</font></a>查看来源网站<br />"
			...
			LogUtil.getInstance().log(
                "caiji-auto", PRICE_OPERTION, null, "{'title':'知识 -机械("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
    ...
    public static void main(String[] args) {
        ...
        CaijZhongSujixieTask js=new CaijZhongSujixieTask();
        try {
