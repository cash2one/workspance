...
public class CaijiMornForecastTask implements ZZTask{
    
    final static String PRICE_OPERTION = "price_caiji";
    final static String DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";
    CaiJiCommonOperate operate = new CaiJiCommonOperate();

    final static Map<String, String> MornForecast_MAP = new HashMap<String, String>();
    static{
        // httpget 使用的编码 不然会有乱码
        MornForecast_MAP.put("charset", "utf-8");
        
        //采集地址 正式
        MornForecast_MAP.put("url", "http://so.spcce.com/Info/%E6%97%A9%E9%97%B4%E7%82%B9%E8%AF%84-0-0-0-1.html");
        MornForecast_MAP.put("contentUrl", "");
        MornForecast_MAP.put("listStart", "<div class=\"left align-left\">"); // 列表页开头
        MornForecast_MAP.put("listEnd", "<div class=\"pages\">"); // 列表页结尾
        
        MornForecast_MAP.put("contentStart", "<div id=\"contentText\">");
        MornForecast_MAP.put("contentEnd", "<div class=\"Share\">");
        MornForecast_MAP.put("split", "<div class=\"zxListItem\">");
        MornForecast_MAP.put("splitLink", "\"");
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
            String content ="";
            try {
                url = MornForecast_MAP.get("url");
                content = operate.httpClientHtml(url, MornForecast_MAP.get("charset"));
            ...
            if(content==null){
                errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>爱塑网-早间预测"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />"
                                + "如果有，请<a href='http://apps2.zz91.com/task/job/definition/doTask.htm?jobName=CaijiMornForecastTask&start="+DateUtil.getDate(new Date(), "yyyy-MM-dd")+" 00:00:00'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'早间预测("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            Integer start = content.indexOf(MornForecast_MAP.get("listStart"));
            Integer end = content.indexOf(MornForecast_MAP.get("listEnd"));
            String result = content.substring(start, end);
            String[] str = result.split(MornForecast_MAP.get("split"));

            // 检查日期是否对应今天
            String formatDate = DateUtil.toString(baseDate, "yyyyyyy-MM-dd");
            ...
            if(list.size()<1){
                errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                        + "<font color='#FF0000'>爱塑网-早间预测"+"("+typeId+")"+"</font></a>查看来源网站<br />"
                                + "如果有，请<a href='http://apps2.zz91.com/task/job/definition/doTask.htm?jobName=CaijiMornForecastTask&start="+DateUtil.getDate(new Date(), "yyyy-MM-dd")+" 00:00:00'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
                LogUtil.getInstance().log(
                        "caiji-auto", PRICE_OPERTION, null, "{'title':'早间预测("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
            ...
            for(String aStr:list){
                ...
                if(StringUtils.isEmpty(linkStr)){
                    ...
                    String[] alink = linkStr.split(MornForecast_MAP.get("splitLink"));
                    if(alink.length>0){
                        String resultTitle = operate.changeLetter(Jsoup.clean(linkStr, Whitelist.none()));
                        resultTitle = resultTitle.substring(4);
                        String zaoJian = resultTitle.substring(resultTitle.indexOf("预测")-2, resultTitle.indexOf("预测"));
                        if (!zaoJian.equals("早间")) {
                            resultTitle = resultTitle.replace("预测", "早间预测");
                        }
                        String resultContent ="";
                        String newUrl = MornForecast_MAP.get("contentUrl") + alink[5]; 
                        resultContent = operate.getContent(MornForecast_MAP, newUrl);
                        int tmpStart = 0;
        ...
        if (flagFailure != 0){
            String content = "早间预测"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
                    +errContentUrl + "如果有，请<a href='http://apps2.zz91.com/task/job/definition/doTask.htm?jobName=CaijiMornForecastTask&start="+DateUtil.getDate(new Date(), "yyyy-MM-dd")+" 00:00:00'>"
                            + "<font color='#FF0000'>抓取</font></a>！";
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'早间预测("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                            + "爱塑网</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'早间预测("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public static void main(String[] args) {
        ...
        CaijiMornForecastTask js=new CaijiMornForecastTask();
        try {
