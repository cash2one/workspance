...
/**
 * 互助抓取 中金网-资讯-行业知识  对应bbs_post 中的类别:废料学院-行业知识-废金属
 * 采集地址:http://www.metalsinfo.com/news/list_pid_19-7.html
 * @author root
 *
 */
public class CaijimetalsinfoMetalTecknologTask implements ZZTask {
    ...
    static{
        ...
        //采集地址 正式
        QQZSS_MAP.put("url", "http://www.metalsinfo.com/news/list_pid_19.html");
        QQZSS_MAP.put("contentUrl", "http://www.metalsinfo.com");
        QQZSS_MAP. put("listStart", "<ul class=\"ul0\">"); // 列表页开头
        QQZSS_MAP.put("listEnd", "<div class=\"cP1\">"); // 列表页结尾
        QQZSS_MAP.put("split", "<li>");
        
        QQZSS_MAP.put("contentStart", "<div id='ndC'");
        QQZSS_MAP.put("contentEnd", "<div id=\"nd_share\"" );
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
        	for(Integer i=1;i<26;i++){
        		...
        		try {
        			if(i==1){
        				url="http://www.metalsinfo.com/news/list_pid_19.html";
        			}else {
						url="http://www.metalsinfo.com/news/list_pid_19-"+i+".html";
					}
        		...
        		for(String aStr:list){
        			...
        			if(aStr!=null){
        				String[] alink=aStr.split("</a>");
        				if (alink.length>=2){
        					String titleString=Jsoup.clean(alink[0], Whitelist.none());
        					if (titleString.equals("[行业标准]")||titleString.equals("[品种知识]")) {								
        						String linkStr="";
        						...
        						if(StringUtils.isEmpty(linkStr)){
        							...
        							continue;
            							...
            							String resultTitle = Jsoup.clean(alink[1], Whitelist.none());
            							Integer starts = alink[1].indexOf("/");
            							if (starts==-1) {
            								sendEmail(url,typeId,defaultTime);
                							continue;
										}
            							Integer ends = alink[1].indexOf("target");
            							if (ends==-1) {
            								sendEmail(url,typeId,defaultTime);
                							continue;
										}
            							linkStr = QQZSS_MAP.get("contentUrl")+alink[1].substring(starts,(ends-2));
        ...
        if (flagFailure != 0){
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'中金网-资讯-行业知识知识("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
                            + "中金网</a>','defaultTime':'"+defaultTime+"','catchTime':'"+catchTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
            return true;
        }
    }
    public void sendEmail(String url,Integer typeId,String defaultTime) throws ParseException{
    	String errContentUrl =" 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                + "<font color='#FF0000'>中金网-资讯-行业知识"+"("+typeId+")"+"</font></a>查看来源网站<br />"
			...
			LogUtil.getInstance().log(
                "caiji-auto", PRICE_OPERTION, null, "{'title':'资讯-行业知识("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                        + "中金网</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
    ...
    public static void main(String[] args) {
        ...
        CaijimetalsinfoMetalTecknologTask js=new CaijimetalsinfoMetalTecknologTask();
        try {
