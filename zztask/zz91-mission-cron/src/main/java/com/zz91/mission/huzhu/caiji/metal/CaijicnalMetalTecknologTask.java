...
/**
 * 互助抓取 中铝网-新闻-铝业常识  对应bbs_post 中的类别:废料学院-行业知识-废金属
 * 采集地址:http://news.cnal.com/knowledge/
 * @author root
 *
 */
public class CaijicnalMetalTecknologTask implements ZZTask{
    ...
    static{
        ...
        //采集地址 正式
        QQZSS_MAP.put("url", "http://news.cnal.com/knowledge/");
        QQZSS_MAP.put("contentUrl", "");
        QQZSS_MAP. put("listStart", "<div class=\"grid-news-list\">"); // 列表页开头
        QQZSS_MAP.put("listEnd", "<div class=\"page-new\">"); // 列表页结尾
        QQZSS_MAP.put("split", "<h3>");
        
        QQZSS_MAP.put("contentStart", "<div class=\"marketcontent\">");
        QQZSS_MAP.put("contentEnd", "<div class=\"page-new\">" );
    ...
    public boolean exec(Date baseDate) throws Exception {
        ...
        do {
        	for(Integer i=1;i<26;i++){
        		...
        		try {
        			if(i==1){
        				url="http://news.cnal.com/knowledge/";
        			}else {
						url="http://news.cnal.com/knowledge/index_"+i+".shtml";
					}
        			...
        			if(aStr!=null){					
        						...
        						if(StringUtils.isEmpty(linkStr)){
        							...
        							continue;
            							...
            							Integer ends = aStr.indexOf(".shtml");
            							if (ends==-1) {
            								sendEmail(url,typeId,defaultTime);
                							continue;
										}
            							linkStr = aStr.substring(starts,(ends+6));
        ...
        if (flagFailure != 0){
            ...
            LogUtil.getInstance().log(
                    "caiji-auto", PRICE_OPERTION, null, "{'title':'中铝网-新闻-铝业常识知识("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
    ...
    public void sendEmail(String url,Integer typeId,String defaultTime) throws ParseException{
    	String errContentUrl =" 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                + "<font color='#FF0000'>中铝网-新闻-铝业常识"+"("+typeId+")"+"</font></a>查看来源网站<br />"
    ...
    public static void main(String[] args) {
        ...
        CaijicnalMetalTecknologTask js=new CaijicnalMetalTecknologTask();
        try {
