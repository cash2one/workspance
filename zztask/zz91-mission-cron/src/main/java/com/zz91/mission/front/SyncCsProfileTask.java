...
public class SyncCsProfileTask implements ZZTask {
	...
	public static void main(String[] args) {
		...
		try {
			task.exec(DateUtil.getDate("2014-9-16", "yyyy-MM-dd"));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		System.out.println("开始同步："+companyId);
		...
		sb.append(" c.domain_zz91, c.is_block,c.industry_code");
		...
		DBUtils.select("ast", sb.toString(), new IReadDataHandler() {
			...
			public void handleRead(ResultSet rs) throws SQLException {
				while (rs.next()) {
					...
					column.put("industry_code", rs.getString(14));
				}
			}
		});
		...
		DBUtils.select("ast", sb2.toString(), new IReadDataHandler() {
			...
			public void handleRead(ResultSet rs) throws SQLException {
				while (rs.next()) {
					...
					column.put("gmt_last_login", rs.getString(4));
		...
		try {
			...
			if(StringUtils.isEmpty((String) column.get("gmt_last_login"))){
				pstat.setDate(16, valueofDate(column.get("gmt_last_login")));
			}else{
				pstat.setString(16, valueofString(column.get("gmt_last_login")));
			}
			...
			pstat.setString(25, valueofString(column.get("industry_code")));
			pstat.executeUpdate();
		} catch (SQLException e) {
			if(error.size()<=100){
