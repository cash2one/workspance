 ...
 * 给近期供求过期的普会留言
 * @author root
 *近期的标准：2014-01-01 之后到今天有刷新过供求(刷新数据)
 */
public class SysRecentExpireProductTask implements ZZTask{
	...
	private void queryCompanyId(int i,int limit,String targetDate)throws Exception{
		...
		DBUtils.select("ast", "select id ,company_id,title from products  where check_status='1' and is_del='0' and is_pause='0' and refresh_time>='2014-01-01' and expire_time<'"+targetDate+"' order by id desc limit "+(i*limit)+","+limit, new IReadDataHandler() {
			@Override
	...
	private void sysInquiry(Map<String, String> productMap,String account){
			...
			String title="您近期发布的信息【"+productMap.get("title")+"】已过期";
			String content="您近期发布的供求信息【"+productMap.get("title")+"】已过期，为保障您的供求信息能继续获得客户的关注，请您尽快前往生意管家商机管理-管理供求-已过期列表页刷新供求信息";
	...
	private int queryMaxSize(String targetDate) throws Exception{
		...
		try {
			...
			rs=conn.createStatement().executeQuery("select count(0) from products where check_status='1' and is_del='0' and is_pause='0' and refresh_time>='2014-01-01' and expire_time<'"+targetDate+"'");
			while(rs.next()){
				max=rs.getInt(1);
			}
		} catch (Exception e) {
	...
	public boolean clear(Date baseDate) throws Exception {
	 ...
	 public static void main(String [] args) throws ParseException, Exception{
	    	...
	    	SysRecentExpireProductTask sysRecentExpireProductTask= new SysRecentExpireProductTask();
	    	sysRecentExpireProductTask.exec(DateUtil.getDate("2016-3-18 12:12:12", "yyyy-MM-dd"));
