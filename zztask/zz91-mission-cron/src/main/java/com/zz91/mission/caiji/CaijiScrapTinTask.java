...
public class CaijiScrapTinTask implements ZZTask{
    ...
    public boolean exec(Date baseDate) throws Exception {
            ...
            for(String urlStr:urlArray){
                ...
                if(content==null){
                    errContentUrl = errContentUrl + " 抓取来源网站内容失败！请点击<a href='" +  url +"'>"
                            + "<font color='#FF0000'>富宝-废锡"+"("+typeId+")"+"</font></a>查看来源网站是否更改地址！<br />";
                    LogUtil.getInstance().log(
                            "caiji-auto", PRICE_OPERTION, null, "{'title':'废锡("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                ...
                if(list.size()<1){
                    errContentUrl = errContentUrl + " 抓取失败，来源网站没有数据可抓取！请点击<a href='" +  url +"'>"
                            + "<font color='#FF0000'>富宝-废锡"+"("+typeId+")"+"</font></a>查看来源网站<br />";
                    LogUtil.getInstance().log(
                            "caiji-auto", PRICE_OPERTION, null, "{'title':'废锡("+typeId+")','type':'failure','url':'<a href='" + url +"' target='_blank'>"
                                    + "富宝</a>','defaultTime':'"+defaultTime+"','date':'"+ DateUtil.toString(new Date(), DATE_FORMAT) + "'}");
                    flagFailure++;
                    ...
                    if(StringUtils.isEmpty(linkStr)){
                        ...
                        if(alink.length>0){
                           ...
                           if (flg != null && flg > 0) {
                               ...
                               LogUtil.getInstance().log(
                                       "caiji-auto", PRICE_OPERTION, null, "{'title':'废锡("+typeId+")','type':'success','url':'<a href='" + url +"' target='_blank'>"
        ...
        if (flagFailure != 0){
            String content = "废锡"+"("+typeId+")"+"未抓取"+flagFailure+"条。分别为：<br />"
    ...
    public static void main(String[] args) {
        ...
        CaijiScrapTinTask js=new CaijiScrapTinTask();
        try {
