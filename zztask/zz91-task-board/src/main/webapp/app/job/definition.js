...
com.zz91.task.board.job.definition.Field = ["id","jobName","cron","jobGroup","jobClassName","description","isInUse","jobClasspath","endTime","groupId"];
			...
			menu:[{
				...
				id:"group",
					store:new Ext.data.JsonStore( {
						fields : [ "account", "name" ],
						autoLoad:true,
						url : Context.ROOT + "/job/definition/queryGroup.htm"
				})
			 },"-",{
			    iconCls:"fav16",
            	text:"归类",
            	handler:function(btn){
	                ...
	                for (var i=0,len = row.length;i<len;i++){
	                  com.zz91.task.board.job.definition.addGroup(groupId,row[i].get("id"));
	                }
            	}
             },"-",{
			    iconCls:"favb16",
            	...
            	handler:function(btn){
	            	//TODO 查找数据
            		var groupId=Ext.getCmp("group").getValue();
            		_store.baseParams["groupId"]=groupId;
...
com.zz91.task.board.job.definition.addGroup = function(groupId,definitionId){
	var url=Context.ROOT+"/job/definition/addGroupDefinition.htm";
	Ext.Ajax.request({
        url:url,
        params:{"groupId":groupId,"definitionId":definitionId},
        success:function(response,opt){
        	var obj =Ext.decode(response.responseText);
            Ext.Msg.alert(obj.data);
            if(obj.success){
            	Ext.getCmp("definitiongrid").getStore().reload();
            }
        },
