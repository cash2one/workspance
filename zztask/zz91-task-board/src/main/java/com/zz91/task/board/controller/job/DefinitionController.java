...
public class DefinitionController extends BaseController {
	...
	@RequestMapping
	public ModelAndView queryDefinition(Map<String, Object> out, Pager<JobDefinition> page,Integer groupId) {
		if(groupId==null){
			page = jobDefinitionService.pageJobDefinition(null, JobDefinitionService.GROUP, page);
		}else{
		    page = groupDefinitionService.queryDefinitionByGroupId(page, groupId);
		}
		return printJson(page, out);
	}

	final static String UPLOAD_ROOT = "/mnt/data/task";
	...
	public ModelAndView createDefinition(Map<String, Object> out,
		...
		do {
			...
			if (id != null && id.intValue() > 0) {
				groupDefinitionService.insert(jobDefinition.getGroupId(), id);
	...
	public ModelAndView queryDefinitionById(Map<String, Object> out, Integer id) {
		Integer groupId=groupDefinitionService.queryGroupIdByDefiId(id);
...
//		page.setRecords(list);
	...
	public ModelAndView updateDefinition(Map<String, Object> out,
		...
		if (impact != null && impact.intValue() > 0) {
			...
			// 判断任务是否启用状态
			result.setSuccess(true);
			groupDefinitionService.updateGroupIdByDefiId(definition.getGroupId(), definition.getId());
...
//	}
	...
	public ModelAndView deleteDefinition(Map<String, Object> out, Integer id) {
		...
		groupDefinitionService.delete(id);
...
//			}
	...
	@RequestMapping
	public ModelAndView queryAllGroup(Map<String, Object> out,Pager<JobGroup> page,String groupName) {
		if(StringUtils.isNotEmpty(groupName)){
			page=jobGroupService.queryGroupByKeywords(page, groupName);
		}else{
			page=jobGroupService.queryAllGroup(page);
		}
		return printJson(page, out);
	}
	@RequestMapping
	public ModelAndView addGroup(Map<String, Object> out,JobGroup jobGroup) {
		ExtResult result = new ExtResult();
		result.setSuccess(false);
		Integer groupMark=0;
		//添加操作
		if(jobGroup.getId()==null){
			JobGroup group=jobGroupService.queryGroupByGroupName(jobGroup.getGroupName());
			if(group==null){
				jobGroupService.addGroupInfo(jobGroup.getGroupName());
				result.setSuccess(true);
				result.setData("添加成功");
			}else{
				if(group.getIsDel()==1){
					jobGroupService.delete(group.getId(), 0);
					result.setSuccess(true);
					result.setData("添加成功");
				}else{
					result.setSuccess(true);
					result.setData("添加失败，该分组已经存在，请换分组名称");
				}
			}
		}else{
			//修改操作
			groupMark=jobGroupService.updateGroupById(jobGroup.getId(), jobGroup.getGroupName());
			if(groupMark>0){
				result.setSuccess(true);
				result.setData("修改成功");
			}else{
	...
	@RequestMapping
	public ModelAndView deleteGroup(Map<String, Object> out,Integer id) {
		ExtResult result = new ExtResult();
		result.setSuccess(false);
		Integer deleteMark=jobGroupService.delete(id,1);
		if(deleteMark>0){
			result.setSuccess(true);
		}
		return printJson(result, out);
	...
	@RequestMapping
	public ModelAndView init(Map<String, Object> out,Integer id) {
		JobGroup jobGroup=jobGroupService.queryGroupById(id);
		return printJson(jobGroup, out);
	}
	@RequestMapping
	public ModelAndView queryGroup(Map<String, Object> out) { 
		List<Map<String,Object>> list=new ArrayList<Map<String,Object>>();
		List<JobGroup> listGroup=jobGroupService.queryAllGroup();
		for(JobGroup group:listGroup){
			Map<String,Object> map=new HashMap<String,Object>();
			map.put("account", group.getId());
	...
	@RequestMapping
	public ModelAndView addGroupDefinition(Map<String, Object> out,Integer groupId,Integer definitionId) {
		ExtResult result = new ExtResult();
		result.setSuccess(false);
		Integer mark=0;
		if(groupId!=null&&definitionId!=null){
			Integer gId=groupDefinitionService.queryGroupIdByDefiId(definitionId);
			if(gId!=null){
				groupDefinitionService.updateGroupIdByDefiId(groupId, definitionId);
				result.setSuccess(true);
				result.setData("分组成功");
			}else{
				mark=groupDefinitionService.insert(groupId, definitionId);
			}
		}
		if(groupId==null){
			result.setData("请选择你需要的分组名称");
		}
		if(definitionId==null){
			result.setData("请选择你要添加或修改分组的任务");
		}
		if(mark>0){
			result.setSuccess(true);
			result.setData("分组成功");
